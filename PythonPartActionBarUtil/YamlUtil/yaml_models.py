...
class Base(BaseModel):
    ...
    def append_path(cls, file: str, path: str) -> str:
        """Append directory folder to filde path.
...
class AppConfig(Base):
    ...
    def get_icon_path(self, icon):
        return f"{self.installation.target_location}\\PythonPartsActionbar\\AllepPlugins\\{self.plugin.developer}\\{self.plugin.name}\\{icon}"
    ...
    def create_button(
        ...
        """
        icon_path   = f"{self.installation.target_location}\\PythonPartsActionbar\\AllepPlugins\\{self.plugin.developer}\\{self.plugin.name}"
        script_path = f"{self.installation.target_location}\\Library\\AllepPlugins\\{self.plugin.developer}\\{self.plugin.name}"
        attrs = {
            "id": event_id,
    ...
    def get_file_location(self) -> str:
        """ Get actb and npd file location.

        Returns:
            str: file location
        """
        return f"{self.installation.get_install_location()}\\{self.plugin.developer}\\{self.plugin.name}\\{self.plugin.name}"
    ...
    def create_actb_file(self) -> ET.Element:
        ...
        # Add NameID in group
        npd_hash                = AllPlanUtility.GetPluginNameHash(f"{self.installation.get_update_target_location()}\\PythonPartsActionbar\\AllepPlugins\\{self.plugin.developer}\\{self.plugin.name}\\{self.plugin.name}.npd")
    ...
    def write_file(self):
        ...
        if self.tools:
            ...
            os.makedirs(f"{self.installation.get_install_location()}\\{self.plugin.developer}\\{self.plugin.name}", exist_ok=True)

            with open(f"{self.installation.get_install_location()}\\{self.plugin.developer}\\{self.plugin.name}\\{self.plugin.name}.npd", "wb") as node_file:
        ...
        if self.tools and self.task_area:
            ...
            with open(f"{self.installation.get_install_location()}\\{self.plugin.developer}\\{self.plugin.name}\\{self.plugin.name}.actb", "wb") as node_file:
