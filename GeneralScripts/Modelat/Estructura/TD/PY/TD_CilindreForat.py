...
print('Load TD_CilindreForat.py')
...
def create_element(build_ele, doc):
    ...
    TDCilindreForat = CilindreForat(build_ele.zUnique.value, build_ele.Ample.value, build_ele.BarraAltura.value, build_ele.BarraGruix.value,
                                      build_ele.FounColor.value, build_ele.BarraLayer.value)

    #build_ele.BarraAltura.value = build_ele.BarraAmple.value

    if not TDCilindreForat.is_valid():
        return ([], [])

    handle_list = TDCilindreForat.create_handles()
    views = [View2D3D ([AllplanBasisElements.ModelElement3D(common_props, TDCilindreForat.create())])]

    attr_list = [AllplanBaseElements.AttributeString(2103, "TH TDCilindreForat ")]

    '''
    pythonpart = PythonPart ("TD_CilindreForat",
...
class CilindreForat():
    ...
    def __init__(self, zUnique, Ample, BarraAltura, BarraGruix,
                 FounColor, BarraLayer, alturaBox = 5):
        """ initialize
    ...
    def __repr__(self):
        ...
        return 'TD_CilindreForat(zUnique=%s, Ample=%s , BarraAltura =%s, BarraGruix=%s'\
    ...
    def filename(self):
        ...
        Returns:
            Script filename
        """
        return "TD_CilindreForat.py"

    def is_valid(self):
        """
        Check for valid values
        """
        if self.Ample<= 0 and self.alturaBox <= 0:
            return False

        return True

    def getTDType(self) :
        return "CilindreForat"
    ...
    def create_cilinder_cancam1(self):
        ...
        cylinderint = AllplanGeo.Cylinder3D(
            refPlacement=   AllplanGeo.AxisPlacement3D(AllplanGeo.Point3D(0, self.Ample/2, 0 ) ),
            radiusMajor=    12,
            radiusMinor=    11,
            apex=           AllplanGeo.Point3D(0, 0, self.alturaBox)
        )

        #if the cylinder should be created as a polyhedron, the tessellation must be performed

        error_code, polyhedronint = AllplanGeo.CreatePolyhedron(cylinderint, 6)
        ...
        error_code, polyhedron = AllplanGeo.CreatePolyhedron(cylinder, 6)
        ...
        if error_code == AllplanGeo.eOK:
            ...
            profile_points =   [
                                AllplanGeo.Point3D(0 , self.Ample/2, -self.BarraAltura),
                                AllplanGeo.Point3D(0  +0.1, self.Ample/2, -self.BarraAltura),
                                AllplanGeo.Point3D(0  +0.1, self.Ample/2 +0.1, -self.BarraAltura),
                                AllplanGeo.Point3D(0  , self.Ample/2 +0.1, -self.BarraAltura),
                                AllplanGeo.Point3D(0 , self.Ample/2, -self.BarraAltura),
                                ]
            ...
            path_points =  [AllplanGeo.Point3D(0 , self.Ample/2, -self.BarraAltura),
                            AllplanGeo.Point3D(0 , self.Ample/2, -self.BarraAltura+20),
                            ]
    ...
    def create_cilinder_cancam11(self):
        ...
        #----- create cylinder interior
        cylinderint = AllplanGeo.Cylinder3D(
            refPlacement=   AllplanGeo.AxisPlacement3D(AllplanGeo.Point3D(0 - 10, self.Ample/2, self.BarraAltura ) ),
            radiusMajor=    6.5,
            radiusMinor=    6.5,
        ...
        if error_code == AllplanGeo.eOK:
            ...
            profile_points =   [
                                AllplanGeo.Point3D(0 - 10, self.Ample/2, self.BarraAltura),
                                AllplanGeo.Point3D(0 - 10 +0.1, self.Ample/2, self.BarraAltura),
                                AllplanGeo.Point3D(0 - 10 +0.1, self.Ample/2 +0.1, self.BarraAltura),
                                AllplanGeo.Point3D(0 - 10 , self.Ample/2 +0.1, self.BarraAltura),
                                AllplanGeo.Point3D(0 - 10, self.Ample/2, self.BarraAltura),
                                ]
            ...
            path_points =  [AllplanGeo.Point3D(0 - 10, self.Ample/2, self.BarraAltura),
                            AllplanGeo.Point3D(0 - 10, self.Ample/2, self.BarraAltura+20),
                            ]
    ...
    def create_cilinderinterior_cancam11(self):
        ...
        #----- create cylinder interior
        cylinderint = AllplanGeo.Cylinder3D(
            refPlacement=   AllplanGeo.AxisPlacement3D(AllplanGeo.Point3D(0 - 10, self.Ample/2, self.BarraAltura - self.BarraGruix  ) ),
            radiusMajor=    6.5,
            radiusMinor=    6.5,
            apex=           AllplanGeo.Point3D(0, 0, 20)
        )
