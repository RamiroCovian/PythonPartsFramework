...
def create_element(build_ele, doc):
    ...
    TDHoritzontal = PP_TD_Horitzontal(build_ele.zUnique.value, build_ele.DistanciaEntreTD.value, build_ele.BarraAmple.value, build_ele.BarraAltura.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value,
...
class PP_TD_Horitzontal():
    ...
    def __init__(self, zUnique, DistanciaEntreTD, BarraAmple, BarraAltura, BarraLlargada, BarraGruix,
                    ...
                    EncaixosPar, retallInici = 0, llargBarraAmbRetallFinal = 0):
        """ initialize
    ...
    def create_PP_positions_femelles(self, ampleTDV, alturaTDV, matrixPosX, matrixPosY, matrixMostrar, offsetX, orientacio, Ample_forat_femellaVert, barraLlargadaReal, offsetHor,FemellesAux = [], desplXI = 0):
        ...
        if TDV_count > 0:
            ...
            trans_matrix.Translate(AllplanGeo.Vector3D(matrixPosX[0], matrixPosY[0], 0))
            ...
            if len(self.Femelles) <= 1 :
                ...
                bob = FemellesCollection( Femella = False,
                                        ...
                                        #PosFemellaX = deltax + self.matrixPosXAux[_+1] - espaiCentrar,
                                        PosFemellaX = matrixPosX[0] - espaiCentrar,
                                        #PosFemellaX =  posX,
                                        PosFemellaY = offsetY,
                ...
                if len(matrixPosX) >= TDV_count and  len(self.matrixPosYAux) >= TDV_count and TDV_count != 0 and _+1 <= len(matrixPosX)-1 and len(self.Femelles) >= _:
                    ...
                    if len(self.Femelles) <= _+1 :
                        ...
                        if _+1 < len(matrixMostrar):
                            ...
                            mostrar = True
                        if _+1 < len(orientacio):
                            orientacio = orientacio[_+1]
                        else:
                            orientacio = "Esq"
                        bob = FemellesCollection( Femella = matrixMostrar[_+1],
                                                FemellaOr = orientacio,
                    ...
                    if len(self.Femelles) <= _+1 :
                        ...
                        if _+1 < len(orientacio):
                            ...
                            self.Femelles[_+1] = self.Femelles[_+1]._replace(FemellaOr = "Esq")
                        #self.Femelles[_+1] = self.Femelles[_+1]._replace(FemellaOr = orientacio[_+1])
            ...
            for _ in  range(TDV_count,len(matrixMostrar)-1):
                ...
                if len(self.Femelles) <= _+1 :
                    ...
                    if _+1 < len(orientacio):
                        ...
                        orientacio = "Esq"
                    bob = FemellesCollection( Femella = mostrar,
                                            FemellaOr = orientacio, #orientacio[_+1],
                                            #PosFemellaX = deltax + self.matrixPosXAux[_+1] - espaiCentrar,
    ...
    def create(self) :#-> AllplanElementAdapter.AllplanElement:
        '''
        ...
        #elem = AllplanElementAdapter.AllplanElement()
        #AllplanBaseElements.AttributeSet()
        #AllplanBasisElements.AttributeContainer()
        #elem.SetAttributes([AllplanBaseElements.AttributeString(508, "CAVITAT "),
        #                    AllplanBaseElements.AttributeString(2103, "CAVITAT "),
        #                    AllplanBaseElements.AttributeString(1083, "CAVITAT "),
        #                    AllplanBaseElements.AttributeString(507, "TEST")])
        #elem.SetGeometryObject(barra)
    ...
    def crear_encaix_geo(self, geometry, nEncaix):
        ...
        if posEncaix >= self.retallInici -1 and (posEncaix < self.retallInici + self.llargBarraAmbRetallFinal or self.llargBarraAmbRetallFinal == 0):
            ...
            if orientacio == "Dre":
                ...
                vector = AllplanGeo.Vector3D(posEncaix , self.BarraAltura/2 - AmplEncaix/2, 0)
    ...
    def get_foundation_common_props(self): #-> AllplanBaseElements.CommonProperties:
        ...
        #common_prop.LinkType = AllplanBasisElements.LinkType.eLinkToRoom
