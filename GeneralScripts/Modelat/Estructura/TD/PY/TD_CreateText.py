...
def create_element(build_ele, doc):
    ...
    TDTextSpline = TextSpline(build_ele.zUnique.value, build_ele.BarraAmple.value, build_ele.BarraAltura.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value,
    ...
    if not TDTextSpline.is_valid():
        return ([], [])




    handle_list = TDTextSpline.create_handles()
    views = [View2D3D ([AllplanBasisElements.ModelElement3D(common_props, TDTextSpline.create())])]

    attr_list = [AllplanBaseElements.AttributeString(2445, TDTextSpline.get_seccio()),
                AllplanBaseElements.AttributeString(220, TDTextSpline.get_llargada()),
...
class TextSpline():
    ...
    def __repr__(self):
        return 'TD_CreateText(zUnique=%s, BarraAmple=%s, BarraAltura=%s, BarraLlargada=%s, BarraGruix=%s,' \
