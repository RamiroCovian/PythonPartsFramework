...
def create_element(build_ele, doc):
    ...
    TDVertical = LProvisional(build_ele.zUnique.value, build_ele.BarraAmple.value, build_ele.BarraAltura.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value,
    ...
    attr_list = [AllplanBaseElements.AttributeString(2445, TDVertical.get_seccio()),
...
class LProvisional():
    ...
    def get_params_list(self):
        ...
        param_list.append ("esExtrem = %s\n" % self.esExtrem)
        param_list.append ("esExtremFinal = %s\n" % self.esExtremFinal)
        param_list.append ("numL = %s\n" % self.numL)

        return param_list

    def __repr__(self):
        return 'TD_LProvisional(zUnique=%s, BarraAmple=%s, BarraAltura=%s, BarraLlargada=%s, BarraGruix=%s,' \
            'IsUseGlobalProp=%s, FounColor=%s, BarraLayer=%s, esExtrem=%s, esExtremFinal=%s, numL=%s)\n' \
            % (self.zUnique, self.BarraAmple, self.BarraAltura, self.BarraLlargada, self.BarraGruix,
               self.IsUseGlobalProp, self.FounColor, self.BarraLayer, self.esExtrem, self.esExtremFinal, self.numL)
