...
def modify_control_properties(build_ele     : BuildingElement,
    ...
    if event_id == 2001:
        ...
        #print(ctrl_prop_util.build_ele_list[0].BarresHorListToShow.valuelist)
...
def create_element_class(build_ele, doc): # type: ignore
    """
    ...
    nRef = 0
    if getNumFromText(build_ele, build_ele.SelectorReforc.value, 7) != "":
        nRef = getNumFromText(build_ele, build_ele.SelectorReforc.value, 7)
    ...
    mostrarActual = ""
    if build_ele.SelectorPP.value == 1 and build_ele.SelectorTDH.value == "EN Inferior":
        mostrarActual = "ENHINF"
    elif build_ele.SelectorPP.value == 1 and build_ele.SelectorTDH.value == "EN Superior":
        mostrarActual = "ENHSUP"
    elif build_ele.SelectorPP.value == 1 and build_ele.SelectorTDH.value == "Mes Tubs...":
        mostrarActual = "ENHInt"
    elif build_ele.SelectorPP.value == 2:
        if build_ele.SelectorPPAnt.value != 2 and build_ele.SelectorPP.value == 2:
            mostrar_valors_actuals(build_ele, nVer)
        mostrarActual = "ENV"+str(nVer)
    elif build_ele.SelectorPP.value == 4:
        mostrarActual = "ENR"+str(nRef)
    elif build_ele.SelectorPP.value == 3:
        nBalcFin = 0
        lenBalcFin = len(build_ele.SelectorBalcFin.value)
        nBalcFinS = build_ele.SelectorBalcFin.value[lenBalcFin-1:]
        if len(build_ele.SelectorBalcFin.value) >= 10:
        ...
        if build_ele.UpdateBalconeraFinestra.value:
            ...
            if build_ele.VerticalAutoBalc.value:
                ...
                if getNumFromText(build_ele, build_ele.SelectorTDHorEsqBalc.value, 3) != "":
                    posBarraAntBalcFines = getNumFromText(build_ele, build_ele.SelectorTDHorEsqBalc.value, 3)
                if getNumFromText(build_ele, build_ele.SelectorTDHorDreBalc.value, 3) != "":
                    posBarraSegBalcFines = getNumFromText(build_ele, build_ele.SelectorTDHorDreBalc.value, 3)
            ...
            nHorSup=0
            if getNumFromText(build_ele, build_ele.SelectorTDHorSupBalc.value, 3) != "":
                nHorSup = getNumFromText(build_ele, build_ele.SelectorTDHorSupBalc.value, 3)
            if getNumFromText(build_ele, build_ele.SelectorTDHorInfBalc.value, 3) != "":
                nHorInf = getNumFromText(build_ele, build_ele.SelectorTDHorInfBalc.value, 3)

            if not trobatAnt:
                posBarraAntBalcFines = 0

            if not trobatSeg:
                posBarraSegBalcFines = build_ele.IntegerTDSelector.value-1
            ...
            if build_ele.SelectorTDHorInfBalc.value != "EN Inferior" and build_ele.SelectorTDHorInfBalc.value != "EN Superior":
                ...
                build_ele.BarresHorList.value[nHorInf] = build_ele.BarresHorList.value[nHorInf]._replace(Posicio = build_ele.PosicioZBalconera.value - 5 - build_ele.dadesTDHortInter.value[nHorInf].Altura/2 )
            ...
            if build_ele.SelectorTDHorSupBalc.value != "EN Superior" and build_ele.SelectorTDHorSupBalc.value != "EN Inferior":
                if build_ele.TipusPorta.value == 'Corredissa':
                    build_ele.dadesTDHortInter.value[nHorSup] = build_ele.dadesTDHortInter.value[nHorSup]._replace(Altura = 30 )
                    build_ele.dadesTDHortInter.value[nHorSup] = build_ele.dadesTDHortInter.value[nHorSup]._replace(Ample = 70 )
                ...
                build_ele.BarresHorList.value[nHorSup] = build_ele.BarresHorList.value[nHorSup]._replace(Posicio = build_ele.PosicioZBalconera.value + build_ele.LlargadaBalconera.value + 5 -build_ele.BarraAlturaInf.value)#+ build_ele.dadesTDHortInter.value[nHorSup].Altura/2)
            ...
            if build_ele.TipusPorta.value == 'Corredissa':
                ...
                if build_ele.SelectorTDHorSupBalcCorredisa.value != "EN Inferior" and build_ele.SelectorTDHorSupBalcCorredisa.value != "EN Superior":
                    build_ele.BarresHorList.value[nHorSup2] = build_ele.BarresHorList.value[nHorSup2]._replace(BarraHor = True)
                    build_ele.dadesTDHortInter.value[nHorSup2] = build_ele.dadesTDHortInter.value[nHorSup2]._replace(Altura = 30 )
                    build_ele.dadesTDHortInter.value[nHorSup2] = build_ele.dadesTDHortInter.value[nHorSup2]._replace(Ample = 70 )
                    #build_ele.BarresHorList.value[nHorSup2] = build_ele.BarresHorList.value[nHorSup2]._replace(Posicio = build_ele.PosicioZBalconera.value + build_ele.LlargadaBalconera.value + 6 + 30 + 280 )
                    build_ele.BarresHorList.value[nHorSup2] = build_ele.BarresHorList.value[nHorSup2]._replace(Posicio = build_ele.BarresHorList.value[nHorSup].Posicio + build_ele.dadesTDHortInter.value[nHorSup].Altura + 280 )
                    build_ele.BarresHorList.value[nHorSup2] = build_ele.BarresHorList.value[nHorSup2]._replace(BarraInici = "Tub "+str(posBarraAntBalcFines))
                    build_ele.BarresHorList.value[nHorSup2] = build_ele.BarresHorList.value[nHorSup2]._replace(BarraFinal = "Tub "+str(posBarraSegBalcFines))
            #else:
            ...
            if nVer != posBarraAntBalcFines:
                if posBarraAntBalcFines != 0:
                    #build_ele.listDesplVerticals.value[posBarraAntBalcFines] = build_ele.listDesplVerticals.value[posBarraAntBalcFines]._replace(desplXAbs = build_ele.PosicioXBalconera.value - 80 - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple)
                    #build_ele.listDesplVerticals.value[posBarraAntBalcFines] = build_ele.listDesplVerticals.value[posBarraAntBalcFines]._replace(desplXAbs = build_ele.PosicioXBalconera.value - 5 - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple - build_ele.dadesTDVert.value[0].BarraAmple)
                    #build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs +  build_ele.dadesTDVert.value[0].BarraAmple + (build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2)
                    build_ele.listDesplVerticals.value[posBarraAntBalcFines] = build_ele.listDesplVerticals.value[posBarraAntBalcFines]._replace(desplXAbs = build_ele.PosicioXBalconera.value - build_ele.dadesTDVert.value[0].BarraAmple - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple - 5)
                build_ele.dadesTDVert.value[posBarraAntBalcFines] = build_ele.dadesTDVert.value[posBarraAntBalcFines]._replace(MostrarTDVertical = True)
            else:
                build_ele.dadesTDVert.value[posBarraAntBalcFines] = build_ele.dadesTDVert.value[posBarraAntBalcFines]._replace(MostrarTDVertical = True)
                if posBarraAntBalcFines != 0:
                    #build_ele.desplXVertAbs.value = build_ele.PosicioXBalconera.value - 80 - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple
                    #build_ele.desplXVertAbs.value = build_ele.PosicioXBalconera.value  - 5 - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple - build_ele.dadesTDVert.value[0].BarraAmple
                    build_ele.desplXVertAbs.value =build_ele.PosicioXBalconera.value - build_ele.dadesTDVert.value[0].BarraAmple  - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple - 5
            if nVer != posBarraSegBalcFines:
                if posBarraSegBalcFines != build_ele.IntegerTDSelector.value-1:
                    #build_ele.listDesplVerticals.value[posBarraSegBalcFines] = build_ele.listDesplVerticals.value[posBarraSegBalcFines]._replace(desplXAbs = build_ele.PosicioXBalconera.value + 5 + build_ele.AmpleBalconera.value )#+ 80 )#+ build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2)
                    build_ele.listDesplVerticals.value[posBarraSegBalcFines] = build_ele.listDesplVerticals.value[posBarraSegBalcFines]._replace(desplXAbs = build_ele.PosicioXBalconera.value + 5 + build_ele.AmpleBalconera.value - build_ele.dadesTDVert.value[0].BarraAmple   )
                    #build_ele.listDesplVerticals.value[posBarraSegBalcFines] = build_ele.listDesplVerticals.value[posBarraSegBalcFines]._replace(desplXAbs = build_ele.PosicioXBalconera.value + 5 + build_ele.AmpleBalconera.value - build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple)
                build_ele.dadesTDVert.value[posBarraSegBalcFines] = build_ele.dadesTDVert.value[posBarraSegBalcFines]._replace(MostrarTDVertical = True)
            else:
                if posBarraSegBalcFines != build_ele.IntegerTDSelector.value-1:
                    build_ele.dadesTDVert.value[posBarraSegBalcFines] = build_ele.dadesTDVert.value[posBarraSegBalcFines]._replace(MostrarTDVertical = True)
                #build_ele.desplXVertAbs.value = build_ele.PosicioXBalconera.value + build_ele.AmpleBalconera.value + 80 #+ build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2
                #build_ele.desplXVertAbs.value = build_ele.PosicioXBalconera.value + build_ele.AmpleBalconera.value + 5 #+ build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2
                build_ele.desplXVertAbs.value = build_ele.PosicioXBalconera.value + 5 + build_ele.AmpleBalconera.value - build_ele.dadesTDVert.value[0].BarraAmple #+ build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2
                ...
                for i in range(0,3):
                    ...
                    if nVer != posBarraCorred1:
                        if posBarraCorred1 != 0:
                            build_ele.listDesplVerticals.value[posBarraCorred1] = build_ele.listDesplVerticals.value[posBarraCorred1]._replace(desplXAbs = build_ele.PosicioXBalconera.value - build_ele.dadesTDVert.value[0].BarraAmple  + desplIntCorr - 5 )
                        if build_ele.PosicioXBalconera.value + 5 + build_ele.AmpleBalconera.value > build_ele.listDesplVerticals.value[posBarraCorred1].desplXAbs:
                            build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(MostrarTDVertical = True)
                        else:
                            build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(MostrarTDVertical = False)
                    else:
                        if posBarraCorred1 != 0:
                            build_ele.desplXVertAbs.value =build_ele.PosicioXBalconera.value - build_ele.dadesTDVert.value[0].BarraAmple + desplIntCorr - 5
                        if build_ele.PosicioXBalconera.value + 5 + build_ele.AmpleBalconera.value > build_ele.desplXVertAbs.value:
                            build_ele.MostrarTDVertical.value = True
                            build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(MostrarTDVertical = True)
                        else:
                            build_ele.MostrarTDVertical.value = False
                            build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(MostrarTDVertical = False)
                    build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(BarraInferior = "Tub "+ str(nHorSup))
                    build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(BarraAmple = 30)
                    build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(BarraAltura = 70)
                    build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(FemellaSup = True)
                    build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(FemellaInf = True)
                    if nHorSup2 == "EN Superior":
                        build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(BarraSuperior = str(nHorSup2))
                    else:
                        build_ele.dadesTDVert.value[posBarraCorred1] = build_ele.dadesTDVert.value[posBarraCorred1]._replace(BarraSuperior = "Tub "+ str(nHorSup2))
            ...
            if len(build_ele.listDesplHoritzontalsInf.value) < 2:
                add_baraInferior(build_ele, 2)
            #inferior multiple
            build_ele.SepararTDHoritzontalInf.value = True
            #primera barra fins pos barra Anterior
            if posBarraAntBalcFines == 0:
                build_ele.listDesplHoritzontalsInf.value[0] = build_ele.listDesplHoritzontalsInf.value[0]._replace(BarraLlargadaInf = build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple)
            else:
                build_ele.listDesplHoritzontalsInf.value[0] = build_ele.listDesplHoritzontalsInf.value[0]._replace(BarraLlargadaInf = build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple + build_ele.dadesTDVert.value[0].BarraAmple)#+ build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2)
            build_ele.listDesplHoritzontalsInf.value[0] = build_ele.listDesplHoritzontalsInf.value[0]._replace(desplX = 0)
            #segona barra fins final de barra seguent
            build_ele.listDesplHoritzontalsInf.value[1] = build_ele.listDesplHoritzontalsInf.value[1]._replace(desplX = build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs )#- build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2)
            if build_ele.BarraLlargadaInf.value > (build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs - build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple):
                #build_ele.listDesplHoritzontalsInf.value[1] = build_ele.listDesplHoritzontalsInf.value[1]._replace(BarraLlargadaInf = build_ele.BarraLlargada.value - (build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple ))#- build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2))
                build_ele.listDesplHoritzontalsInf.value[1] = build_ele.listDesplHoritzontalsInf.value[1]._replace(BarraLlargadaInf = build_ele.BarraLlargada.value - build_ele.listDesplHoritzontalsInf.value[1].desplX - build_ele.dadesTDVert.value[0].BarraAmple)
            else:
                build_ele.listDesplHoritzontalsInf.value[1] = build_ele.listDesplHoritzontalsInf.value[1]._replace(BarraLlargadaInf = 100)

            #CREAR PORTA (PREMARC)
            build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(MostrarInclinat = True)
            build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Posicio = 0)

            if trobatAnt and trobatSeg:
                print("trobatANt and trobatSeg")

                if posBarraAntBalcFines == 0:
                    #build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs - build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + (build_ele.dadesTDVert.value[0].BarraAmple/2 - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple/2) - 10)
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = (build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple) - (build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple)  - 10)
                else:
                    #build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs - build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple - 10)
                    #build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs - build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + (build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple/2 - build_ele.dadesTDVert.value[posBarraSegBalcFines].BarraAmple/2) - 10)
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs - (build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple)   - 10)
                if build_ele.SelectorTDHorSupBalc.value != "EN Superior" and build_ele.SelectorTDHorSupBalc.value != "EN Inferior":
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.BarresHorList.value[nHorSup].Posicio  + build_ele.BarraAlturaInf.value + 1)
                else:
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.desplZS.value + build_ele.BarraAlturaInf.value + 1)
                if posBarraAntBalcFines == 0:
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(desplX = build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple + 5)#build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2 + 5)
                else:
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(desplX = build_ele.PosicioXBalconera.value)#build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs + build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2 + 5)
                build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(desplY = build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplYAbs)
            elif not trobatAnt and trobatSeg:
                print("not trobatAnt and trobatSeg")

                build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[posBarraSegBalcFines].desplXAbs - build_ele.listDesplVerticals.value[0].desplXAbs - build_ele.dadesTDVert.value[0].BarraAmple - 10)
                if build_ele.SelectorTDHorSupBalc.value != "EN Superior" and build_ele.SelectorTDHorSupBalc.value != "EN Inferior":
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.BarresHorList.value[nHorSup].Posicio - build_ele.dadesTDHortInter.value[nHorSup].Altura/2 - 6)
                else:
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.desplZS.value + build_ele.BarraAlturaInf.value )

                build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(desplX = build_ele.listDesplVerticals.value[0].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple + 5)#build_ele.PosicioXBalconera.value)#build_ele.listDesplVerticals.value[0].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple + 5)
                build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(desplY = build_ele.listDesplVerticals.value[0].desplYAbs)

            elif trobatAnt and not trobatSeg:
                print("trobatAnt and not trobatSeg")

                build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[build_ele.IntegerTDSelector.value-1].desplXAbs - build_ele.listDesplVerticals.value[posBarraAntBalcFines].desplXAbs - build_ele.dadesTDVert.value[posBarraAntBalcFines].BarraAmple - 10)
                if build_ele.SelectorTDHorSupBalc.value != "EN Superior" and build_ele.SelectorTDHorSupBalc.value != "EN Inferior":
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.BarresHorList.value[nHorSup].Posicio - build_ele.dadesTDHortInter.value[nHorSup].Altura/2 - 6)
                else:
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.desplZS.value + build_ele.BarraAlturaInf.value  +1 )
                ...
                build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Llargada = build_ele.listDesplVerticals.value[build_ele.IntegerTDSelector.value-1].desplXAbs - build_ele.listDesplVerticals.value[0].desplXAbs  - 10)
                if build_ele.SelectorTDHorSupBalc.value != "EN Superior" and build_ele.SelectorTDHorSupBalc.value != "EN Inferior":
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.BarresHorList.value[nHorSup].Posicio + 1 )
                else:
                    build_ele.dadesTDInclinades.value[0] = build_ele.dadesTDInclinades.value[0]._replace(Altura = build_ele.desplZS.value + build_ele.BarraAlturaInf.value  + 1)
            ...
            Nref = 0
            if getNumFromText(build_ele, build_ele.SelectorREFInfBalc.value, 7) != "":
                Nref = getNumFromText(build_ele, build_ele.SelectorREFInfBalc.value, 7)

            if build_ele.SelectorReforc.value == "Varifix " + str(Nref):
                build_ele.BarresRefListToShow.value[Nref] = build_ele.BarresRefListToShow.value[Nref]._replace(BarraRef = True)
                build_ele.BarresRefListToShow.value[Nref] = build_ele.BarresRefListToShow.value[Nref]._replace(BarraIni = "Tub " + str(posBarraAntBalcFines))
                build_ele.BarresRefListToShow.value[Nref] = build_ele.BarresRefListToShow.value[Nref]._replace(BarraFin = "Tub " + str(posBarraSegBalcFines))
                build_ele.BarresRefListToShow.value[Nref] = build_ele.BarresRefListToShow.value[Nref]._replace(Posicio = build_ele.PosicioZBalconera.value )
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(BarraRef = True)
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(BarraIni = "Tub " + str(posBarraAntBalcFines))
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(BarraFin = "Tub " + str(posBarraSegBalcFines))
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(Posicio = build_ele.PosicioZBalconera.value - 30)
            else:
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(BarraRef = True)
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(BarraIni = "Tub " + str(posBarraAntBalcFines))
                build_ele.BarresRefList.value[Nref] = build_ele.BarresRefList.value[Nref]._replace(BarraFin = "Tub " + str(posBarraSegBalcFines))
    ...
    elif build_ele.SelectorPP.value == 5:
        print("SelectorPP = 5")
    ...
    if build_ele.SelectorTDVAnt.value != nVer or (build_ele.SelectorPPAnt.value != build_ele.SelectorPP.value):
        mostrar_valors_actuals(build_ele, nVer)
    ...
    llargadaInf = build_ele.BarraLlargada.value + build_ele.ExtenderInf.value
    desplXSeparat = build_ele.desplXI.value
    desplYSeparat = 0
    if build_ele.SepararTDHoritzontalInf.value:
        #build_ele.BarraLlargada.value = build_ele.listDesplHoritzontalsInf.value[0].BarraLlargadaInf
    ...
    if build_ele.llargadaigualHor.value:
        llargadaINF1 = build_ele.BarraLlargada.value + build_ele.ExtenderInf.value
        llargadaINF1 = llargadaInf
    else:
        if not build_ele.SepararTDHoritzontalInf.value:
            llargadaINF1 = build_ele.BarraLlargadaInf.value
        else:
            llargadaINF1 = build_ele.listDesplHoritzontalsInf.value[0].BarraLlargadaInf
    ...
    if build_ele.TubInferior.value == "L":
        ...
        horitzontalPPInf = PP_EN_Horitzontal(random.random() * 3600,build_ele.DistanciaEntreTD.value, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value, llargadaINF1, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
    ...
    '''
    horitzontalPP2 = PP_TD_Horitzontal(random.random() * 3600,build_ele.DistanciaEntreTD.value, build_ele.BarraAmpleSup.value, build_ele.BarraAlturaSup.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
    ...
    llargadaSup = build_ele.BarraLlargada.value
    ...
    if build_ele.TubSuperior.value == "EXD":
        ...
        horitzontalPPSup = PP_EN_Horitzontal_Sup(random.random() * 3600, AmpleSup, build_ele.BarraAlturaSup.value, llargadaSup, build_ele.BarraGruix.value, build_ele.InvertirENHoritzontalSup.value,# , BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                    build_ele.dadesTDVert.value[0].BarraAmple , build_ele.dadesTDVert.value[build_ele.IntegerTDSelector.value-1].BarraAmple,
        ...
        if build_ele.InvertirENHoritzontalSup.value:
            ...
            if build_ele.InvertirENHoritzontalSupL.value:
                orientacioLsup = False
        #HoritzontalPPSupL = PP_EN_Horitzontal_Inf(random.random() * 3600, build_ele.BarraAmpleSup.value + build_ele.BarraGruix.value, build_ele.BarraAlturaSup.value + build_ele.BarraGruix.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value, orientacioLsup,#Randomz, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
        ...
        if build_ele.TubSuperior.value == "TUB Interior":
            ...
            llargadaSup = build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[0].BarraAmple - build_ele.dadesTDVert.value[build_ele.IntegerTDSelector.value-1].BarraAmple
        else:
        ...
        horitzontalPPSup = PP_EN_Horitzontal(random.random() * 3600,build_ele.DistanciaEntreTD.value, AmpleSup, build_ele.BarraAlturaSup.value, llargadaSup, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
        ...
        '''
        horitzontalPPSup = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.BarraAmpleSup.value, build_ele.BarraAlturaSup.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                build_ele.IsUseGlobalProp.value, build_ele.FounColor.value, build_ele.BarraLayer.value,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
        ...
        #HoritzontalPPSupL = PP_EN_Horitzontal_Inf(random.random() * 3600, build_ele.BarraAmpleSup.value + build_ele.BarraGruix.value, build_ele.BarraAlturaSup.value + build_ele.BarraGruix.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value, orientacioLsup,#Randomz, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
        HoritzontalPPSupL = PP_EN_Horitzontal_Inf(random.random() * 3600, build_ele.BarraAmpleSupL.value, 50, build_ele.BarraLlargada.value-5, build_ele.BarraGruix.value, orientacioLsup, build_ele.InvertirENHoritzontalSupLUpDown.value,#Randomz, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
        ...
        femellesSup = getFemelles(build_ele, AmpleSup)
        llargadaSup = llargadaSup - build_ele.dadesTDVert.value[0].Gruix - 1 - build_ele.dadesTDVert.value[build_ele.IntegerTDSelector.value-1].Gruix - 1
        #HoritzontalPPSupL = PP_EN_Horitzontal_Inf(random.random() * 3600, build_ele.BarraAmpleSup.value + build_ele.BarraGruix.value, build_ele.BarraAlturaSup.value + build_ele.BarraGruix.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value, orientacioLsup,#Randomz, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
    ...
    chair = PP_EN_Vertical(random.random() * 3600, build_ele.dadesTDVert.value[0].BarraAmple+10, build_ele.dadesTDVert.value[0].BarraAltura, build_ele.BarraLlargadaVert.value,build_ele.dadesTDVert.value[0].Gruix,
                            ...
                            build_ele.dadesTDVert.value[0].Ample_forat_femellaVert, build_ele.dadesTDVert.value[0].Altura_forat_femellaVert, build_ele.dadesTDVert.value[0].Separacio_forat_femellaVert,
                            Femelles, #matriu -----
                            build_ele.dadesTDVert.value[0].posicio_centre_massesVert,
                            build_ele.dadesTDVert.value[0].IsFirstCancamVert, build_ele.dadesTDVert.value[0].Dis1cancamVert,
                            build_ele.dadesTDVert.value[0].IsSecondCancamVert, build_ele.dadesTDVert.value[0].Dis2cancamVert,
                            build_ele.dadesTDVert.value[0].PestanyaSuperiorVert, build_ele.dadesTDVert.value[0].PestanyaInferiorVert,
                            Encaixos)
    ...
    for i in range(0,build_ele.IntegerTDSelector.value):
        matrixcentrar.append(build_ele.dadesTDVert.value[i].BarraAmple/2)
        matrixPosX.append(build_ele.listDesplVerticals.value[i].desplXAbs )
        matrixPosY.append(build_ele.listDesplVerticals.value[i].desplY)
        matrixMostrar.append(build_ele.dadesTDVert.value[i].MostrarTDVertical)
        matrixMostrarInf.append(((build_ele.dadesTDVert.value[i].MostrarTDVertical and build_ele.dadesTDVert.value[i].PestanyaInferiorVert and build_ele.dadesTDVert.value[i].FemellaInf) or (build_ele.dadesTDVert.value[i].MostrarTDVertical and build_ele.EncaixHorInf.value and not build_ele.dadesTDVert.value[i].EncaixInf) or (build_ele.dadesTDVert.value[i].MostrarTDVertical and build_ele.dadesTDVert.value[i].FemellaInf))and build_ele.dadesTDVert.value[i].BarraInferior == "EN Inferior")
        matrixMostrarSup.append(((build_ele.dadesTDVert.value[i].MostrarTDVertical and build_ele.dadesTDVert.value[i].PestanyaSuperiorVert and build_ele.dadesTDVert.value[i].FemellaSup) or (build_ele.dadesTDVert.value[i].MostrarTDVertical and build_ele.EncaixHorSup.value and not build_ele.dadesTDVert.value[i].EncaixSup) or (build_ele.dadesTDVert.value[i].MostrarTDVertical and build_ele.dadesTDVert.value[i].FemellaSup))and build_ele.dadesTDVert.value[i].BarraSuperior == "EN Superior")
    listAmples = []
    listAmples2 = []
    listAltures = []
    for TDVert in build_ele.dadesTDVert.value:
    ...
    #for i in range(0,len(build_ele.listDesplVerticals.value)):
    if not build_ele.ReduirTempsCarrega.value and build_ele.TubSuperior.value != "L":
        ...
        if (build_ele.EncaixHorInf.value and build_ele.desplYI.value > 0.0):
            ...
            if build_ele.invertirEncaixInf.value:
                ...
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    #matrixPosX.append(build_ele.listDesplVerticals.value[i].desplXAbs)
                    if build_ele.dadesTDVert.value[i].EncaixInf:
                        matrixOr.append("Esq")
                    else:
                        matrixOr.append("Inf")
                trans_list = horitzontalPPInf.create_PP_positions_femelles(listAmples, listAltures, matrixPosX, matrixPosY, matrixMostrarInf, offsetXI, matrixOr, build_ele.Ample_forat_femellaVert.value, build_ele.BarraLlargada.value, build_ele.desplYI.value,[], build_ele.desplXI.value)
            else:
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    matrixOffsetY.append(  build_ele.listDesplVerticals.value[i].desplY + build_ele.dadesTDVert.value[i].BarraAltura - build_ele.desplYI.value)
                trans_list = horitzontalPPInf.create_PP_positions_encaix(listAmples2, matrixPosX, matrixPosY, matrixcentrar, matrixMostrar, matrixOffsetY, offsetXI, 'Sup', build_ele.BarraLlargada.value)
            ...
            if build_ele.invertirEncaixInf.value:
                ...
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    matrixOffsetY.append( build_ele.BarraAmpleInf.value - build_ele.listDesplVerticals.value[i].desplY + build_ele.desplYI.value)#build_ele.dadesTDVert.value[i].BarraAltura

                trans_list = horitzontalPPInf.create_PP_positions_encaix(listAmples2, matrixPosX, matrixPosY, matrixcentrar,  matrixMostrar, matrixOffsetY, offsetXI, 'Inf', build_ele.BarraLlargada.value )
                horitzontalPPInf.set_false_femelles()
        else:
            horitzontalPPInf.set_false_encaix()
        ...
        if build_ele.EncaixHorSup.value and build_ele.desplYS.value > 0.0 and (build_ele.TubSuperior.value == "TUB" or build_ele.TubSuperior.value == "TUB Interior"):
            if build_ele.invertirEncaixSup.value:
                ...
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    #matrixPosX.append(build_ele.listDesplVerticals.value[i].desplXAbs)
                    if build_ele.dadesTDVert.value[i].EncaixSup:
                        matrixOr.append("Dre")
                    else:
                        matrixOr.append("Inf")
                trans_listSup = horitzontalPPSup.create_PP_positions_femelles(listAmples, listAltures, matrixPosX, matrixPosY, matrixMostrarSup, offsetXI, matrixOr, build_ele.Ample_forat_femellaVert.value, build_ele.BarraLlargada.value,build_ele.desplYS.value,[], build_ele.desplXI.value)
            else:
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    matrixOffsetY.append(  build_ele.listDesplVerticals.value[i].desplY + build_ele.dadesTDVert.value[i].BarraAltura - build_ele.desplYS.value)#build_ele.dadesTDVert.value[i].BarraAltura
                trans_listSup = horitzontalPPSup.create_PP_positions_encaix(listAmples2, matrixPosX, matrixPosY, matrixcentrar, matrixMostrar, matrixOffsetY, offsetXS, 'Sup', build_ele.BarraLlargada.value)
            ...
            if build_ele.invertirEncaixSup.value:
                ...
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    matrixPosX.append(build_ele.listDesplVerticals.value[i].desplXAbs)
                    #matrixPosX.append(build_ele.desplXS.value + build_ele.BarraAmpleSup.value - build_ele.listDesplVerticals.value[i].desplX + build_ele.dadesTDVert.value[i].BarraAmple)
                    if build_ele.dadesTDVert.value[i].EncaixSup:
                        matrixOr.append("Dre")
                    else:
                        matrixOr.append("Sup")
                ...
                for i in range(0,len(build_ele.listDesplVerticals.value)):
                    #matrixOffsetY.append(build_ele.BarraAmpleSup.value + build_ele.desplYS.value + build_ele.listDesplVerticals.value[i].desplY )
                    matrixOffsetY.append(AmpleSup + build_ele.desplYS.value + build_ele.listDesplVerticals.value[i].desplY )
                trans_listSup = horitzontalPPSup.create_PP_positions_encaix(listAmples2, matrixPosX, matrixPosY, matrixcentrar,  matrixMostrar, matrixOffsetY, offsetXS, 'Inf', build_ele.BarraLlargada.value)
                horitzontalPPSup.set_false_femelles()
        else:
            horitzontalPPSup.set_false_encaix()
            for i in range(0,len(build_ele.listDesplVerticals.value)):
                matrixOr.append("Dre")
            trans_listSup = horitzontalPPSup.create_PP_positions_femelles(listAmples, listAltures, matrixPosX, matrixPosY, matrixMostrarSup, offsetXS, matrixOr, build_ele.Ample_forat_femellaVert.value,build_ele.BarraLlargada.value, build_ele.desplYS.value,[], build_ele.desplXI.value)
    ...
    if not build_ele.InvertirENHoritzontalInf.value:
        vectorH1.SetValue(13, build_ele.desplYI.value - (build_ele.BarraAmpleInf.value - build_ele.dadesTDVert.value[0].BarraAltura))
    ...
    if not build_ele.SepararTDHoritzontalInf.value:
        llargadaInfCavitat = build_ele.BarraLlargada.value
    else:
        llargadaInfCavitat = build_ele.listDesplHoritzontalsInf.value[0].BarraLlargadaInf
    horitzontalPPOBject = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value+10, llargadaInfCavitat, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                ...
                                build_ele.PestanyaSuperiorINF.value, build_ele.PestanyaInferiorINF.value,#PestanyaSuperior, PestanyaInferior,
                                [])
    horitzontalPPOBjectRecess = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value+10, llargadaInfCavitat, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
    ...
    if build_ele.MostrarTDHoritzontalInf.value :#and not build_ele.SepararTDHoritzontalInf.value:
        ...
        """
            ...
            #vectorH1.SetValue(12, build_ele.listDesplHoritzontalsInf.value[0].BarraAmpleInf)
            #vectorH1.SetValue(13, build_ele.listDesplHoritzontalsInf.value[0].BarraAlturaInf)
            vectorH1.SetValue(14, 0)
        """
        """
        for i in range(0, len(build_ele.ColisParINF.value)):

            if build_ele.ColisParINF.value[i].Colis and build_ele.ColisParINF.value[i].MostrarBox and build_ele.ColisParINF.value[i].Posicio < build_ele.BarraLlargada.value:
        ...
        if build_ele.mostrarLiniaIntInf1.value:
            if len(build_ele.listDesplHoritzontalsInf.value)<=0:
                add_baraInferior(build_ele, 1)
            punt_central = desplXSeparat + build_ele.desplXI.value + build_ele.BarraAlturaInf.value/2
            pointUbi = AllplanGeo.Point3D(desplXSeparat, vectorH1[13] + build_ele.BarraAmpleInf.value/2 , vectorH1[14] +  build_ele.BarraAlturaInf.value/2)
            if build_ele.SepararTDHoritzontalInf.value:
                ...
                pointUbi = AllplanGeo.Point3D(desplXSeparat, vectorH1[13] + build_ele.listDesplHoritzontalsInf.value[0].BarraAmpleInf/2 + build_ele.listDesplHoritzontalsInf.value[0].desplLinA, vectorH1[14] +  build_ele.BarraAlturaInf.value/2)
            ...
            polyhedronCentral = create_polyline_interior(build_ele, punt_central,build_ele.listDesplHoritzontalsInf.value[0].BarraAmpleInf,llargadaINF1, True, pointUbi, int(linia), build_ele.SelectorLayerInf.value)
        ...
        if build_ele.mostrarLiniaIntInf2.value:
            ...
            if build_ele.SepararTDHoritzontalInf.value:
                punt_central = desplXSeparat + build_ele.listDesplHoritzontalsInf.value[0].desplX + build_ele.listDesplHoritzontalsInf.value[0].BarraAlturaInf/2
                pointUbi = AllplanGeo.Point3D(desplXSeparat, vectorH1[13] + build_ele.listDesplHoritzontalsInf.value[0].BarraAmpleInf/2 + build_ele.listDesplHoritzontalsInf.value[0].desplLinB, vectorH1[14] +  build_ele.BarraAlturaInf.value/2)
            if (len(build_ele.SelectorLiniaInf.value)) <=1:
                build_ele.SelectorLiniaInf.value = "Tipus 1"
            linia = build_ele.SelectorLiniaInf.value[len(build_ele.SelectorLiniaInf.value)-1]
            polyhedronCentral = create_polyline_interior(build_ele, punt_central,build_ele.listDesplHoritzontalsInf.value[0].BarraAmpleInf,llargadaINF1, True, pointUbi, int(linia), build_ele.SelectorLayerInf.value)
    ...
    if build_ele.SepararTDHoritzontalInf.value:
        for nBarresInf in range(1, build_ele.nBarresTDHoritzontalInf.value):
            elements , lHoritzontalPP, elements_model_eleList = crear_barres_inferiors_multiples(build_ele, nBarresInf, mostrarActual, doc)
    ...
    if build_ele.TubSuperior.value == "L":
        vectorH2.SetValue(12, build_ele.desplXS.value + build_ele.dadesTDVert.value[0].Gruix + 1)
    ...
    if build_ele.TubSuperior.value == "TUB Interior":
        vectorH2.SetValue(12, vectorH2[12] + build_ele.dadesTDVert.value[0].BarraAmple)
        vectorH2C.SetValue(12, vectorH2[12])
    ...
    if build_ele.MostrarTDHoritzontalSup.value:
        ...
        if  build_ele.IsFirstCancamSUP.value:
            if build_ele.posicio_centre_massesSUP.value  + build_ele.Dis1cancamSUP.value/ 2 < build_ele.BarraLlargada.value and build_ele.posicio_centre_massesSUP.value  + build_ele.Dis2cancamSUP.value/ 2 < build_ele.BarraLlargada.value:
        ...
        for i in range(0, len(build_ele.ColisParSUP.value)):
            ...
            if build_ele.ColisParSUP.value[i].Colis and build_ele.ColisParSUP.value[i].MostrarBox and build_ele.ColisParSUP.value[i].Posicio < build_ele.BarraLlargada.value:
        ...
        if build_ele.mostrarLiniaIntSup1.value:
            ...
            llargadaEixSup = build_ele.BarraLlargada.value
            try:
                llargadaEixSup = float(horitzontalPPSup.get_llargada())
            except Exception as e :
                print("llargada superior incorrecta 1")
            polyhedronCentral = create_polyline_interior(build_ele, punt_central, AmpleSup, llargadaEixSup, True, pointUbi, int(linia), build_ele.SelectorLayerSup.value)
            if polyhedronCentral != []:
                group_elems.append(polyhedronCentral[0])
                group_elems_preview.append(polyhedronCentral[0])
                group_elems.append(polyhedronCentral[3])
    ...
    '''
    if build_ele.IntegerTDSelector.value < 3 :
        ...
        #verticalPPOBject = PP_TD_Vertical(build_ele.dadesTDVert.value[0].BarraAmple+10, build_ele.dadesTDVert.value[0].BarraAltura, build_ele.BarraLlargadaVert.value,build_ele.dadesTDVert.value[0].Gruix,
        verticalPPOBject = PP_EN_Vertical(random.random() * 3600, build_ele.BarraAmpleVert.value+10, build_ele.BarraAlturaVert.value, build_ele.BarraLlargadaVert.value, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                False, common_propsObject.Color,  common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                [], [], [], #ColisPar, PotaPar, #matrius
                                0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                [], #Femelles, #matriu
        ...
        natrix1.SetValue(12, build_ele.BarraLlargada.value - build_ele.BarraAmpleInf.value)
        group_elems.append(PythonPart ("PP_EN_Vertical", parameter_list = chair.get_params_list(),
                            hash_value = chair.hash(), python_file = chair.filename(),
        ...
        #verticalPPOBject = PP_TD_Vertical(build_ele.dadesTDVert.value[0].BarraAmple+10, build_ele.dadesTDVert.value[0].BarraAltura, build_ele.BarraLlargadaVert.value,build_ele.dadesTDVert.value[0].Gruix,
        verticalPPOBject2 = PP_EN_Vertical(random.random() * 3600, build_ele.BarraAmpleVert.value+10, build_ele.BarraAlturaVert.value, build_ele.BarraLlargadaVert.value, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                False, common_propsObject.Color,  common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                [], [], [], #ColisPar, PotaPar, #matrius
                                0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                [], #Femelles, #matriu
    ...
    if not build_ele.ReduirTempsCarrega.value:
        try:
            ...
            while build_ele.IntegerTDSelector.value > len(trans_list):
                ...
                trans_matrix.Translate(AllplanGeo.Vector3D(build_ele.listDesplVerticals.value[len(trans_list)-1].desplXAbs, build_ele.listDesplVerticals.value[len(trans_list)-1].desplYAbs, 0))
            ...
            for nVertMat in range(0,build_ele.IntegerTDSelector.value):
                matrix = AllplanGeo.Matrix3D()
                matrix.SetValue(12, build_ele.listDesplVerticals.value[nVertMat].desplXAbs )#tipus desplaçament desplXAbs#+ build_ele.dadesTDVert.value[j].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2)
                matrix.SetValue(13, build_ele.listDesplVerticals.value[nVertMat].desplYAbs)
                matrix.SetValue(14, 0)
                i = j-1

                if j >= len(build_ele.dadesTDVert.value):
                    set_values(build_ele, j)
                lennVer = len(build_ele.SelectorTDV.value)

                '''
                if build_ele.dadesTDVert.value[j].esProvisional:
                    build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(EncaixSup= True)
                    build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(EncaixInf= True)
                    build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(PestanyaSuperiorVert = False)
                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                        build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(PestanyaInferiorVert = True)
                    else:
                        build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(PestanyaInferiorVert = False)
                    if j == nVer:
                ...
                for nForat in range(0,len(Forats)):
                    if Forats[nForat].Forat and Forats[nForat].MostrarBox and build_ele.dadesTDVert.value[j].MostrarTDVertical and build_ele.listDesplVerticals.value[j].desplXAbs <= build_ele.BarraLlargada.value + build_ele.ExtenderInf.value:
                ...
                ampleSeg = 0
                if j > 0:
                    ampleAnt = build_ele.dadesTDVert.value[j-1].BarraAmple
                    ampleSeg = build_ele.dadesTDVert.value[j].BarraAmple
                ...
                #build_ele.listDesplVerticals.value[j] =  build_ele.listDesplVerticals.value[j]._replace(desplXAbs = matrix[12] + build_ele.listDesplVerticals.value[j].desplX )
                #build_ele.listDesplVerticals.value[j] =  build_ele.listDesplVerticals.value[j]._replace(desplYAbs = matrix[13] )
                ...
                for nValueEdit in range(0,len(build_ele.BarresHorList.value)):
                    if build_ele.BarresHorList.value[nValueEdit].acabatEditar and not  build_ele.BarresHorList.value[nValueEdit].Edit:
                        build_ele.desplXVertAbs.value = build_ele.listDesplVerticals.value[j].desplXAbs
                        build_ele.desplYVertAbs.value = build_ele.listDesplVerticals.value[j].desplYAbs
                ...
                if not mostrarInterior:
                    if build_ele.DistanciaEntreTDAnt.value != build_ele.DistanciaEntreTD.value :
                        #build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplXAbs = matrix[12])#no modificar posicio al canviar la distancia
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplXAbs = build_ele.DistanciaEntreTD.value * j )
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplYAbs = matrix[13])
                        if j == nVer and build_ele.SelectorPP.value == 2:
                            build_ele.desplXVertAbs.value = matrix[12]
                            build_ele.desplYVertAbs.value = matrix[13]
                    #elif build_ele.BarraLlargadaAnt.value != build_ele.BarraLlargada.value and j != build_ele.len(trans_list)-1 and build_ele.listDesplVerticals.value[j].desplXAbs == 0 and j != 0 :
                    elif build_ele.BarraLlargadaAnt.value != build_ele.BarraLlargada.value and j != build_ele.IntegerTDSelector.value  and build_ele.listDesplVerticals.value[j].desplXAbs == 0 and j != 0 :
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplXAbs = matrix[12])
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplX = 0)
                        ...
                        if j == nVer and build_ele.SelectorPP.value == 2:
                            ...
                            if nVer != 0 and build_ele.desplXVertAbs.value == 0:
                                ...
                                build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplXAbs = build_ele.desplXVertAbs.value )
                                build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplYAbs = build_ele.desplYVertAbs.value)
                        else:
                            if j != 0 and build_ele.listDesplVerticals.value[j].desplXAbs == 0:
                                build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplXAbs = build_ele.DistanciaEntreTD.value * j )
                                build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplYAbs = 0)
                                build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplX = build_ele.listDesplVerticals.value[j].desplXAbs - matrix[12])
                                build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplY = 0)
                    ...
                    if j == nVer and build_ele.SelectorPP.value == 2:
                        ...
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplX = build_ele.desplXVertAbs.value - matrix[12])
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplY = build_ele.desplYVertAbs.value - matrix[13])
                    else:
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplX = build_ele.listDesplVerticals.value[j].desplXAbs - matrix[12])
                        build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplY = build_ele.listDesplVerticals.value[j].desplYAbs - matrix[13])
                ...
                if j == build_ele.IntegerTDSelector.value-1:
                    #print("posicio Final: " + str(build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[j].BarraAmple - build_ele.dadesTDVert.value[0].BarraAmple))
                    build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplXAbs = build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[j].BarraAmple - build_ele.dadesTDVert.value[0].BarraAmple)#- build_ele.dadesTDVert.value[j].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2)
                    build_ele.listDesplVerticals.value[j] = build_ele.listDesplVerticals.value[j]._replace(desplX = 0)

                        #build_ele.listDesplVerticals.value[0] = build_ele.listDesplVerticals.value[0]._replace(desplX = build_ele.listDesplVerticals.value[0].desplXAbs)
                ...
                #matrix0.SetValue(12, matrix[12] + build_ele.listDesplVerticals.value[j].desplX)
                matrix0.SetValue(12, build_ele.listDesplVerticals.value[j].desplXAbs + (build_ele.dadesTDVert.value[j].BarraAmple+build_ele.dadesTDVert.value[0].BarraAmple)/2 )
                matrix0.SetValue(12, build_ele.listDesplVerticals.value[j].desplXAbs +build_ele.dadesTDVert.value[0].BarraAmple )
                if j== 0 :
                    matrix0.SetValue(12, build_ele.listDesplVerticals.value[j].desplXAbs - (build_ele.dadesTDVert.value[j].BarraAmple-build_ele.dadesTDVert.value[0].BarraAmple)/2 )
                    matrix0.SetValue(13, matrix[13] + build_ele.listDesplVerticals.value[j].desplYAbs )
                else:
                    #matrix0.SetValue(13, matrix[13] )
                    matrix0.SetValue(13, build_ele.listDesplVerticals.value[j].desplYAbs )
                    matrix.SetValue(13,build_ele.listDesplVerticals.value[j].desplYAbs)
                ...
                alturaInf = 0
                if build_ele.dadesTDVert.value[j].BarraInferior != "EN Inferior":
                    nHorInf = build_ele.dadesTDVert.value[j].BarraInferior
                    if getNumFromText(build_ele, build_ele.dadesTDVert.value[j].BarraInferior, 3) != "":
                        nHorInf = getNumFromText(build_ele, build_ele.dadesTDVert.value[j].BarraInferior, 3)

                        if build_ele.dadesTDVert.value[j].EncaixInf:
                            matrix0.SetValue(14, build_ele.BarresHorList.value[nHorInf].Posicio + build_ele.BarraAlturaInf.value +build_ele.dadesTDHortInter.value[nHorInf].Altura )
                            alturaInf = 0
                        else:
                            matrix0.SetValue(14, build_ele.BarresHorList.value[nHorInf].Posicio + build_ele.BarraAlturaInf.value)
                            alturaInf = build_ele.dadesTDHortInter.value[nHorInf].Altura
                else:
                    #matrix0.SetValue(14, build_ele.BarraAlturaInf.value )
                    try:
                        matrix0.SetValue(14, build_ele.BarraAlturaInf.value + build_ele.listDesplVerticals.value[j].desplZAbs)
                    except Exception as e:
                        matrix0.SetValue(14, build_ele.BarraAlturaInf.value)
                        print(" ----------------- NO existeix desplZAbs en listDesplVerticals")
                    alturaInf = 0
                ...
                for nBarraInfEnc in range(0,build_ele.nBarresTDHoritzontalInf.value):
                    ...
                    if not trobatInf and build_ele.SepararTDHoritzontalInf.value and build_ele.listDesplVerticals.value[j].desplXAbs >= build_ele.listDesplHoritzontalsInf.value[nBarraInfEnc].desplX and build_ele.listDesplVerticals.value[j].desplXAbs <=  build_ele.listDesplHoritzontalsInf.value[nBarraInfEnc].desplX + build_ele.listDesplHoritzontalsInf.value[nBarraInfEnc].BarraLlargadaInf:
                        #encaixosAux.append(bob)
                        trobatInf = True
                #if build_ele.desplYI.value != 0.0 or build_ele.EncaixHorInf.value:#or (build_ele.dadesTDVert.value[j].BarraAltura.value > build_ele.BarraAmpleInf.value):
                '''
                if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                    if build_ele.EncaixHorInf.value and not build_ele.dadesTDVert.value[j].EncaixInf :#or (build_ele.dadesTDVert.value[j].BarraAltura.value > build_ele.BarraAmpleInf.value):
                ...
                #if build_ele.desplYS.value != 0.0 :
                '''
                if build_ele.EncaixHorSup.value and not build_ele.dadesTDVert.value[j].EncaixSup :
                    ...
                    #build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(PestanyaSuperiorVert = True)
                '''
                if not build_ele.dadesTDVert.value[j].LlargadaAut and j == nVer:
                    BarrallargadaVert = build_ele.BarraLlargadaIndiv.value
                elif not build_ele.dadesTDVert.value[j].LlargadaAut :
                    BarrallargadaVert = build_ele.dadesTDVert.value[j].BarraAlcada
                else:
                ...
                nHorInf = 0
                nHorSup = 0
                #if not build_ele.dadesTDVert.value[j].EncaixInf or not build_ele.dadesTDVert.value[j].EncaixSup :
                #if build_ele.dadesTDVert.value[j].BarraInferior != "EN Inferior" or build_ele.dadesTDVert.value[j].BarraSuperior != "EN Superior":
                alturaInf = 0
                ...
                if build_ele.dadesTDVert.value[j].BarraInferior != "EN Inferior":
                    if getNumFromText(build_ele, build_ele.dadesTDVert.value[j].BarraInferior, 3) != "":
                        ...
                        #posHorInf = build_ele.dadesTDHortInter.value[nHorInf].Altura
                        if not build_ele.dadesTDVert.value[j].EncaixInf:
                            alturaInf = build_ele.dadesTDHortInter.value[nHorInf].Altura
                            posHorInf = build_ele.BarresHorList.value[nHorInf].Posicio + build_ele.BarraAlturaInf.value#+ build_ele.dadesTDHortInter.value[nHorInf].Altura
                        else:
                            alturaInf = -build_ele.dadesTDHortInter.value[nHorInf].Altura
                            posHorInf = build_ele.BarresHorList.value[nHorInf].Posicio + build_ele.dadesTDHortInter.value[nHorInf].Altura + build_ele.BarraAlturaInf.value

                if build_ele.dadesTDVert.value[j].LlargadaAut:# build_ele.dadesTDVert.value[j].BarraSuperior == "EN Superior":
                    if not build_ele.dadesTDVert.value[j].EncaixSup:
                    ...
                    posHorSup = build_ele.dadesTDVert.value[j].BarraAlcada + build_ele.BarraAlturaInf.value
                    #alturaInf += build_ele.BarraAlturaInf.value

                if build_ele.dadesTDVert.value[j].BarraSuperior != "EN Superior":
                    #alturaInf -= build_ele.BarraAlturaInf.value
                    nHorSup = 0
                    if getNumFromText(build_ele, build_ele.dadesTDVert.value[j].BarraSuperior, 3) != "":
                        nHorSup = getNumFromText(build_ele, build_ele.dadesTDVert.value[j].BarraSuperior, 3)
                        if not build_ele.dadesTDVert.value[j].EncaixSup:
                            posHorSup = build_ele.BarresHorList.value[nHorSup].Posicio + build_ele.dadesTDHortInter.value[nHorSup].Altura + build_ele.BarraAlturaInf.value#+ alturaInf
                        else:
                            posHorSup = build_ele.BarresHorList.value[nHorSup].Posicio + build_ele.BarraAlturaInf.value#+ alturaInf#- build_ele.dadesTDHortInter.value[nHorSup].Altura/2

                #BarrallargadaVert += posHorSup - posHorInf
                if build_ele.dadesTDVert.value[j].LlargadaAut:
                    ...
                    build_ele.dadesTDVert.value[j].BarraAlcada
                if (j == 0 or j == build_ele.IntegerTDSelector.value-1) and build_ele.dadesTDVert.value[j].BarraSuperior == "EN Superior" and build_ele.dadesTDVert.value[j].LlargadaAut and build_ele.TubSuperior.value == "TUB Interior":
                    if  build_ele.dadesTDVert.value[j].EncaixSup:
                        BarrallargadaVert = BarrallargadaVert + build_ele.BarraAlturaSup.value  #+ build_ele.BarraAlturaInf.value

                vermellVert =  build_ele.dadesTDVert.value[j].FounColor
                if build_ele.dadesTDVert.value[j].vermell:
                    vermellVert = 6

                if build_ele.dadesTDVert.value[j].LlargadaAut and j == nVer:
                    build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(BarraAlcada = BarrallargadaVert)

                #-------------------- crear TD Interior cada vegada -----------------------
                TDV_Int = PP_EN_Vertical(random.random() * 3600, build_ele.dadesTDVert.value[j].BarraAmple, build_ele.dadesTDVert.value[j].BarraAltura, BarrallargadaVert ,build_ele.dadesTDVert.value[j].Gruix,
                                            build_ele.IsUseGlobalPropVert.value, vermellVert, build_ele.BarraLayerVert.value,
                                            Colis, Potes, Forats, #matrius -------------------------------
                                            build_ele.dadesTDVert.value[j].Ample_forat_femellaVert, build_ele.dadesTDVert.value[j].Altura_forat_femellaVert, build_ele.dadesTDVert.value[j].Separacio_forat_femellaVert,
                                            Femelles, #matriu Femelles -------------------------------------
                                            build_ele.dadesTDVert.value[j].posicio_centre_massesVert,
                                            build_ele.dadesTDVert.value[j].IsFirstCancamVert, build_ele.dadesTDVert.value[j].Dis1cancamVert,
                                            build_ele.dadesTDVert.value[j].IsSecondCancamVert, build_ele.dadesTDVert.value[j].Dis2cancamVert,
                                            build_ele.dadesTDVert.value[j].PestanyaSuperiorVert, build_ele.dadesTDVert.value[j].PestanyaInferiorVert,
                                            Encaixos,
                                            [],[],False, [],
                                            build_ele.dadesTDVert.value[j].PestanyesInv)#build_ele.pestanyesInvVert.value)#Encaixos

                verticalPPOBject5 = PP_EN_Vertical(random.random() * 3600,build_ele.dadesTDVert.value[j].BarraAmple + 6 , build_ele.dadesTDVert.value[j].BarraAltura + 2, BarrallargadaVert,0,
                                        False,  common_propsObject.Color,  common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                        [], [],[], #ColisPar, PotaPar, #matrius
                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                        [], #Femelles, #matriu
                                        ...
                                        False, False,
                                        [])
                verticalPPOBject5Recess = PP_EN_Vertical(random.random() * 3600,build_ele.dadesTDVert.value[j].BarraAmple + 6 , build_ele.dadesTDVert.value[j].BarraAltura, BarrallargadaVert,0,
                                        False,  common_propsObjectRecess.Color,  common_propsObjectRecess.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                        [], [],[], #ColisPar, PotaPar, #matrius
                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                        [], #Femelles, #matriu
                ...
                if  j < len(build_ele.nListBarresHor.value):
                    ...
                    for nbarraHorAnterior in range(0,len(build_ele.BarresHorList.value)-1):
                        ...
                        if len(build_ele.BarresHorList.value[nbarraHorAnterior].BarraFinal) == 5:
                            ...
                            NBarraFinal = build_ele.BarresHorList.value[nbarraHorAnterior].BarraFinal[-2]
                            NBarraFinal = build_ele.BarresHorList.value[nbarraHorAnterior].BarraFinal[-1] + NBarraFinal
                        else:
                            NBarraFinal = build_ele.BarresHorList.value[nbarraHorAnterior].BarraFinal[-3]
                            NBarraFinal = build_ele.BarresHorList.value[nbarraHorAnterior].BarraFinal[-2] + NBarraFinal
                            NBarraFinal = build_ele.BarresHorList.value[nbarraHorAnterior].BarraFinal[-1] + NBarraFinal
                        ...
                        if len(build_ele.BarresHorList.value[nbarraHorAnterior].BarraInici) == 5:
                            ...
                            NBarraInici = build_ele.BarresHorList.value[nbarraHorAnterior].BarraInici[-2]
                            NBarraInici = build_ele.BarresHorList.value[nbarraHorAnterior].BarraInici[-1] + NBarraInici
                        else:
                            NBarraInici = build_ele.BarresHorList.value[nbarraHorAnterior].BarraInici[-3]
                            NBarraInici = build_ele.BarresHorList.value[nbarraHorAnterior].BarraInici[-2] + NBarraInici
                            NBarraInici = build_ele.BarresHorList.value[nbarraHorAnterior].BarraInici[-1] + NBarraInici
                        ...
                        if NBarraFinal == j:
                            ...
                            ampleForatFem = build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample/2
                            ampleForatFemInv = build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2
                            tubGirat = build_ele.dadesTDHortInter.value[nbarraHorAnterior].pestanyesInv
                            if ampleForatFem > 15:
                                ampleForatFem = 15
                            ...
                            alturaInfEncaix = build_ele.dadesTDHortInter.value[nHorInf].Altura
                            plusaltura = (30 - build_ele.dadesTDHortInter.value[nHorInf].Altura)
                            if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                alturaInfFem = -4
                                alturaInfEncaix = 0#build_ele.BarraAlturaInf.value
                                plusaltura = 0

                            desplZVertical = build_ele.listDesplVerticals.value[j].desplZAbs

                            if  build_ele.dadesTDVert.value[NBarraFinal].EncaixInf:#not build_ele.EncaixHorInf.value or
                                #listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].PestanyaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarraAlturaInf.value -1 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem])
                                if tubGirat:
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura - 15/2  , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura - 15/2  , getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                    else:
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio  - build_ele.BarresHorList.value[nHorInf].Posicio -  (ampleForatFemInv )/2 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio  - build_ele.BarresHorList.value[nHorInf].Posicio -  (ampleForatFemInv )/2 , getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                else:
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor ,  build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor ,  build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2 , getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])
                                    else:
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  - build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  - build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])
                                ...
                                if tubGirat:
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura - (ampleForatFemInv )/2 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura - (ampleForatFemInv )/2 , getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample])
                                    else:
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (ampleForatFemInv )/2 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (ampleForatFemInv )/2 , getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample])
                                    ...
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        ...
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf)  + alturaInfEncaix - build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])
                                        listFemellesAnt.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaSup and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf)  + alturaInfEncaix - build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])


                            if tubGirat:
                                offsetYAnt.append(build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs )
                                offsetYAnt.append(build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs )
                            else:
                                offsetYAnt.append(build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample/2 -  ampleForatFem  + build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs + ampleForatFem/2 )
                                offsetYAnt.append(build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample/2 -  ampleForatFem  + build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs + ampleForatFem/2 )
                        ...
                        if NBarraInici == j:
                            ...
                            alturaInfFem = build_ele.BarraAlturaInf.value/2
                            alturaInfEncaix = build_ele.dadesTDHortInter.value[nHorInf].Altura + build_ele.BarraAlturaInf.value
                            if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                alturaInfFem = -4
                                alturaInfEncaix = 0#build_ele.BarraAlturaInf.value
                            ...
                            if  build_ele.dadesTDVert.value[NBarraInici].EncaixInf:#not build_ele.EncaixHorInf.value or
                                #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].PestanyaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarraAlturaInf.value -1 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFem ])

                                if tubGirat:
                                    #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf) - alturaInfFem  -4 - ampleForatFemInv - ampleForatFemInv/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample, ampleForatFemInv, tubGirat ])
                                    #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf) - alturaInfFem  -4 - ampleForatFemInv - ampleForatFemInv/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample, ampleForatFemInv, tubGirat ])
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarraAlturaInf.value + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (ampleForatFemInv - (4.25*2))/2 - 0.25, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample, ampleForatFemInv, tubGirat ])
                                        #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarraAlturaInf.value + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (ampleForatFemInv - (4.25*2))/2 - 0.25, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample, ampleForatFemInv, tubGirat ])
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (15 )/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (15 )/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                    else:
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  -  (ampleForatFemInv )/2 , build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ])
                                    ...
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        ...
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  - build_ele.dadesTDHortInter.value[nHorInf].Altura/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ])
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - build_ele.BarresHorList.value[nHorInf].Posicio  - build_ele.dadesTDHortInter.value[nHorInf].Altura/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura, ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ])

                            else:
                                if tubGirat:
                                    #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf)+1.5 - ampleForatFemInv/2 + alturaInfEncaix, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ,ampleForatFemInv, tubGirat])
                                    #listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf)+1.5 - ampleForatFemInv/2 + alturaInfEncaix, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample ,ampleForatFemInv, tubGirat])
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor ,build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (ampleForatFemInv )/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample])
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor ,build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - desplZVertical + build_ele.dadesTDHortInter.value[nHorInf].Altura -  (ampleForatFemInv )/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFemInv, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample])
                                    else:
                                    ...
                                    if build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
                                        ...
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf)  + alturaInfEncaix - build_ele.dadesTDHortInter.value[nHorInf].Altura/2, build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio, build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])
                                        listFemellesAct.append([build_ele.dadesTDHortInter.value[nbarraHorAnterior].femellaInf and build_ele.BarresHorList.value[nbarraHorAnterior].BarraHor , build_ele.BarresHorList.value[nbarraHorAnterior].Posicio - (posHorInf)  + alturaInfEncaix - build_ele.dadesTDHortInter.value[nHorInf].Altura/2, getOrientacioInv(build_ele.BarresHorList.value[nbarraHorAnterior].Orientacio), build_ele.BarresHorList.value[nbarraHorAnterior].AutoLongitud, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura ,ampleForatFem, tubGirat, build_ele.dadesTDHortInter.value[nbarraHorAnterior].Altura])


                            if tubGirat:
                                offsetY.append( build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs )
                                offsetY.append( build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs )
                            else:
                                offsetY.append(build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample/2 -ampleForatFem + build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs + ampleForatFem/2 )
                                offsetY.append(build_ele.dadesTDHortInter.value[nbarraHorAnterior].Ample/2 -ampleForatFem + build_ele.dadesTDHortInter.value[nbarraHorAnterior].desplY - build_ele.listDesplVerticals.value[j].desplYAbs + ampleForatFem/2 )
                ...
                '''
                if j > 0:
                    if build_ele.dadesTDVert.value[j-1].MostrarTDVertical:
                        n = 1
                    else:
                        n = 2
                    ...
                    #    offsetYAnt.append(build_ele.dadesTDHortInter.value[nBarraHor].Ample/2 - build_ele.dadesTDVert.value[j].Ample_forat_femellaVert/2 + build_ele.dadesTDHortInter.value[nBarraHor].desplY - build_ele.listDesplVerticals.value[j].desplYAbs)
                ...
                for nBarraHor in range(inici,final):
                    offsetY.append(build_ele.dadesTDHortInter.value[nBarraHor].Ample/2 - build_ele.dadesTDVert.value[j].Ample_forat_femellaVert/2 + build_ele.dadesTDHortInter.value[nBarraHor].desplY - build_ele.listDesplVerticals.value[j].desplYAbs)
                #'''
                ...
                if j > 0:
                    ...
                    for nBarraHor in range(iniciAnt,finalAnt):
                        #offsetYAnt.append(build_ele.dadesTDHortInter.value[nBarraHor].Ample/2 - build_ele.dadesENVert.value[j].Ample_forat_femellaVert/2 + build_ele.dadesTDHortInter.value[nBarraHor].desplY - build_ele.listDesplVerticals.value[j].desplYAbs)
                        ampleForatFem = build_ele.dadesTDHortInter.value[nBarraHor].Ample/2/2
                        if ampleForatFem > 15/2:
                            ampleForatFem = 15/2
                        offsetYAnt.append(build_ele.dadesTDHortInter.value[nBarraHor].Ample/2 -  ampleForatFem  + build_ele.dadesTDHortInter.value[nBarraHor].desplY - build_ele.listDesplVerticals.value[j].desplYAbs)
                ...
                for nBarraHor in range(inici,final):
                    #offsetY.append(build_ele.dadesTDHortInter.value[nBarraHor].Ample/2 - build_ele.dadesENVert.value[j].Ample_forat_femellaVert/2 + build_ele.dadesTDHortInter.value[nBarraHor].desplY - build_ele.listDesplVerticals.value[j].desplYAbs)
                    ampleForatFem = build_ele.dadesTDHortInter.value[nBarraHor].Ample/2/2
                    if ampleForatFem > 15/2:
                        ampleForatFem = 15/2
                    offsetY.append(build_ele.dadesTDHortInter.value[nBarraHor].Ample/2 -ampleForatFem + build_ele.dadesTDHortInter.value[nBarraHor].desplY - build_ele.listDesplVerticals.value[j].desplYAbs)
                """
                #FemellesAux = TDV_Int.actualitzar_femelles_TDH(listFemellesAnt, listFemellesAct, j, offsetY, offsetYAnt, crearFemellesxEncaixInf and build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior", desplInf, crearFemellesxEncaixSup and build_ele.dadesTDVert.value[j].BarraSuperior == "EN Superior", desplSup, separacioFemellaInf , separacioFemellaSup, separacioFemellaInt)
                ...
                '''
                if j != 0:
                    if not build_ele.dadesTDVert.value[j-1].MostrarTDVertical:
                        if j-2 >= 0:
                            if posnListTDIAnt2 >= 0:
                                FemellesAux = TDV_Int.actualitzar_femelles_TDH(listFemellesAnt2, listFemellesAct, j, offsetY, offsetYAnt, crearFemellesxEncaixInf and build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior", desplInf, crearFemellesxEncaixSup and build_ele.dadesTDVert.value[j].BarraSuperior == "EN Superior", desplSup, separacioFemellaInf , separacioFemellaSup, separacioFemellaInt)
                '''
                ...
                try:
                    ...
                    if  j < len(build_ele.nListBarresHor.value):
                        ...
                        for posBarraHor in range(inici, final):#recorrer totes les Barres Horitzontals de cada vertical
                            ...
                            tubHorSeleccionat = 0
                            if getNumFromText(build_ele, build_ele.SelectorTDHTotal.value, 3) != "":
                                tubHorSeleccionat = getNumFromText(build_ele, build_ele.SelectorTDHTotal.value, 3)

                            if build_ele.SelectorPP.value == 1 and build_ele.SelectorTDH.value == "Mes Tubs..." and tubHorSeleccionat == posBarraHor and not build_ele.BarresHorList.value[posBarraHor].Edit:
                                build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Edit = True)
                                build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(Edit = True)
                                mostrar_valors_horToShow(build_ele, posBarraHor)
                            if build_ele.SelectorPP.value == 1 and build_ele.SelectorTDH.value == "Mes Tubs..." and tubHorSeleccionat == posBarraHor and build_ele.BarresHorList.value[posBarraHor].Edit:
                                guardar_valors_horToShow(build_ele, posBarraHor)
                                #guardar_valors_hor(build_ele, j, posBarraHor)
                            if build_ele.SelectorPP.value != 1 or build_ele.SelectorTDH.value != "Mes Tubs...":
                                #build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(Edit = False)
                                build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Edit = False)
                                #print(build_ele.BarresHorList.value[posBarraHor].Edit)

                            if build_ele.SelectorTDHTotal.value != build_ele.SelectorTDHTotalAnt.value :#or build_ele.SelectorTDH.value != build_ele.SelectorTDHAnt.value:
                                tubHorSeleccionatAnt = 0
                                if getNumFromText(build_ele, build_ele.SelectorTDHTotalAnt.value, 3) != "":
                                    tubHorSeleccionatAnt = getNumFromText(build_ele, build_ele.SelectorTDHTotalAnt.value, 3)

                                build_ele.BarresHorList.value[tubHorSeleccionatAnt] = build_ele.BarresHorList.value[tubHorSeleccionatAnt]._replace(Edit = False)
                            build_ele.SelectorTDHTotalAnt.value = build_ele.SelectorTDHTotal.value
                            build_ele.SelectorTDHAnt.value = build_ele.SelectorTDH.value

                            #if NBarraInici < len(trans_list)  and build_ele.BarresHorList.value[posBarraHor].BarraHor:
                            if NBarraInici < build_ele.IntegerTDSelector.value  :#and build_ele.BarresHorList.value[posBarraHor].BarraHor:
                                #if NBarraFinal+1 < len(trans_list):
                                if NBarraFinal+1 < build_ele.IntegerTDSelector.value:
                                    if not build_ele.BarresHorList.value[posBarraHor].AutoLongitud:
                                        llargada = build_ele.BarresHorList.value[posBarraHor].Longitud
                                    else:
                                        if build_ele.dadesTDVert.value[NBarraFinal].MostrarTDVertical :#and not build_ele.BarresVertList.value[j*NombreBarresVertInt].BarraVert:
                                            if NBarraInici == 0 and NBarraFinal == build_ele.IntegerTDSelector.value -1 :
                                                #print("1 - de 0  a ultima : "+ str(posBarraHor))
                                                llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs  - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple -  build_ele.dadesTDHortInter.value[posBarraHor].desplX#- build_ele.dadesTDVert.value[0].BarraAmple - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2
                                            elif NBarraInici == 0:
                                                #print("1 - de 0  a n : "+ str(posBarraHor))
                                                llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs  - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs #- build_ele.dadesTDVert.value[0].BarraAmple/2 + build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2
                                            elif NBarraFinal == build_ele.IntegerTDSelector.value -1:
                                                #print("1 - de n  a ultima : "+ str(posBarraHor))
                                                llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs  - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple -  build_ele.dadesTDHortInter.value[posBarraHor].desplX#- build_ele.dadesTDVert.value[NBarraFinal].BarraAmple
                                            else:
                                                #print("1 - de n  a n : "+ str(posBarraHor))
                                                #llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple  + (build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2) - build_ele.dadesTDHortInter.value[NBarraInici].desplX
                                                llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple  - build_ele.dadesTDHortInter.value[posBarraHor].desplX#+ (build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2) - build_ele.dadesTDHortInter.value[NBarraInici].desplX
                                        elif not build_ele.dadesTDVert.value[NBarraFinal].MostrarTDVertical and build_ele.BarresVertList.value[(NBarraFinal)*NombreBarresVertInt].BarraVert:
                                            llargada = build_ele.BarresVertList.value[(NBarraFinal)*NombreBarresVertInt].PosicioAbs - build_ele.dadesTDVertInter.value[(NBarraFinal)*NombreBarresVertInt].BarraAmple/2 - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - build_ele.dadesTDHortInter.value[posBarraHor].desplX
                                        else:
                                            llargada = build_ele.listDesplVerticals.value[NBarraFinal+1].desplXAbs - build_ele.dadesTDVert.value[NBarraFinal+1].BarraAmple/2 - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - build_ele.dadesTDHortInter.value[posBarraHor].desplX
                                    ...
                                    if not build_ele.BarresHorList.value[posBarraHor].AutoLongitud:
                                        ...
                                        if NBarraInici == 0 and NBarraFinal == build_ele.IntegerTDSelector.value -1 :
                                            ...
                                            llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs  - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDHortInter.value[posBarraHor].desplX#- build_ele.dadesTDVert.value[0].BarraAmple #- build_ele.dadesTDVert.value[NBarraFinal].BarraAmple
                                        elif NBarraFinal == build_ele.IntegerTDSelector.value -1:
                                            #print("2 - de n  a ultima : "+ str(posBarraHor))
                                            #llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs  - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple# - build_ele.dadesTDVert.value[0].BarraAmple/2 + build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - (build_ele.dadesTDVert.value[NBarraInici].BarraAmple - (build_ele.dadesTDVert.value[0].BarraAmple/2))
                                            #llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs  - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple - build_ele.dadesTDVert.value[0].BarraAmple# - build_ele.dadesTDVert.value[0].BarraAmple/2 + build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - (build_ele.dadesTDVert.value[NBarraInici].BarraAmple - (build_ele.dadesTDVert.value[0].BarraAmple/2))
                                            #llargada = (build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple - build_ele.dadesTDVert.value[0].BarraAmple) - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple#build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2
                                            llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple - build_ele.dadesTDHortInter.value[posBarraHor].desplX#build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2
                                        #if NBarraFinal == build_ele.IntegerTDSelector.value -1:
                                        #    llargada = build_ele.listDesplVerticals.value[NBarraFinal].desplXAbs - build_ele.dadesTDVert.value[NBarraFinal].BarraAmple/2 - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2
                                        else:
                                            llargada = build_ele.dadesTDVert.value[NBarraInici].BarraAmple
                                ...
                                if testEditBarraHorInterior :#and nVer == j:
                                    ...
                                    if  build_ele.BarresHorList.value[posBarraHor].acabatEditar == False and build_ele.BarresHorList.value[posBarraHor].Edit == True: #al clicar a edit
                                        ...
                                        build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Edit = True)
                                        build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(acabatEditar = True)
                                    ...
                                    if build_ele.dadesTDHortInter.value[posBarraHor].acabatEditar == True :#and nVer == j:
                                        #guardar valors
                                        #guardar_valors_hor(build_ele, j, posBarraHor - inici)
                                        if build_ele.SelectorPP.value == 2:
                                            mostrar_valors_actuals(build_ele, nVer)
                                ...
                                for nBarraVertical in range(0, len(build_ele.dadesTDVert.value)):
                                    #print(str(build_ele.dadesTDVert.value[nBarraVertical].BarraInferior) + "== 'Tub " + str(posBarraHor)+"'")
                                    #print(str(build_ele.dadesTDVert.value[nBarraVertical].BarraSuperior) + "== 'Tub " + str(posBarraHor)+"'")
                                    if build_ele.dadesTDVert.value[nBarraVertical].BarraInferior == "Tub " +str(posBarraHor):#Numeracio malament #+ str(j*(posBarraHor - inici)):
                                        pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 +0.5
                                        pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2) - build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2

                                        if NBarraInici != 0:
                                            pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple) - ( build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2)
                                            pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple)
                                        else:
                                            pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple) + build_ele.dadesTDVert.value[nBarraVertical].BarraAmple + (build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2)
                                            pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs

                                        orientInfA =  "Esq"
                                        desplY = build_ele.listDesplVerticals.value[nBarraVertical].desplYAbs
                                        desplYInf = build_ele.dadesTDHortInter.value[posBarraHor].desplY
                                        puntCentralY = build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2 + desplY
                                        puntCentralYInf = build_ele.dadesTDHortInter.value[posBarraHor].Ample/2 + desplYInf

                                        Separacio_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAmple - 2.0
                                        Ample_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2
                                        if Ample_forat_femella >= 15:
                                            Ample_forat_femella = 15

                                        posFemellaY = puntCentralY - puntCentralYInf + Ample_forat_femella
                                        if build_ele.dadesTDVert.value[nBarraVertical].EncaixInf:
                                            orientInfA =  "Esq"
                                            if Ample_forat_femella< 15:
                                                posFemellaY = desplY - desplYInf + build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2 -  Ample_forat_femella/2
                                            else:
                                                posFemellaY = puntCentralY - puntCentralYInf + Ample_forat_femella/2
                                                posFemellaY = desplY - desplYInf + build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2 - Ample_forat_femella/2
                                            if build_ele.dadesTDVert.value[nBarraVertical].PestanyesInv:
                                                if desplY - desplYInf == 0:
                                                    posFemellaY = 0.1
                                                else:

                                                    posFemellaY = desplY - desplYInf
                                                Separacio_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAltura - 2
                                                Ample_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2
                                                if Ample_forat_femella >= 15:
                                                    Ample_forat_femella = 15
                                        else:
                                            desplY = build_ele.listDesplVerticals.value[nBarraVertical].desplYAbs
                                        ...
                                        if build_ele.dadesTDVert.value[nBarraVertical].PestanyesInv:
                                            pos = pos + build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - Ample_forat_femella/2
                                        ...
                                        bob = TDHorCollection(Femella = build_ele.dadesTDVert.value[nBarraVertical].MostrarTDVertical and build_ele.dadesTDVert.value[nBarraVertical].FemellaInf,
                                                            ...
                                                            tubGirat = build_ele.dadesTDVert.value[nBarraVertical].PestanyesInv)
                                        femelles.append(bob)
                                        orientInv = getOrientacioInv(orientInfA)
                                        bob = bob._replace(FemellaOr = orientInv)
                                        femelles.append(bob)
                                    #if build_ele.dadesTDVert.value[nBarraVertical].BarraSuperior == "Tub " + str(j) + "."+ str(posBarraHor - inici+1):
                                    if build_ele.dadesTDVert.value[nBarraVertical].BarraSuperior == "Tub " + str(posBarraHor):#Numeracio Malament #+ str(j*(posBarraHor - inici)):
                                        pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2 -0.5
                                        pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - build_ele.listDesplVerticals.value[NBarraInici].desplXAbs - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2 -0.5
                                        pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2) - build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2
                                        ...
                                        if build_ele.dadesTDVert.value[nBarraVertical].EncaixSup:
                                            orientSupA =  "Dre"
                                            if Ample_forat_femella< 15:
                                                posFemellaY = puntCentralY - puntCentralYSup + Ample_forat_femella
                                            else:
                                                posFemellaY = puntCentralY - puntCentralYSup + build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2 - Ample_forat_femella
                                                posFemellaY = desplY - desplYSup + build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2 - Ample_forat_femella/2
                                            if build_ele.dadesTDVert.value[nBarraVertical].PestanyesInv:
                                        ...
                                        if orientacio == "Sup" or orientacio == "Inf":
                                            ...
                                            if barraInici == "Inferior" and orientacio == "Dre":
                                                posY = Altura/2 - 15/2 + desply - build_ele.desplYI.value + desplAct - build_ele.listDesplVerticals.value[j].desplY - build_ele.dadesTDHortInter.value[barraSupHor].desplY
                                            else:
                                                if orientacio == "Esq":
                                                    posY = Altura/2 - 15/2 + desply  - desplAct - build_ele.listDesplVerticals.value[j].desplY #- build_ele.dadesTDHortInter.value
                                                else:
                                                    posY = Altura/2 - 15/2 + desply   - build_ele.listDesplVerticals.value[j].desplY - build_ele.dadesTDHortInter.value[barraSupHor].desplY
                                        '''
                                        ...
                                        if build_ele.dadesTDVert.value[nBarraVertical].PestanyesInv:
                                            pos = pos + build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - Ample_forat_femella/2
                                        TDHorCollection = collections.namedtuple('StirrupList', 'Femella FemellaOr PosFemellaX PosFemellaY Separacio_forat_femella Ample_forat_femella tubGirat')
                                        bob = TDHorCollection(Femella = build_ele.dadesTDVert.value[nBarraVertical].MostrarTDVertical and build_ele.dadesTDVert.value[nBarraVertical].FemellaSup,
                                ...
                                """
                                for nForat in range(0,len(forats)):
                                    if forats[nForat].Forat and forats[nForat].MostrarBox and build_ele.BarresHorList.value[posBarraHor].BarraHor:
                                        group_elems.append(createBoxForatHorInt(build_ele, posBarraHor, forats,j,nForat))
                                """

                                vermellHor = build_ele.dadesTDHortInter.value[posBarraHor].FounColor
                                if build_ele.dadesTDHortInter.value[posBarraHor].vermell:
                                    vermellHor = 6

                                tableAux = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.dadesTDHortInter.value[posBarraHor].Ample, build_ele.dadesTDHortInter.value[posBarraHor].Altura,llargada, build_ele.dadesTDHortInter.value[posBarraHor].Gruix,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                                        build_ele.dadesTDHortInter.value[posBarraHor].IsUseGlobalProp, vermellHor, build_ele.BarraLayer.value,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                                        ...
                                                        encaixos,build_ele.dadesTDHortInter.value[posBarraHor].pestanyesInv) #EncaixosPar))

                                if not tableAux.is_valid():
                                    return[]


                                posInici = build_ele.listDesplVerticals.value[NBarraInici].desplXAbs
                                ...
                                if build_ele.BarresHorList.value[posBarraHor].Orientacio == 'Inf' and not build_ele.BarresHorList.value[posBarraHor].AutoLongitud :
                                    #matrix_Aux.SetValue(12, matrix[12] + build_ele.listDesplVerticals.value[j].desplX - llargada - build_ele.dadesTDHortInter.value[posBarraHor].desplX  + build_ele.listDesplVerticals.value[j].desplX  )
                                    #matrix_Aux.SetValue(12, build_ele.listDesplVerticals.value[j].desplXAbs - build_ele.dadesTDVert.value[j].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 2 - llargada)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)

                                    matrix_Aux.SetValue(12, posInici - build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 + build_ele.dadesTDHortInter.value[posBarraHor].desplX + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 1.5 - llargada)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)
                                    matrix_Aux.SetValue(12, posInici )#+ build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 + build_ele.dadesTDHortInter.value[posBarraHor].desplX + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 1.5 - llargada)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)

                                else:
                                    #matrix_Aux.SetValue(12, matrix[12] + build_ele.listDesplVerticals.value[j].desplX + build_ele.dadesTDVert.value[j].BarraAmple - build_ele.dadesTDHortInter.value[posBarraHor].desplX)
                                    #matrix_Aux.SetValue(12, build_ele.listDesplVerticals.value[j].desplXAbs + build_ele.dadesTDVert.value[j].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 2)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)
                                    if NBarraInici == 0:
                                        matrix_Aux.SetValue(12, posInici + build_ele.dadesTDVert.value[NBarraInici].BarraAmple + build_ele.dadesTDHortInter.value[posBarraHor].desplX)#/2 + build_ele.dadesTDHortInter.value[posBarraHor].desplX + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 1.5)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)
                                    else:
                                        matrix_Aux.SetValue(12, posInici + build_ele.dadesTDVert.value[NBarraInici].BarraAmple + build_ele.dadesTDVert.value[0].BarraAmple + build_ele.dadesTDHortInter.value[posBarraHor].desplX)# + build_ele.dadesTDVert.value[NBarraInici].BarraAmple/2 + build_ele.dadesTDHortInter.value[posBarraHor].desplX + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 1.5)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)
                                ...
                                if not build_ele.EncaixHorInf.value:
                                    matrix_Aux.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio + build_ele.BarraAlturaInf.value )#- build_ele.dadesTDHortInter.value[posBarraHor].Altura/2 -1 )
                                else:
                                    matrix_Aux.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio + build_ele.BarraAlturaInf.value)#- build_ele.dadesTDHortInter.value[posBarraHor].Altura/2 -1 )
                                ...
                                if build_ele.BarresAdjList.value[posBarraHor].BarraAdj:
                                    ...
                                    horitzontalPPOBject4 = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.dadesTDHortInter.value[posBarraHor].Ample, build_ele.dadesTDHortInter.value[posBarraHor].Altura+10,build_ele.BarresAdjList.value[posBarraHor].Longitud, 0,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                                        False, common_propsObject.Color, common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                                        [], [],[], #ColisPar, PotaPar, #matrius
                                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                                        [], #Femelles, #matriu
                                    ...
                                    if matrix_Adj[12]< build_ele.BarraLlargada.value + build_ele.ExtenderInf.value:
                                ...
                                horitzontalPPOBject3 = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.dadesTDHortInter.value[posBarraHor].Ample + 2, build_ele.dadesTDHortInter.value[posBarraHor].Altura+6,llargada, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                                        False, common_propsObject.Color, common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                                        [], [],[], #ColisPar, PotaPar, #matrius
                                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                                        [], #Femelles, #matriu
                                                        ...
                                                        False, False,#PestanyaSuperior, PestanyaInferior,
                                                        [])
                                horitzontalPPOBject3Recess = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.dadesTDHortInter.value[posBarraHor].Ample + 2 , build_ele.dadesTDHortInter.value[posBarraHor].Altura+6,llargada, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                                        False, common_propsObjectRecess.Color, common_propsObjectRecess.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                                        [], [],[], #ColisPar, PotaPar, #matrius
                                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                                        [], #Femelles, #matriu
                                ...
                                if matrix_Aux[12]< build_ele.BarraLlargada.value + build_ele.ExtenderInf.value and build_ele.BarresHorList.value[posBarraHor].BarraHor:
                                    ...
                                    if build_ele.dadesTDHortInter.value[posBarraHor].mostrarLiniaVertA:
                                        ...
                                        if (len(build_ele.dadesTDHortInter.value[posBarraHor].layer) > 1):
                                            ...
                                            build_ele.dadesTDHortInter.value[posBarraHor] = build_ele.dadesTDHortInter.value[posBarraHor]._replace(layer = "EN_FIXACIO_X")
                                            layer = "EN_FIXACIO_X"
                    ...
                    print("no s'ha creat Horitzontal:", e)
                ...
                if j< len(matrixMostrar):
                    if matrixMostrar[j] and matrix0[12]< build_ele.BarraLlargada.value + build_ele.ExtenderInf.value:
                        ...
                        if j != 0 and j != build_ele.IntegerTDSelector.value -1 :
                            group_elems.append(PythonPart ("Cavitat_Vertical", parameter_list = get_var_chair_vert(build_ele, j, []),
                        ...
                        if build_ele.listDesplVerticals.value[j].mostrarLiniaVertA:
                            punt_central = matrix0[12] + build_ele.listDesplVerticals.value[j].desplX + build_ele.dadesTDVert.value[j].BarraAltura/2
                            pointUbi = AllplanGeo.Point3D(matrix0[12] + build_ele.dadesTDVert.value[j].BarraAmple/2 + build_ele.listDesplVerticals.value[j].desplLinA, matrix0[13] + build_ele.dadesTDVert.value[j].BarraAltura/2 , matrix0[14])
                            if (len(build_ele.dadesTDVert.value[j].linia)) <=1:
                                build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(linia = "Tipus 1")
                            linia = build_ele.dadesTDVert.value[j].linia[len(build_ele.dadesTDVert.value[j].linia)-1]
                            if len(build_ele.dadesTDVert.value[j].layer) == 0:
                                build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(layer = "EN_FIXACIO_X")
                            layer = build_ele.dadesTDVert.value[j].layer
                            polyhedronCentral = create_polyline_interior(build_ele, punt_central, build_ele.dadesTDVert.value[j].BarraAltura, BarrallargadaVert, False, pointUbi, int(linia), layer)
                            if polyhedronCentral != []:
                                group_elems.append(polyhedronCentral[0])
                                group_elems_preview.append(polyhedronCentral[0])
                                group_elems.append(polyhedronCentral[3])
                                ...
                                #model_ele_list2.append(AllplanBasisElements.ModelElement3D(polyhedronCentral[1], polyhedronCentral[0]))
                                #preview_ele_list.append(AllplanBasisElements.ModelElement3D(polyhedronCentral[1], polyhedronCentral[2]))
                        if build_ele.listDesplVerticals.value[j].mostrarLiniaVertB:
                            punt_central = matrix0[12] + build_ele.listDesplVerticals.value[j].desplX + build_ele.dadesTDVert.value[j].BarraAltura/2
                            pointUbi = AllplanGeo.Point3D(matrix0[12] + build_ele.dadesTDVert.value[j].BarraAmple/2 + build_ele.listDesplVerticals.value[j].desplLinB, matrix0[13] + build_ele.dadesTDVert.value[j].BarraAltura/2 , matrix0[14])
                            if (len(build_ele.dadesTDVert.value[j].linia)) <=1:
                                build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(linia = "Tipus 1")
                            linia = build_ele.dadesTDVert.value[j].linia[len(build_ele.dadesTDVert.value[j].linia)-1]
                            if len(build_ele.dadesTDVert.value[j].layer) == 0:
        ...
        for posBarraRef in range(inici, final):#recorrer totes les Barres Reforcs de cada vertical
            #----------------------- Afegir Barra Ref -----------------------------------
            if posBarraRef >= len(build_ele.BarresRefList.value):
                set_values_barresRef(build_ele, posBarraRef)


            if nRef == posBarraRef and build_ele.SelectorPP.value == 4:
                ...
                #si no estan mostrats
                #if  build_ele.BarresRefListToShow.value[0].acabatEditar == False and nRef == posBarraRef: #al clicar a edit
                if  build_ele.valueAntReforc.value != nRef and nRef == posBarraRef: #al clicar a edit

                    #build_ele.BarresRefList.value[posBarraRef] = build_ele.BarresRefList.value[posBarraRef]._replace(Edit = True)
                    #mostrar valors
                    ...
                    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(acabatEditar = True)
                ...
                if build_ele.dadesENReftInter.value[posBarraRef].acabatEditar == True and nRef == posBarraRef:
                    ...
                    if build_ele.SelectorPP.value == 2:
                        mostrar_valors_actuals(build_ele, nVer)
                    elif build_ele.SelectorPP.value == 1:
                        print("build_ele.SelectorPP.value: " + str(build_ele.SelectorPP.value))
                    elif build_ele.SelectorPP.value == 3:
                        print("build_ele.SelectorPP.value: " + str(build_ele.SelectorPP.value))
                    build_ele.dadesENReftInter.value[posBarraRef] = build_ele.dadesENReftInter.value[posBarraRef]._replace(acabatEditar = False)
                    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(acabatEditar = False)
            if len(build_ele.dadesENReftInter.value) <= posBarraRef  :
                set_valors_Ref_ini(build_ele, posBarraRef)
            ...
            llargada = build_ele.BarresRefList.value[posBarraRef].Longitud
            BarraIniciRef = 0
            BarraFinalRef = 1
            if getNumFromText(build_ele, build_ele.BarresRefList.value[posBarraRef].BarraIni, 3) != "":
                BarraIniciRef = getNumFromText(build_ele, build_ele.BarresRefList.value[posBarraRef].BarraIni, 3)
            if getNumFromText(build_ele, build_ele.BarresRefList.value[posBarraRef].BarraFin, 3) != "":
                BarraFinalRef = getNumFromText(build_ele, build_ele.BarresRefList.value[posBarraRef].BarraFin, 3)
            ...
            if build_ele.BarresRefList.value[posBarraRef].BarraRef:
                llargada = build_ele.BarresRefList.value[posBarraRef].Longitud
                if build_ele.BarresRefList.value[posBarraRef].AutoLongitud:
                    if build_ele.listDesplVerticals.value[BarraFinalRef].desplXAbs -(build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs + build_ele.dadesTDVert.value[BarraIniciRef].BarraAmple) > 0:
                        if BarraIniciRef == 0:
                            #llargada = build_ele.listDesplVerticals.value[BarraFinalRef].desplXAbs - build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs  + (build_ele.dadesTDVert.value[BarraFinalRef].BarraAmple/2 - build_ele.dadesTDVert.value[BarraIniciRef].BarraAmple/2)
                            llargada = build_ele.listDesplVerticals.value[BarraFinalRef].desplXAbs - build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs  + build_ele.dadesENReftInter.value[posBarraRef].desplX
                        else:
                            llargada = build_ele.listDesplVerticals.value[BarraFinalRef].desplXAbs - build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs - build_ele.dadesTDVert.value[BarraIniciRef].BarraAmple  + build_ele.dadesENReftInter.value[posBarraRef].desplX
                    else:
                        print("El segon tub ha d'estar en una posicio mes avançada que el primer")
                        ctypes.windll.user32.MessageBoxW(0, "El segon tub ha d'estar en una posicio mes avançada que el primer, selecciona els tubs inicial i final de forma correcta del reforç '" + str(posBarraRef) +"' (longitud " + str(build_ele.listDesplVerticals.value[BarraFinalRef].desplXAbs -(build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs + build_ele.dadesTDVert.value[BarraIniciRef].BarraAmple)) + ")", 0)
                        llargada = 30
                else:
                    llargada = build_ele.BarresRefList.value[posBarraRef].Longitud
                ...
                reforcRef = PP_EN_Horitzontal_reforc(random.random() * 3600, build_ele.dadesENReftInter.value[posBarraRef].Ample, build_ele.dadesENReftInter.value[posBarraRef].Altura,llargada-5 , build_ele.dadesENReftInter.value[posBarraRef].Gruix, build_ele.BarresRefList.value[posBarraRef].Orientacio,# (ENV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada( ), BarraGruix,
                ...
                if BarraIniciRef == 0:
                    matrix_AuxR.SetValue(12, build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs  + build_ele.dadesTDVert.value[BarraIniciRef].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2 + 2.5  + build_ele.dadesENReftInter.value[posBarraRef].desplX)
                else:
                    matrix_AuxR.SetValue(12, build_ele.listDesplVerticals.value[BarraIniciRef].desplXAbs  + build_ele.dadesTDVert.value[0].BarraAmple + build_ele.dadesTDVert.value[BarraIniciRef].BarraAmple + 2.5  + build_ele.dadesENReftInter.value[posBarraRef].desplX)
                matrix_AuxR.SetValue(13, build_ele.listDesplVerticals.value[BarraIniciRef].desplYAbs + build_ele.dadesENReftInter.value[posBarraRef].desplY)
                matrix_AuxR.SetValue(14, build_ele.BarresRefList.value[posBarraRef].Posicio)
                ...
                if  mostrarActual == "ENR"+str(nRef) and build_ele.BarresRefList.value[posBarraRef].BarraRef and nRef == posBarraRef :
                ...
                ReforcPPOBjectR = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.dadesENReftInter.value[posBarraRef].Ample, build_ele.dadesENReftInter.value[posBarraRef].Altura+10,llargada, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                                        False, common_propsObject.Color, common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                                        [], [],[], #ColisPar, PotaPar, #matrius
                                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                                        [], #Femelles, #matriu
                                                        ...
                                                        False, False,#PestanyaSuperior, PestanyaInferior,
                                                        [])
                ReforcPPOBjectRRecess = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.dadesENReftInter.value[posBarraRef].Ample, build_ele.dadesENReftInter.value[posBarraRef].Altura+10,llargada, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                                        False, common_propsObjectRecess.Color, common_propsObjectRecess.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                                        [], [],[], #ColisPar, PotaPar, #matrius
                                                        0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                                        [], #Femelles, #matriu
                ...
                if build_ele.dadesENReftInter.value[posBarraRef].mostrarLiniaVertA:
                    ...
                    polyhedronCentral = create_polyline_interior(build_ele, punt_central, build_ele.dadesENReftInter.value[posBarraRef].Altura, llargada-5, True, pointUbi, int(linia), layer)
                    if polyhedronCentral != []:
                        group_elems.append(polyhedronCentral[0])
                        group_elems_preview.append(polyhedronCentral[0])
                        group_elems.append(polyhedronCentral[3])
        ...
        try:
            ...
            for posBarraIncl in range(inici, final):#recorrer totes les Barres Inclinades
                ...
                #                    build_ele.IsUseGlobalProp.value, build_ele.FounColor.value, build_ele.BarraLayer.value # IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                #                    ) #EncaixosPar)
                inclinatPP = PP_EN_Premarc(random.random() * 3600, build_ele.ProfBalconera.value, build_ele.dadesTDInclinades.value[posBarraIncl].Altura , build_ele.dadesTDInclinades.value[posBarraIncl].Llargada, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                ...
                inclinatfDEN= build_ele.nomPremarc.value
                if build_ele.reconeixerDen.value:
                ...
                if build_ele.RotarPremarc.value:
                    vectorI1.Rotation(z_axis, AllplanGeo.Angle.FromDeg(180))
                    vectorI1.SetValue(12, build_ele.dadesTDInclinades.value[posBarraIncl].desplX + build_ele.dadesTDInclinades.value[posBarraIncl].Llargada )
                    vectorI1.SetValue(13, build_ele.dadesTDInclinades.value[posBarraIncl].desplY + build_ele.ProfBalconera.value -10 + build_ele.PosicioYBalconera.value )
                else:
                    vectorI1.SetValue(12, build_ele.dadesTDInclinades.value[posBarraIncl].desplX)
                    vectorI1.SetValue(13, build_ele.dadesTDInclinades.value[posBarraIncl].desplY + build_ele.PosicioYBalconera.value)
            ...
            print("No s'ha pogut crear el Premarc " + e)
...
def get_var_chair_vert(build_ele, i, FemellesAux):
    ...
    param_list.append ("Ample_forat_femella = %s\n" % build_ele.dadesTDVert.value[i].Ample_forat_femellaVert)
    param_list.append ("Altura_forat_femella = %s\n" % build_ele.dadesTDVert.value[i].Altura_forat_femellaVert)
    param_list.append ("Separacio_forat_femella = %s\n" %build_ele.dadesTDVert.value[i].Separacio_forat_femellaVert)
    ...
    for y in range(build_ele.nListFemellesVert.value[i].Posicio, build_ele.nListFemellesVert.value[i].Posicio + build_ele.nListFemellesVert.value[i].nTotal):
        Femelles.append(build_ele.FemellesVertList.value[y])

    listFemelles = Femelles+FemellesAux

    param_list.append ("Femelles = %s\n" % listFemelles)#matriu -------------FEMELLES--------------------
    param_list.append ("posicio_centre_masses = %s\n" % build_ele.dadesTDVert.value[i].posicio_centre_massesVert)
    param_list.append ("IsFirstCancam = %s\n" % build_ele.dadesTDVert.value[i].IsFirstCancamVert)
    param_list.append ("Dis1cancam = %s\n" % build_ele.dadesTDVert.value[i].Dis1cancamVert)
    param_list.append ("IsSecondCancam = %s\n" % build_ele.dadesTDVert.value[i].IsSecondCancamVert)
    param_list.append ("Dis2cancam = %s\n" % build_ele.dadesTDVert.value[i].Dis2cancamVert)
    param_list.append ("PestanyaSuperior = %s\n" % build_ele.dadesTDVert.value[i].PestanyaSuperiorVert)
    param_list.append ("PestanyaInferior = %s\n" % build_ele.dadesTDVert.value[i].PestanyaInferiorVert)
    Encaixos = []
...
def update_mat_nListVert(build_ele):
    ...
    #for barraVert in build_ele.dadesTDVert.value:
...
def guardar_valors_actuals(build_ele, i):
    ...
    while len(build_ele.listDesplVerticals.value) <= i:
        build_ele.listDesplVerticals.value.append(bobDespl)
    else:
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplX = build_ele.desplXVert.value)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplY = build_ele.desplYVert.value)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplXAbs = build_ele.desplXVertAbs.value)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplYAbs = build_ele.desplYVertAbs.value )
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplZAbs = build_ele.desplZVertAbs.value )
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(mostrarLiniaVertA = build_ele.mostrarLiniaVertA.value)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplLinA = build_ele.desplVertLinA.value)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(mostrarLiniaVertB = build_ele.mostrarLiniaVertB.value)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplLinB = build_ele.desplVertLinB.value)
    ...
    if len(build_ele.dadesTDVert.value) <= i:
        build_ele.dadesTDVert.value.append(bobDadesGlobal)
    else:
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(MostrarTDVertical = build_ele.MostrarTDVertical.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraAmple = build_ele.BarraAmpleVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraAltura = build_ele.BarraAlturaVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(LlargadaAut = build_ele.llargadaAutomatica.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraAlcada = build_ele.BarraLlargadaIndiv.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraSuperior = build_ele.SelectorTDHorSup.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraInferior = build_ele.SelectorTDHorInf.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Gruix = build_ele.BarraGruixVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(EncaixInf = build_ele.encaixInf.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(EncaixSup = build_ele.encaixSup.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FemellaInf = build_ele.femellaInf.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FemellaSup = build_ele.femellaSup.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(invertirEncaixSup = build_ele.invertirEncaixVertSup.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(invertirEncaixInf = build_ele.invertirEncaixVertInf.value)
        #build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FemellaInf = False)
        #build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FemellaSup = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Ample_forat_femellaVert = build_ele.Ample_forat_femellaVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Altura_forat_femellaVert = build_ele.Altura_forat_femellaVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Separacio_forat_femellaVert = build_ele.Separacio_forat_femellaVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(posicio_centre_massesVert = build_ele.posicio_centre_massesVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(IsFirstCancamVert = build_ele.IsFirstCancamVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Dis1cancamVert = build_ele.Dis1cancamVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(IsSecondCancamVert = build_ele.IsSecondCancamVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Dis2cancamVert = build_ele.Dis2cancamVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(PestanyaSuperiorVert = build_ele.PestanyaSuperiorVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(PestanyaInferiorVert = build_ele.PestanyaInferiorVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(PestanyesInv = build_ele.pestanyesInvVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(esProvisional = build_ele.esProvisional.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(linia = build_ele.SelectorLiniaVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(layer = build_ele.SelectorLayerVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(vermell = build_ele.esVermellVert.value)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FounColor = build_ele.FounColorVert.value)
    ...
    y = 0
    for x in range(inici, inici+final):
        if x >= len(build_ele.FemellesVertList.value):
        ...
        if build_ele.FemellesVertListToShow.value != [] and len(build_ele.FemellesVertListToShow.value) > y:
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(Femella = build_ele.FemellesVertListToShow.value[y].Femella)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(FemellaOr = build_ele.FemellesVertListToShow.value[y].FemellaOr)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(PosFemellaX = build_ele.FemellesVertListToShow.value[y].PosFemellaX)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(PosFemellaY = build_ele.FemellesVertListToShow.value[y].PosFemellaY)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(Ample_forat_femella = build_ele.dadesTDVert.value[y].Ample_forat_femellaVert)

            try:
                build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(PosFemellaXOri = build_ele.FemellesVertListToShow.value[y].PosFemellaXOri)
            except Exception as e:
                build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(PosFemellaXOri = 0)
        y += 1
    ...
    for x in range(inici, inici+final):
        ...
        if build_ele.ForatsVertListToShow.value != [] and len(build_ele.ForatsVertListToShow.value) > y:
            ...
            build_ele.ForatsVertList.value[x] = build_ele.ForatsVertList.value[x]._replace(MostrarBox = False)
        y += 1
    ...
    for x in range(inici, inici+final):
        ...
        if build_ele.BarresHorListToShow.value != [] and len(build_ele.BarresHorListToShow.value) > y:
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(BarraHor = build_ele.BarresHorListToShow.value[y].BarraHor)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(Orientacio = build_ele.BarresHorListToShow.value[y].Orientacio)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(Posicio = build_ele.BarresHorListToShow.value[y].Posicio)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(AutoLongitud = build_ele.BarresHorListToShow.value[y].AutoLongitud)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(Longitud = build_ele.BarresHorListToShow.value[y].Longitud)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(Edit = build_ele.BarresHorListToShow.value[y].Edit)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(acabatEditar = build_ele.BarresHorListToShow.value[y].acabatEditar)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(BarraInici = build_ele.BarresHorListToShow.value[y].BarraInici)
            build_ele.BarresHorList.value[x] = build_ele.BarresHorList.value[x]._replace(BarraFinal = build_ele.BarresHorListToShow.value[y].BarraFinal)
        y += 1
    '''
...
def mostrar_valors_actuals(build_ele, i):
    ...
    while len(build_ele.listDesplVerticals.value) <= i:
        ...
        build_ele.desplXVert.value = build_ele.listDesplVerticals.value[i].desplX
        build_ele.desplYVert.value = build_ele.listDesplVerticals.value[i].desplY
        build_ele.desplXVertAbs.value = build_ele.listDesplVerticals.value[i].desplXAbs
        build_ele.desplYVertAbs.value = build_ele.listDesplVerticals.value[i].desplYAbs
        build_ele.desplZVertAbs.value = build_ele.listDesplVerticals.value[i].desplZAbs
        build_ele.mostrarLiniaVertA.value = build_ele.listDesplVerticals.value[i].mostrarLiniaVertA
        build_ele.desplVertLinA.value = build_ele.listDesplVerticals.value[i].desplLinA
        build_ele.mostrarLiniaVertB.value = build_ele.listDesplVerticals.value[i].mostrarLiniaVertB
        build_ele.desplVertLinB.value = build_ele.listDesplVerticals.value[i].desplLinB
    ...
    bobDadesGlobal = TDCollectionDadesVert(MostrarTDVertical = build_ele.MostrarTDVertical.value,
                                            ...
                                            LlargadaAut = build_ele.llargadaAutomatica.value,
                                            BarraAlcada = build_ele.BarraLlargadaIndiv.value,
                                            BarraSuperior = "EN Superior",#build_ele.SelectorTDHorSup.value,
                                            BarraInferior = "EN Inferior",#build_ele.SelectorTDHorInf.value,
                                            Gruix = build_ele.BarraGruixVert.value,
                                            EncaixSup = build_ele.encaixSup.value,
                                            EncaixInf = build_ele.encaixInf.value,
                                            FemellaSup = build_ele.femellaSup.value,
    ...
    if len(build_ele.dadesTDVert.value) <= i:
        ...
        build_ele.MostrarTDVertical.value = build_ele.dadesTDVert.value[i].MostrarTDVertical
        build_ele.BarraAmpleVert.value = build_ele.dadesTDVert.value[i].BarraAmple
        build_ele.BarraAlturaVert.value = build_ele.dadesTDVert.value[i].BarraAltura
        build_ele.llargadaAutomatica.value = build_ele.dadesTDVert.value[i].LlargadaAut
        build_ele.BarraLlargadaIndiv.value = build_ele.dadesTDVert.value[i].BarraAlcada
        build_ele.SelectorTDHorSup.value = build_ele.dadesTDVert.value[i].BarraSuperior
        build_ele.SelectorTDHorInf.value = build_ele.dadesTDVert.value[i].BarraInferior
        build_ele.BarraGruixVert.value = build_ele.dadesTDVert.value[i].Gruix
        build_ele.encaixSup.value = build_ele.dadesTDVert.value[i].EncaixSup
        build_ele.encaixInf.value = build_ele.dadesTDVert.value[i].EncaixInf
        build_ele.femellaSup.value = build_ele.dadesTDVert.value[i].FemellaSup
        build_ele.femellaInf.value = build_ele.dadesTDVert.value[i].FemellaInf
        build_ele.invertirEncaixVertSup.value =  build_ele.dadesTDVert.value[i].invertirEncaixSup
        build_ele.invertirEncaixVertInf.value =  build_ele.dadesTDVert.value[i].invertirEncaixInf
        #build_ele.femellaSup.value = False#build_ele.dadesTDVert.value[i].FemellaSup
        #build_ele.femellaInf.value = False#build_ele.dadesTDVert.value[i].FemellaInf
        build_ele.Ample_forat_femellaVert.value = build_ele.dadesTDVert.value[i].Ample_forat_femellaVert
        build_ele.Separacio_forat_femellaVert.value = build_ele.dadesTDVert.value[i].Separacio_forat_femellaVert
        build_ele.posicio_centre_massesVert.value = build_ele.dadesTDVert.value[i].posicio_centre_massesVert
        build_ele.IsFirstCancamVert.value = build_ele.dadesTDVert.value[i].IsFirstCancamVert
        build_ele.Dis1cancamVert.value = build_ele.dadesTDVert.value[i].Dis1cancamVert
        build_ele.IsSecondCancamVert.value = build_ele.dadesTDVert.value[i].IsSecondCancamVert
        build_ele.Dis2cancamVert.value = build_ele.dadesTDVert.value[i].Dis2cancamVert
        build_ele.PestanyaSuperiorVert.value = build_ele.dadesTDVert.value[i].PestanyaSuperiorVert
        build_ele.PestanyaInferiorVert.value = build_ele.dadesTDVert.value[i].PestanyaInferiorVert
        build_ele.pestanyesInvVert.value = build_ele.dadesTDVert.value[i].PestanyesInv
        build_ele.esProvisional.value = build_ele.dadesTDVert.value[i].esProvisional
        build_ele.SelectorLiniaVert.value = build_ele.dadesTDVert.value[i].linia
        build_ele.SelectorLayerVert.value = build_ele.dadesTDVert.value[i].layer
        build_ele.esVermellVert.value = build_ele.dadesTDVert.value[i].vermell
        build_ele.FounColorVert.value = build_ele.dadesTDVert.value[i].FounColor
    ...
    if i < len(build_ele.nListFemellesVert.value):
        ...
        final = 0
    for x in range(inici, inici+final):
        if len(build_ele.FemellesVertList.value) <= x:
            ...
            build_ele.FemellesVertListToShow.value.append(build_ele.FemellesVertList.value[x])
    ...
    for x in range(inici, inici+final):
     ...
     #**********BarresHor*************
    '''
    build_ele.BarresHorListToShow.value = []
    if i < len(build_ele.nListBarresHor.value):
        inici = build_ele.nListBarresHor.value[i].Posicio
        final = build_ele.nListBarresHor.value[i].nTotal
    else:
    ...
    for x in range(inici, inici+final):
        ...
        if len(build_ele.BarresHorList.value) <= x:
            ...
            build_ele.BarresHorListToShow.value.append(bob)
        else:
            build_ele.BarresHorListToShow.value.append(build_ele.BarresHorList.value[x])
    '''
...
def set_values(build_ele, i):
    ...
    while len(build_ele.listDesplVerticals.value) <= i:
        ...
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplX = 0)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplY = 0)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplXAbs = 0 )
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplYAbs = 0 )
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplZAbs = 0 )
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(mostrarLiniaVertA = True)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplLinA = 0)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(mostrarLiniaVertB = False)
        build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplLinB = 0)
    ...
    #if i == nVer:
    #    build_ele.listDesplVerticals.value[i] = build_ele.listDesplVerticals.value[i]._replace(desplXAbs = build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[i].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2)
    ...
    bobDadesGlobal = TDCollectionDadesVert(MostrarTDVertical = False,
                                            ...
                                            vermell = False,
                                            FounColor = 1)
    while len(build_ele.dadesTDVert.value) <= i:
        build_ele.dadesTDVert.value.append(bobDadesGlobal)
    else:
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraAmple = 40)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraAltura = 50)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraAlcada = 2600)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(LlargadaAut = True)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraSuperior = "EN Superior")
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(BarraInferior = "EN Inferior")
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Gruix = 1.5)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(EncaixSup = True)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(EncaixInf = True)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(invertirEncaixSup = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(invertirEncaixInf = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Ample_forat_femellaVert = 15)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Altura_forat_femellaVert = 15)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Separacio_forat_femellaVert = 50)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(posicio_centre_massesVert = 500.00)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(IsFirstCancamVert = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Dis1cancamVert = 500)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(IsSecondCancamVert = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(Dis2cancamVert = 1000)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(PestanyaSuperiorVert = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(PestanyaInferiorVert = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(PestanyesInv = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FemellaSup = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FemellaInf = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(esProvisional = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(linia = "Tipus 1")
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(layer = "EN_ESTRUCTURA")
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(vermell = False)
        build_ele.dadesTDVert.value[i] = build_ele.dadesTDVert.value[i]._replace(FounColor = 1)

    build_ele.dadesTDVert.value[0] = build_ele.dadesTDVert.value[0]._replace(MostrarTDVertical = True)
    build_ele.dadesTDVert.value[i-1] = build_ele.dadesTDVert.value[i-1]._replace(MostrarTDVertical = True)
...
def set_values_femelles(build_ele, i):
    ...
    '''
    if len(build_ele.FemellesVertList.value) <= i:
        for x in range(len(build_ele.FemellesVertList.value), i+1):
            ...
            build_ele.FemellesVertList.value.append(bob)
...
def set_values_barresRef(build_ele, i):
    ...
    pos = 0
    cont = 0
    if len(build_ele.BarresRefList.value) <= i:
        for x in range(len(build_ele.BarresRefList.value), i+1):
            if cont == 3:
                pos = 0
                cont= 0
            cont += 1
            ...
            build_ele.BarresRefList.value.append(bob)
...
def actualitzar_amb_noves_femelles(build_ele, FemellesAux, i):
    ...
    listAnt = build_ele.FemellesVertList.value
    y = 0
    for x in range(inici, final):
        if x >= len(build_ele.FemellesVertList.value):
            build_ele.FemellesVertList.value.append(FemellesAux[y])
        elif build_ele.FemellesVertListToShow.value != [] and len(build_ele.FemellesVertListToShow.value) > y:
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(Femella = FemellesAux[y].Femella)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(FemellaOr = FemellesAux[y].FemellaOr)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(PosFemellaX = FemellesAux[y].PosFemellaX)
        y += 1

    y = build_ele.nListFemellesVert.value[i].nTotal + build_ele.nListFemellesVert.value[i].Posicio
    for x in range(final, len(build_ele.FemellesVertList.value)+len(FemellesAux)):
        if x >= len(build_ele.FemellesVertList.value):
            build_ele.FemellesVertList.value.append(listAnt[y])
        elif build_ele.FemellesVertListToShow.value != [] and len(build_ele.FemellesVertListToShow.value) > y:
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(Femella = listAnt[y].Femella)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(FemellaOr = listAnt[y].FemellaOr)
            build_ele.FemellesVertList.value[x] = build_ele.FemellesVertList.value[x]._replace(PosFemellaX = listAnt[y].PosFemellaX)
        y += 1
...
def mostrar_valors_horToShow(build_ele, posBarraHor):
    '''
    mostrar valors de les Barres Horitzontals intermitges al BarresHorListToShow
    get: posBarraHor -> posicio[] barra Horitzontal

    return: -

    '''
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(BarraHor = build_ele.BarresHorList.value[posBarraHor].BarraHor)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(Orientacio = build_ele.BarresHorList.value[posBarraHor].Orientacio)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(Posicio = build_ele.BarresHorList.value[posBarraHor].Posicio)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(AutoLongitud = build_ele.BarresHorList.value[posBarraHor].AutoLongitud)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(Longitud = build_ele.BarresHorList.value[posBarraHor].Longitud)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(BarraInici = build_ele.BarresHorList.value[posBarraHor].BarraInici)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(BarraFinal = build_ele.BarresHorList.value[posBarraHor].BarraFinal)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(Edit = build_ele.BarresHorList.value[posBarraHor].Edit)
    build_ele.BarresHorListToShow.value[0] = build_ele.BarresHorListToShow.value[0]._replace(acabatEditar = build_ele.BarresHorList.value[posBarraHor].acabatEditar)
...
def guardar_valors_horToShow(build_ele, posBarraHor):
    ...
    '''
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(BarraHor = build_ele.BarresHorListToShow.value[0].BarraHor)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Orientacio = build_ele.BarresHorListToShow.value[0].Orientacio)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Posicio = build_ele.BarresHorListToShow.value[0].Posicio)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(AutoLongitud = build_ele.BarresHorListToShow.value[0].AutoLongitud)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Longitud = build_ele.BarresHorListToShow.value[0].Longitud)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(BarraInici = build_ele.BarresHorListToShow.value[0].BarraInici)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(BarraFinal = build_ele.BarresHorListToShow.value[0].BarraFinal)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(Edit = build_ele.BarresHorListToShow.value[0].Edit)
    build_ele.BarresHorList.value[posBarraHor] = build_ele.BarresHorList.value[posBarraHor]._replace(acabatEditar = build_ele.BarresHorListToShow.value[0].acabatEditar)
...
def mostrar_valors_hor(build_ele, nBarraVert, nHor):
    ...
    #build_ele.desplZHorAbs.value = build_ele.dadesTDHortInter.value[nBarraVert * NombreBarresHorInt + nHor].desplZ
    build_ele.desplZHorAbs.value = build_ele.BarresHorList.value[nBarraVert * NombreBarresHorInt + nHor].Posicio
    ...
    build_ele.ForatsVertListToShow.value[0] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 0]
    build_ele.ForatsVertListToShow.value[0] = build_ele.ForatsVertListToShow.value[0]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[1] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 1]
    build_ele.ForatsVertListToShow.value[1] = build_ele.ForatsVertListToShow.value[1]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[2] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 2]
    build_ele.ForatsVertListToShow.value[2] = build_ele.ForatsVertListToShow.value[2]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[3] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 3]
    build_ele.ForatsVertListToShow.value[3] = build_ele.ForatsVertListToShow.value[3]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[4] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 4]
    build_ele.ForatsVertListToShow.value[4] = build_ele.ForatsVertListToShow.value[4]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[5] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 5]
    build_ele.ForatsVertListToShow.value[5] = build_ele.ForatsVertListToShow.value[5]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[6] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 6]
    build_ele.ForatsVertListToShow.value[6] = build_ele.ForatsVertListToShow.value[6]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[7] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 7]
    build_ele.ForatsVertListToShow.value[7] = build_ele.ForatsVertListToShow.value[7]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[8] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 8]
    build_ele.ForatsVertListToShow.value[8] = build_ele.ForatsVertListToShow.value[8]._replace(MostrarBox = False)
    build_ele.ForatsVertListToShow.value[9] = build_ele.ForatsHorInt.value[nBarraVert * NombreBarresHorInt + nHor*10 + 9]
    build_ele.ForatsVertListToShow.value[9] = build_ele.ForatsVertListToShow.value[9]._replace(MostrarBox = False)
    ...
    build_ele.PestanyaSuperiorHor.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].PestanyaSup
    build_ele.PestanyaInferiorHor.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].PestanyaInf
    build_ele.femellaSup.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].femellaSup
    build_ele.femellaInf.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].femellaInf

    build_ele.pestanyesInv.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].pestanyesInv

    build_ele.AtrPersAutomatic.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].AtrPersAutomatic
    build_ele.AtributPersonTub.value = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].AtributPersonTub
...
def guardar_valors_hor(build_ele, nBarraVert, nHor):
    ...
    build_ele.BarresHorList.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.BarresHorList.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Posicio = build_ele.desplZHorAbs.value)
    ...
    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Llargada = build_ele.BarraLlargadaVert.value)
    ...
    build_ele.FemellesHorInt.value[nBarraVert* NombreBarresHorInt + nHor* 3 + 0] = build_ele.FemellesHorInt.value[nBarraVert* NombreBarresHorInt + nHor* 3 + 0]._replace(Separacio_forat_femella = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].Separacio_forat_femella)
    build_ele.FemellesHorInt.value[nBarraVert* NombreBarresHorInt + nHor* 3 + 1] = build_ele.FemellesHorInt.value[nBarraVert* NombreBarresHorInt + nHor* 3 + 1]._replace(Separacio_forat_femella = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].Separacio_forat_femella)
    build_ele.FemellesHorInt.value[nBarraVert* NombreBarresHorInt + nHor* 3 + 2] = build_ele.FemellesHorInt.value[nBarraVert* NombreBarresHorInt + nHor* 3 + 2]._replace(Separacio_forat_femella = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor].Separacio_forat_femella)
    ...
    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(PestanyaSup = build_ele.PestanyaSuperiorHor.value)
    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(PestanyaInf = build_ele.PestanyaInferiorHor.value)
    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(femellaSup = build_ele.femellaSup.value)
    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(femellaInf = build_ele.femellaInf.value)

    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(pestanyesInv = build_ele.pestanyesInv.value)

    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(AtrPersAutomatic = build_ele.AtrPersAutomatic.value)
    build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDHortInter.value[nBarraVert* NombreBarresHorInt + nHor]._replace(AtributPersonTub = build_ele.AtributPersonTub.value)
...
def guardar_valors_incl(build_ele, nBarraVert, nHor):
    ...
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(MostrarInclinat = build_ele.MostrarInclinat.value)

    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(nom = build_ele.nomPremarc.value)

    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(desplX = build_ele.desplXVertAbs.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(desplY = build_ele.desplYVertAbs.value)

    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(mostrarLiniaVertA = build_ele.mostrarLiniaVertA.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(desplLinA = build_ele.desplVertLinA.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(mostrarLiniaVertB = build_ele.mostrarLiniaVertB.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(desplLinB = build_ele.desplVertLinB.value)

    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Ample = build_ele.BarraAmpleVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Altura = build_ele.BarraAlturaVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Llargada = build_ele.BarraLlargadaVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Gruix = build_ele.BarraGruixVert.value)


    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(IsUseGlobalProp = build_ele.IsUseGlobalPropVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(FounColor = build_ele.FounColor.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(BarraLayer = build_ele.BarraLayerVert.value)
    ...
    '''
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Ample_forat_femella = build_ele.Ample_forat_femellaVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Altura_forat_femella = build_ele.Altura_forat_femellaVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Separacio_forat_femella = build_ele.Separacio_forat_femellaVert.value)


    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(posicio_centre_masses = build_ele.posicio_centre_massesVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(IsFirstCancam = build_ele.IsFirstCancamVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Dis1cancam = build_ele.Dis1cancamVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(IsSecondCancam = build_ele.IsSecondCancamVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(Dis2cancam = build_ele.Dis2cancamVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(PestanyaSup = build_ele.PestanyaSuperiorVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(PestanyaInf = build_ele.PestanyaInferiorVert.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(femellaSup = build_ele.femellaSup.value)
    build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor] = build_ele.dadesTDInclinades.value[nBarraVert* NombreBarresHorInt + nHor]._replace(femellaInf = build_ele.femellaInf.value)
...
def set_valors_hor_ini(build_ele, nBarraVert, nHor):
    ...
    while nBarraVert * 3 + nHor >= len(build_ele.dadesTDHortInter.value):
        ...
        bob = TDHorCollection(Ample = 10,
                            Altura = 50,
                            Llargada = build_ele.BarraLlargada.value,
                            Gruix = build_ele.BarraGruixVert.value,
                            desplX = 0.0,
                            desplY = 0.0,
                            mostrarLiniaVertA = True,
...
def get_valors_actuals(build_ele, j):
    ...
    for x in range(build_ele.nListFemellesVert.value[j].Posicio, build_ele.nListFemellesVert.value[j].Posicio + build_ele.nListFemellesVert.value[j].nTotal):
        if x >= len(build_ele.FemellesVertList.value):
            set_values_femelles(build_ele, x)
        Femelles.append(build_ele.FemellesVertList.value[x])
...
def get_valors_actuals(build_ele, j, AmpleSup):
    ...
    for x in range(build_ele.nListEncaixVert.value[j].Posicio, build_ele.nListEncaixVert.value[j].Posicio + build_ele.nListEncaixVert.value[j].nTotal):
        if x >= len(build_ele.EncaixVertList.value):
            set_values_encaix(build_ele, x)
        Encaixos.append(build_ele.EncaixVertList.value[x])
    #if not build_ele.dadesTDVert.value[j].EncaixSup :#and build_ele.desplYS.value - (-build_ele.listDesplVerticals.value[j].desplYAbs) >= 0:
    nBarraINF = 0
    nBarraSup = 1
    ...
    ampleSup = AmpleSup
    if build_ele.dadesTDVert.value[j].BarraSuperior != "EN Superior":
        if len(build_ele.dadesTDVert.value[j].BarraSuperior) == 5:
            nBarraSup = build_ele.dadesTDVert.value[j].BarraSuperior[-1]
            nBarraSup = int(nBarraSup)
        elif len(build_ele.dadesTDVert.value[j].BarraSuperior) == 6:
            nBarraSup = build_ele.dadesTDVert.value[j].BarraSuperior[-2]
            nBarraSup = build_ele.dadesTDVert.value[j].BarraSuperior[-1] + nBarraSup
            nBarraSup = int(nBarraSup)
        elif len(build_ele.dadesTDVert.value[j].BarraSuperior) == 7:
            nBarraSup = build_ele.dadesTDVert.value[j].BarraSuperior[-3]
            nBarraSup = build_ele.dadesTDVert.value[j].BarraSuperior[-2] + nBarraSup
            nBarraSup = build_ele.dadesTDVert.value[j].BarraSuperior[-1] + nBarraSup
            nBarraSup = int(nBarraSup)
        posicoFinal = build_ele.BarresHorList.value[nBarraSup].Posicio
        alturaSup = build_ele.dadesTDHortInter.value[nBarraSup].Altura


    if build_ele.dadesTDVert.value[j].BarraInferior != "EN Inferior":
        nBarraInf = 0
        if len(build_ele.dadesTDVert.value[j].BarraInferior) == 5:
            nBarraInf = build_ele.dadesTDVert.value[j].BarraInferior[-1]
            nBarraInf = int(nBarraInf)
        elif len(build_ele.dadesTDVert.value[j].BarraInferior) == 6:
            nBarraInf = build_ele.dadesTDVert.value[j].BarraInferior[-2]
            nBarraInf = build_ele.dadesTDVert.value[j].BarraInferior[-1] + nBarraInf
            nBarraInf = int(nBarraInf)
        elif len(build_ele.dadesTDVert.value[j].BarraInferior) == 7:
            nBarraInf = build_ele.dadesTDVert.value[j].BarraInferior[-3]
            nBarraInf = build_ele.dadesTDVert.value[j].BarraInferior[-2] + nBarraInf
            nBarraInf = build_ele.dadesTDVert.value[j].BarraInferior[-1] + nBarraInf
            nBarraInf = int(nBarraInf)
        posicioInicial = build_ele.BarresHorList.value[nBarraInf].Posicio
        alturaInf = build_ele.dadesTDHortInter.value[nBarraInf].Altura


        if not build_ele.dadesTDVert.value[j].EncaixInf:
            sumatori += build_ele.dadesTDHortInter.value[nBarraInf].Altura

        orientacioInf = "Dre"
        try:
            if build_ele.dadesTDVert.value[j].invertirEncaixInf:
                orientacioInf = "Esq"
        except Exception as e:
            build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(invertirEncaixInf = build_ele.invertirEncaixVertInf.value)
            print("valor invertirEncaixInf no guardat a la barra: " + str(j))
        if build_ele.dadesTDVert.value[j].invertirEncaixInf:
            orientacioInf = "Esq"

        posicio =  build_ele.dadesTDHortInter.value[nBarraInf].Altura/2
        if orientacioInf == "Dre":
            Profunditat = build_ele.dadesTDHortInter.value[nBarraInf].desplY + build_ele.dadesTDHortInter.value[nBarraInf].Ample + (-build_ele.listDesplVerticals.value[j].desplYAbs) +1
        else:
            Profunditat = build_ele.dadesTDHortInter.value[nBarraInf].desplY + build_ele.dadesTDHortInter.value[nBarraInf].Ample + (-build_ele.listDesplVerticals.value[j].desplYAbs) +1
            #Profunditat = build_ele.dadesTDVert.value[j].BarraAltura - Profunditat
        ...
        bob = ENCollection( BarraFront = not build_ele.dadesTDVert.value[j].EncaixInf,
                            EncaixOr = orientacioInf,
                            Longitud = alturaInf,
                            Amplitud = build_ele.dadesTDVert.value[j].BarraAmple,
    ...
    if  build_ele.dadesTDVert.value[j].BarraInferior == "EN Inferior":
        ...
        Profunditat =  build_ele.desplYS.value + (-build_ele.listDesplVerticals.value[j].desplYAbs) +1
    else:
        Profunditat = build_ele.dadesTDHortInter.value[nBarraSup].desplY + build_ele.dadesTDHortInter.value[nBarraSup].Ample + (-build_ele.listDesplVerticals.value[j].desplYAbs) +1

    if build_ele.dadesTDVert.value[j].EncaixInf and not build_ele.dadesTDVert.value[j].EncaixSup and build_ele.dadesTDVert.value[j].BarraInferior != "EN Inferior":
        posicio -= alturaInf
    ...
    try:
        if build_ele.dadesTDVert.value[j].invertirEncaixSup:
            orientacioSup = "Esq"
    except Exception as e:
        build_ele.dadesTDVert.value[j] = build_ele.dadesTDVert.value[j]._replace(invertirEncaixSup = build_ele.invertirEncaixVertSup.value)
    ...
    if build_ele.dadesTDVert.value[j].BarraSuperior == "EN Superior":
        if orientacioSup == "Esq":
            ...
            Profunditat = build_ele.desplYS.value + (-build_ele.listDesplVerticals.value[j].desplYAbs) + ampleSup
    ENCollection = collections.namedtuple('StirrupList', 'BarraFront EncaixOr Longitud Amplitud Posicio Profunditat Pestanya Separator')
    bob = ENCollection( BarraFront = not build_ele.dadesTDVert.value[j].EncaixSup,
    ...
    if (j == 0 or j == build_ele.IntegerTDSelector.value-1) and ( build_ele.TubSuperior.value == "TUB Interior"):
        orientSupA = "Inf"
        ...
        bob = TDHorCollection(Femella = build_ele.MostrarTDHoritzontalSup.value,
                            ...
                            PosFemellaY = build_ele.desplYS.value - build_ele.listDesplVerticals.value[j].desplYAbs + AmpleSup/2 - AmpleSup/4,
...
def createBoxForat(build_ele, i, forats, nForat):
    ...
    nBoxForat = BoxForat(random.random() * 3600, build_ele.dadesTDVert.value[i].BarraAmple, build_ele.dadesTDVert.value[i].BarraAltura, build_ele.BarraLlargadaVert.value, build_ele.BarraGruix.value,
    ...
    vectorCan = AllplanGeo.Matrix3D()
    vectorCan.SetValue(12, build_ele.listDesplVerticals.value[i].desplXAbs - (build_ele.dadesTDVert.value[i].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2) )
    vectorCan.SetValue(13, build_ele.listDesplVerticals.value[i].desplYAbs)
    vectorCan.SetValue(14, 0)#barrainici pos
    nHorInf = 0
    if build_ele.dadesTDVert.value[i].BarraInferior != "EN Inferior":
        if len(build_ele.dadesTDVert.value[i].BarraInferior) == 7:
            nHorInfCentenes = int(build_ele.dadesTDVert.value[i].BarraInferior[-3])
            nHorInfDecenes = int(build_ele.dadesTDVert.value[i].BarraInferior[-2])
            nHorInfUnitats = int(build_ele.dadesTDVert.value[i].BarraInferior[-1])
            nHorInf = nHorInfCentenes*100 + nHorInfDecenes*10 + nHorInfUnitats
        elif len(build_ele.dadesTDVert.value[i].BarraInferior) == 6:
            nHorInfDecenes = int(build_ele.dadesTDVert.value[i].BarraInferior[-2])
            nHorInfUnitats = int(build_ele.dadesTDVert.value[i].BarraInferior[-1])
            nHorInf = nHorInfDecenes*10 + nHorInfUnitats
        elif len(build_ele.dadesTDVert.value[i].BarraInferior) == 5:
            nHorInf = int(build_ele.dadesTDVert.value[i].BarraInferior[-1])

        if not build_ele.dadesTDVert.value[i].EncaixInf:
            posHorInf = build_ele.BarresHorList.value[nHorInf].Posicio - build_ele.dadesTDHortInter.value[nHorInf].Altura/2
        else:
...
def createBoxForatHorInt(build_ele, posBarraHor, forats, i, nForats):
    ...
    nBoxForat = BoxForat(random.random() * 3600, build_ele.dadesTDHortInter.value[posBarraHor].Ample, build_ele.dadesTDHortInter.value[posBarraHor].Altura, build_ele.BarraLlargadaVert.value, build_ele.BarraGruix.value,
    ...
    vectorCan.SetValue(12, build_ele.listDesplVerticals.value[i].desplXAbs + build_ele.dadesTDVert.value[i].BarraAmple/2 + build_ele.dadesTDVert.value[0].BarraAmple/2 -build_ele.BarraGruix.value + 2 )#+ build_ele.ForatsHorInt.value[posBarraHor].Posicio)#- build_ele.dadesTDHortInter.value[posBarraHor].desplX)
    #vectorCan.SetValue(13, build_ele.dadesTDVert.value[i].desplYAbs)
    vectorCan.SetValue(13, build_ele.dadesTDHortInter.value[posBarraHor].desplY)
    #vectorCan.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio)
    if not build_ele.EncaixHorInf.value:
        ...
        #vectorCan.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio - build_ele.ForatsHorInt.value[posBarraHor].Posicio)
        vectorCan.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio + build_ele.BarraAlturaInf.value - build_ele.dadesTDHortInter.value[posBarraHor].Altura/2)
    else:
        vectorCan.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio - build_ele.dadesTDHortInter.value[posBarraHor].Altura/2)
        #vectorCan.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio - build_ele.ForatsHorInt.value[posBarraHor].Posicio)
        vectorCan.SetValue(14, build_ele.BarresHorList.value[posBarraHor].Posicio + build_ele.BarraAlturaInf.value/2 - build_ele.dadesTDHortInter.value[posBarraHor].Altura)
...
def set_all_edit_to_false(build_ele):
    ...
    for nbarra in range(0,len(build_ele.BarresHorListToShow.value)):
        if(build_ele.BarresHorListToShow.value[nbarra].Edit):
            build_ele.BarresHorListToShow.value[nbarra] = build_ele.BarresHorListToShow.value[nbarra]._replace(Edit = False)
    ...
    for nbarra in range(0, len(build_ele.BarresHorList.value)):
        if nbarra< len(build_ele.BarresHorList.value):
            if(build_ele.BarresHorList.value[nbarra].Edit) or build_ele.BarresHorList.value[nbarra].acabatEditar:
                build_ele.BarresHorList.value[nbarra] = build_ele.BarresHorList.value[nbarra]._replace(Edit = False)
...
def guardar_valors_inici(build_ele, nVer):
    ...
    if build_ele.IntegerTDSelectorAnterior.value == -1:
        ...
        if len(build_ele.dadesTDVert.value) >= 2:
            build_ele.dadesTDVert.value[1] = build_ele.dadesTDVert.value[1]._replace(MostrarTDVertical = False)
    else:
        #if (build_ele.SelectorTDVAnt.value != nVer or (build_ele.SelectorPPAnt.value != build_ele.SelectorPP.value)):
        if build_ele.IntegerTDSelectorAnterior.value != nVer or (build_ele.SelectorPPAnt.value != build_ele.SelectorPP.value):
...
def crear_barres_inferiors_multiples(build_ele, nBarraInf, mostrarActual, doc):
    ...
    #llargadaInf = build_ele.BarraLlargada.value + build_ele.ExtenderInf.value
    llargadaInf = build_ele.listDesplHoritzontalsInf.value[nBarraInf].BarraLlargadaInf
    #if build_ele.SepararTDHoritzontalInf.value:
        #build_ele.BarraLlargada.value = build_ele.listDesplHoritzontalsInf.value[0].BarraLlargadaInf
        #llargadaInf = build_ele.listDesplHoritzontalsInf.value[nBarraInf].BarraLlargadaInf
        #build_ele.BarraAmpleInf.value = build_ele.listDesplHoritzontalsInf.value[nBarraInf].BarraAmpleInf
        #build_ele.BarraAlturaInf.value = build_ele.listDesplHoritzontalsInf.value[nBarraInf].BarraAlturaInf

    #if nBarraInf>= len(build_ele.listDesplHoritzontalsInf.value):


    '''
    horitzontalPPInf = PP_EN_Horitzontal(random.random() * 3600,build_ele.DistanciaEntreTD.value, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value, llargadaInf, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                build_ele.IsUseGlobalProp.value, build_ele.FounColor.value, build_ele.BarraLayer.value,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                build_ele.ColisParINF.value, build_ele.PotaParINF.value, build_ele.ForatsParINF.value, #ColisPar, PotaPar, #matrius
                                build_ele.Ample_forat_femellaINF.value, build_ele.Altura_forat_femellaINF.value, build_ele.Separacio_forat_femellaINF.value,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
    ...
    if build_ele.TubInferior.value == "L":
        horitzontalPPInf = PP_EN_Horitzontal_Inf(random.random() * 3600, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value, llargadaInf, build_ele.BarraGruix.value, build_ele.InvertirENHoritzontalInf.value, False,#Randomz, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                build_ele.IsUseGlobalProp.value, vermellInf, build_ele.BarraLayer.value,
                                retallInici= build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplX,
                                Encaixos = build_ele.EncaixosParINF.value) #EncaixosPar)
    else:
        horitzontalPPInf = PP_EN_Horitzontal(random.random() * 3600,build_ele.DistanciaEntreTD.value, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value, llargadaInf, build_ele.BarraGruix.value,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                build_ele.IsUseGlobalProp.value, vermellInf, build_ele.BarraLayer.value,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
    ...
    #    seccio = str(build_ele.BarraAmpleInf.value) + "x " + str(build_ele.BarraAlturaInf.value) +"x " +str(build_ele.BarraLlargada.value) + "x " + str(build_ele.BarraGruix.value)
    #else:
    #    seccio = str(build_ele.BarraAlturaInf.value) + "x " + str(build_ele.BarraAmpleInf.value) +"x " +str(build_ele.BarraLlargada.value)+ "x " + str(build_ele.BarraGruix.value)
    atr05 = "cara_X"
    if build_ele.InvertirENHoritzontalInf.value:
        atr05 = "cara_Y"
    if not build_ele.TubInferior.value == "L":
        ...
        if build_ele.invertirEncaixInf.value:
            ...
            for i in range(0,len(build_ele.listDesplVerticals.value)):
                ...
                if build_ele.dadesTDVert.value[i].EncaixInf:
                    ...
                    matrixOr.append("Inf")
            trans_list = horitzontalPPInf.create_PP_positions_femelles(listAmples, listAltures, matrixPosX, matrixPosY, matrixMostrarInf, offsetXI, matrixOr, build_ele.Ample_forat_femellaVert.value, build_ele.BarraLlargada.value, build_ele.desplYI.value, build_ele.FemellesHorIntAuxInf.value, build_ele.desplXI.value)#
        else:
            for i in range(0,len(build_ele.listDesplVerticals.value)):
                matrixOffsetY.append(  build_ele.listDesplVerticals.value[i].desplY + build_ele.dadesTDVert.value[i].BarraAltura - build_ele.desplYI.value)
            trans_list = horitzontalPPInf.create_PP_positions_encaix(listAmples2, matrixPosX, matrixPosY, matrixcentrar, matrixMostrar, matrixOffsetY, offsetXI, 'Sup', build_ele.BarraLlargada.value)
    ...
    vectorH1Mult.SetValue(12, build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplX )#+ build_ele.dadesTDVert.value[0].BarraAmple)
    vectorH1Mult.SetValue(13, build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplY)
    ...
    if build_ele.SepararTDHoritzontalInf.value:
        ...
        horitzontalPPOBject = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value+10, llargadaInf, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                    False, common_propsObject.Color, common_propsObject.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                    [], [], [],#ColisPar, PotaPar, #matrius
                                    0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                    [], #Femelles, #matriu
        ...
        horitzontalPPOBjectRecess = PP_EN_Horitzontal(random.random() * 3600,0, build_ele.BarraAmpleInf.value, build_ele.BarraAlturaInf.value+10, llargadaInf, 0,#DistanciaEntreTD, BarraAmple, BarraAltura,BarraLlargada, BarraGruix,
                                    False, common_propsObjectRecess.Color, common_propsObjectRecess.Layer,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                    [], [], [],#ColisPar, PotaPar, #matrius
                                    0, 0, 0,#Ample_forat_femella, Altura_forat_femella, Separacio_forat_femella,
                                    [], #Femelles, #matriu
        ...
        if build_ele.listDesplHoritzontalsInf.value[nBarraInf].mostrarLiniaA:
            punt_central = build_ele.desplXI.value + build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplX + build_ele.BarraAlturaInf.value/2
            pointUbi = AllplanGeo.Point3D(vectorH1Mult[12], build_ele.desplYI.value + build_ele.BarraAmpleInf.value/2 + build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplLinA,  build_ele.BarraAlturaInf.value/2)
            if (len(build_ele.SelectorLiniaInf.value)) <=1:
                build_ele.SelectorLiniaInf.value = "Tipus 1"
            linia = build_ele.SelectorLiniaInf.value[len(build_ele.SelectorLiniaInf.value)-1]
        ...
        if build_ele.listDesplHoritzontalsInf.value[nBarraInf].mostrarLiniaB:
            punt_central = build_ele.desplXI.value + build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplX + build_ele.BarraAlturaInf.value/2
            pointUbi = AllplanGeo.Point3D(vectorH1Mult[12], build_ele.desplYI.value + build_ele.BarraAmpleInf.value/2 + build_ele.listDesplHoritzontalsInf.value[nBarraInf].desplLinB, build_ele.BarraAlturaInf.value/2)
            linia = build_ele.SelectorLiniaInf.value[len(build_ele.SelectorLiniaInf.value)-1]
            layer = build_ele.SelectorLayerInf.value[len(build_ele.SelectorLayerInf.value)-1]
            polyhedronCentral = create_polyline_interior(build_ele, punt_central,build_ele.BarraAmpleInf.value,build_ele.listDesplHoritzontalsInf.value[nBarraInf].BarraLlargadaInf, True, pointUbi, int(linia), layer)
            if polyhedronCentral != []:
...
def crearLlistaVerticals(build_ele):
    ...
    for barra in range(build_ele.IntegerTDSelector.value, len(build_ele.valueListBarresComboBox.value)):
...
def crearLlistaReforc(build_ele):
    for barra in range(0, build_ele.NumBarresRef.value):
        if barra >= len(build_ele.valueListReforcComboBox.value):
            build_ele.valueListReforcComboBox.value.append("Varifix" + str(barra))
        else:
            build_ele.valueListReforcComboBox.value[barra] = "Varifix " + str(barra)
...
def crearLlistaHoritzontals(build_ele):
    ...
    if len(build_ele.valueVertListBarresHorSup.value) == 0:
        build_ele.valueVertListBarresHorSup.value.append("EN Superior")
    else:
        build_ele.valueVertListBarresHorSup.value[0] = "EN Superior"

    if len(build_ele.valueVertListBarresHorInf.value) == 0:
        build_ele.valueVertListBarresHorInf.value.append("EN Inferior")
    else:
        build_ele.valueVertListBarresHorInf.value[0] = "EN Inferior"
    barraVert = 0
    barraVertAux = 0
    Afegits = 1
...
def crearLlistaFullHoritzontals(build_ele):
    ...
    if len(build_ele.valueListBarresHorBalc.value) == 0:
        build_ele.valueListBarresHorBalc.value.append("EN Superior")
    else:
        build_ele.valueListBarresHorBalc.value[0] = "EN Superior"

    if len(build_ele.valueListBarresHorBalc.value) == 1:
        build_ele.valueListBarresHorBalc.value.append("EN Inferior")
    else:
        build_ele.valueListBarresHorBalc.value[1] = "EN Inferior"
    barraVert = 0
    barraVertAux = 0
    Afegits = 2
    for barra in range(0, len(build_ele.BarresHorList.value)):
...
def copiar_valors_Vert(build_ele,nVer):
    ...
    if len(build_ele.DadesVertCopy.value) == 0:
        ...
        bob = CopyDadesCollection(nBarraGuardada = nVer,
                                  BarraAmple = build_ele.dadesTDVert.value[nVer].BarraAmple,
                                  BarraAltura = build_ele.dadesTDVert.value[nVer].BarraAltura,
                                  BarraLlargada = build_ele.dadesTDVert.value[nVer].BarraAlcada,
                                  desplY = build_ele.listDesplVerticals.value[nVer].desplYAbs,
                                  esProvisional= build_ele.dadesTDVert.value[nVer].esProvisional,
                                  LlargadaAut= build_ele.dadesTDVert.value[nVer].LlargadaAut,
                                  TreureEncaixSup = build_ele.dadesTDVert.value[nVer].EncaixSup,
                                  TreureEncaixInf = build_ele.dadesTDVert.value[nVer].EncaixInf,
                                  invertirEncaixSup = build_ele.dadesTDVert.value[nVer].invertirEncaixSup,
                                  invertirEncaixInf = build_ele.dadesTDVert.value[nVer].invertirEncaixInf,
                                  FemellaSup = build_ele.dadesTDVert.value[nVer].FemellaSup,
                                  FemellaInf = build_ele.dadesTDVert.value[nVer].FemellaInf,
                                  Separator = '')
        ...
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(BarraAmple = build_ele.dadesTDVert.value[nVer].BarraAmple)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(BarraAltura = build_ele.dadesTDVert.value[nVer].BarraAltura)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(BarraLlargada = build_ele.dadesTDVert.value[nVer].BarraAlcada)

        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(desplY = build_ele.listDesplVerticals.value[nVer].desplYAbs)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(esProvisional = build_ele.dadesTDVert.value[nVer].esProvisional)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(LlargadaAut = build_ele.dadesTDVert.value[nVer].LlargadaAut)

        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(TreureEncaixSup = build_ele.dadesTDVert.value[nVer].EncaixSup)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(TreureEncaixInf = build_ele.dadesTDVert.value[nVer].EncaixInf)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(FemellaSup = build_ele.dadesTDVert.value[nVer].FemellaSup)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(FemellaInf = build_ele.dadesTDVert.value[nVer].FemellaInf)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(invertirEncaixSup = build_ele.dadesTDVert.value[nVer].invertirEncaixSup)
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(invertirEncaixInf = build_ele.dadesTDVert.value[nVer].invertirEncaixInf)
...
def pegar_valors_Vert(build_ele,nVer):
    ...
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(BarraAmple = build_ele.DadesVertCopy.value[0].BarraAmple)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(BarraAltura = build_ele.DadesVertCopy.value[0].BarraAltura)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(BarraAlcada = build_ele.DadesVertCopy.value[0].BarraLlargada)

    build_ele.listDesplVerticals.value[nVer] = build_ele.listDesplVerticals.value[nVer]._replace(desplYAbs = build_ele.DadesVertCopy.value[0].desplY)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(esProvisional = build_ele.DadesVertCopy.value[0].esProvisional)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(LlargadaAut = build_ele.DadesVertCopy.value[0].LlargadaAut)

    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(EncaixSup = build_ele.DadesVertCopy.value[0].TreureEncaixSup)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(EncaixInf = build_ele.DadesVertCopy.value[0].TreureEncaixInf)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(invertirEncaixSup = build_ele.DadesVertCopy.value[0].invertirEncaixSup)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(invertirEncaixInf = build_ele.DadesVertCopy.value[0].invertirEncaixInf)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(FemellaSup = build_ele.DadesVertCopy.value[0].FemellaSup)
    build_ele.dadesTDVert.value[nVer] = build_ele.dadesTDVert.value[nVer]._replace(FemellaInf = build_ele.DadesVertCopy.value[0].FemellaInf)
    ...
    for nVerA in  range(nVer*10, nVer*10+10 ):
        ...
        build_ele.ForatsVertList.value[nVerA] = build_ele.ForatsVertList.value[nVerA]._replace(MostrarBox = False)
...
def crear_LProvisionals(build_ele,j, vectorH1):
    Lprovisionals = LProvisional(random.random() * 3600, build_ele.dadesTDVert.value[j].BarraAmple + 2, build_ele.dadesTDVert.value[j].BarraAltura, build_ele.dadesTDVert.value[j].BarraAlcada - 76, build_ele.dadesTDVert.value[j].Gruix,
    ...
    vectorH1A.SetValue(13, vectorH1[13] + build_ele.dadesTDVert.value[j].BarraAltura/2 - 17.5/2)
...
def create_num_on_view(build_ele, j, vectorH1, llargadaAut):
    numVert = TextSpline(random.random() * 3600, build_ele.dadesTDVert.value[j].BarraAmple + 2, build_ele.dadesTDVert.value[j].BarraAltura, build_ele.dadesTDVert.value[j].BarraAlcada - 30, build_ele.dadesTDVert.value[j].Gruix,
    ...
    vectorH1B.SetValue(12, vectorH1[12] - build_ele.dadesTDVert.value[j].BarraAmple  - (20*build_ele.tamanyNumId.value))
    vectorH1B.SetValue(13, vectorH1[13]  )
    #vectorH1B.SetValue(14, build_ele.dadesTDVert.value[j].BarraAlcada + vectorH1[14] )
    vectorH1B.SetValue(14, llargadaAut  + vectorH1[14])
    if j == 0:
        vectorH1B.SetValue(14, build_ele.dadesTDVert.value[j].BarraAlcada - 100 )
...
def create_num_on_view_ref(build_ele, j, vectorH1):
    numVert = TextSpline(random.random() * 3600, build_ele.dadesENReftInter.value[j].Ample + 2, build_ele.dadesENReftInter.value[j].Altura, build_ele.dadesENReftInter.value[j].Llargada - 30, build_ele.dadesTDVert.value[j].Gruix,
    ...
    vectorH1B.SetValue(13, vectorH1[13]  )
    vectorH1B.SetValue(14, build_ele.BarresRefList.value[j].Posicio + build_ele.dadesENReftInter.value[j].Altura + 10 )
    if j == 0:
        vectorH1B.SetValue(14, build_ele.BarresRefList.value[j].Posicio - 100 )
...
def buscar_anterior_mes_proper(build_ele, posicioX):
    ...
    #for nBarra in range(1,len(build_ele.listDesplVerticals.value)):
    for nBarra in range(1,build_ele.IntegerTDSelector.value):
        if build_ele.listDesplVerticals.value[nBarra].desplXAbs < posicioX and build_ele.listDesplVerticals.value[nBarra].desplXAbs >= posicioAntMesPropera and build_ele.dadesTDVert.value[nBarra].MostrarTDVertical and not build_ele.dadesTDVert.value[nBarra].esProvisional and  build_ele.dadesTDVert.value[nBarra].BarraSuperior == "EN Superior" and  build_ele.dadesTDVert.value[nBarra].BarraInferior == "EN Inferior" :
            posicioAntMesPropera = build_ele.listDesplVerticals.value[nBarra].desplXAbs #+ 80
            trobatAnt = True
...
def buscar_seguent_mes_proper(build_ele, posicioX):
    ...
    trobatSeg = False
    posNSegMesPropera = 1
    #posicioSegMesPropera = build_ele.BarraLlargada.value + build_ele.DistanciaEntreTD.value
    posicioSegMesPropera = (build_ele.IntegerTDSelector.value-1) * build_ele.DistanciaEntreTD.value
    posicioSegMesPropera = build_ele.listDesplVerticals.value[build_ele.IntegerTDSelector.value-1].desplXAbs

    #for nBarra in range(1,len(build_ele.listDesplVerticals.value)):
    for nBarra in range(1,build_ele.IntegerTDSelector.value-1):
        if build_ele.listDesplVerticals.value[nBarra].desplXAbs > posicioX and build_ele.listDesplVerticals.value[nBarra].desplXAbs <= posicioSegMesPropera and build_ele.dadesTDVert.value[nBarra].MostrarTDVertical and not build_ele.dadesTDVert.value[nBarra].esProvisional and  build_ele.dadesTDVert.value[nBarra].BarraSuperior == "EN Superior" and  build_ele.dadesTDVert.value[nBarra].BarraInferior == "EN Inferior" :
            posicioSegMesPropera = build_ele.listDesplVerticals.value[nBarra].desplXAbs
            trobatSeg = True
...
def mostrar_BalcFine(build_ele, nBalcFin):
    ...
    if build_ele.dadesBalcFines.value[nBalcFin].AmpleBalconera == 0:
        ...
        build_ele.dadesBalcFines.value[nBalcFin] = build_ele.dadesBalcFines.value[nBalcFin]._replace(AmpleBalconera = 600)
        build_ele.dadesBalcFines.value[nBalcFin] = build_ele.dadesBalcFines.value[nBalcFin]._replace(LlargadaBalconera = 1000)
        build_ele.dadesBalcFines.value[nBalcFin] = build_ele.dadesBalcFines.value[nBalcFin]._replace(PosicioXBalconera = 200)
        build_ele.dadesBalcFines.value[nBalcFin] = build_ele.dadesBalcFines.value[nBalcFin]._replace(PosicioZBalconera = 60)
        build_ele.dadesBalcFines.value[nBalcFin] = build_ele.dadesBalcFines.value[nBalcFin]._replace(BarraHorSup = "Tub 0")
        build_ele.dadesBalcFines.value[nBalcFin] = build_ele.dadesBalcFines.value[nBalcFin]._replace(BarraHorInf = "EN Inferior")
...
def Encaixos_hor_amb_verticals(build_ele):
    listEncaixAdj = []
    for nVert in range(0,build_ele.IntegerTDSelector.value):
        prof = build_ele.dadesTDVert.value[nVert].BarraAlcada - build_ele.desplZS.value + build_ele.BarraAlturaInf.value #- build_ele.BarraAlturaSup.value/2
        if build_ele.dadesTDVert.value[nVert].BarraSuperior == "EN Superior":

            if not build_ele.dadesTDVert.value[nVert].LlargadaAut:
                prof = build_ele.dadesTDVert.value[nVert].BarraAlcada - build_ele.desplZS.value #+ build_ele.BarraAlturaSup.value/2
                #BarraFront = (build_ele.desplZS.value  - build_ele.BarraAlturaSup.value/2) < build_ele.dadesTDVert.value[nVert].BarraAlcada and build_ele.dadesTDVert.value[nVert].MostrarTDVertical and  build_ele.dadesTDVert.value[nVert].EncaixSup #and coincideix
                BarraFront = (build_ele.desplZS.value  - build_ele.BarraAlturaSup.value/2) <= build_ele.dadesTDVert.value[nVert].BarraAlcada and build_ele.dadesTDVert.value[nVert].MostrarTDVertical and  build_ele.dadesTDVert.value[nVert].EncaixSup #and coincideix
            else:
                prof = -1
                BarraFront = False
                #if nVert == 0 or nVert == build_ele.IntegerTDSelector.value-1:
                #    prof = 8
                #    BarraFront = True
            if prof == 0 and build_ele.dadesTDVert.value[nVert].EncaixSup:
                BarraFront = True

            if nVert == 0 :
                posicio = build_ele.listDesplVerticals.value[nVert].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple/2
            elif nVert == build_ele.IntegerTDSelector.value-1:
                posicio = build_ele.BarraLlargada.value - build_ele.dadesTDVert.value[nVert].BarraAmple/2
            else:
                posicio = build_ele.listDesplVerticals.value[nVert].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple/2 + build_ele.dadesTDVert.value[nVert].BarraAmple

            barraLlargada = build_ele.BarraLlargada.value
            if build_ele.TubSuperior.value == "TUB Interior":
                barraLlargada -= build_ele.dadesTDVert.value[0].BarraAmple
                #posicio -=  build_ele.dadesTDVert.value[nVert].BarraAmple
                posicio = build_ele.listDesplVerticals.value[nVert].desplXAbs + (build_ele.dadesTDVert.value[nVert].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2 ) + build_ele.dadesTDVert.value[nVert].BarraAmple/2
                if nVert == 0:
                    posicio -= build_ele.dadesTDVert.value[0].BarraAmple
                if posicio > barraLlargada - build_ele.dadesTDVert.value[0].BarraAmple - build_ele.dadesTDVert.value[build_ele.IntegerTDSelector.value-1].BarraAmple:
                    BarraFront = False
            if posicio < 0 :
                BarraFront = False
            ...
            bob = ENCollection( Encaix = BarraFront,#BarraFront = BarraFront,
                                EncaixOr = "Dre",
                                Longitud = build_ele.dadesTDVert.value[nVert].BarraAmple,
                                Amplitud = build_ele.dadesTDVert.value[nVert].BarraAltura + build_ele.listDesplVerticals.value[nVert].desplYAbs,
                                Posicio = posicio,
                                Profunditat = prof +1 ,
                                Pestanya = False,
                                Separator = '')
        ...
        '''
            ...
            #if build_ele.dadesTDVert.value[nVert].BarraSuperior != "EN Superior":
            if len(build_ele.dadesTDVert.value[nVert].BarraSuperior) == 5:
                nBarraSup = build_ele.dadesTDVert.value[nVert].BarraSuperior[-1]
                nBarraSup = int(nBarraSup)
            elif len(build_ele.dadesTDVert.value[nVert].BarraSuperior) == 6:
                nBarraSup = build_ele.dadesTDVert.value[nVert].BarraSuperior[-2]
                nBarraSup = build_ele.dadesTDVert.value[nVert].BarraSuperior[-1] + nBarraSup
                nBarraSup = int(nBarraSup)
            elif len(build_ele.dadesTDVert.value[nVert].BarraSuperior) == 7:
                nBarraSup = build_ele.dadesTDVert.value[nVert].BarraSuperior[-3]
                nBarraSup = build_ele.dadesTDVert.value[nVert].BarraSuperior[-2] + nBarraSup
                nBarraSup = build_ele.dadesTDVert.value[nVert].BarraSuperior[-1] + nBarraSup
                nBarraSup = int(nBarraSup)
            #posicoFinal = build_ele.BarresHorList.value[nBarraSup].Posicio - build_ele.dadesTDHortInter.value[nBarraSup].Altura/2
            """
            if build_ele.dadesTDVert.value[nVert].BarraInferior != "EN Inferior":
                if len(build_ele.dadesTDVert.value[nVert].BarraInferior) == 5:
                    nBarraInf = build_ele.dadesTDVert.value[nVert].BarraInferior[-1]
                    nBarraInf = int(nBarraInf)
                elif len(build_ele.dadesTDVert.value[nVert].BarraInferior) == 6:
                    nBarraInf = build_ele.dadesTDVert.value[nVert].BarraInferior[-2]
                    nBarraInf = build_ele.dadesTDVert.value[nVert].BarraInferior[-1] + nBarraInf
                    nBarraInf = int(nBarraInf)
                elif len(build_ele.dadesTDVert.value[nVert].BarraInferior) == 7:
                    nBarraInf = build_ele.dadesTDVert.value[nVert].BarraInferior[-3]
                    nBarraInf = build_ele.dadesTDVert.value[nVert].BarraInferior[-2] + nBarraInf
                    nBarraInf = build_ele.dadesTDVert.value[nVert].BarraInferior[-1] + nBarraInf
                    nBarraInf = int(nBarraInf)
                posicioInicial = build_ele.BarresHorList.value[nBarraInf].Posicio
            """
            ...
            if not build_ele.dadesTDVert.value[nVert].LlargadaAut:
                prof = build_ele.dadesTDVert.value[nVert].BarraAlcada - build_ele.BarresHorList.value[nBarraSup].Posicio + build_ele.dadesTDHortInter.value[nBarraSup].Altura/2
                BarraFront = (build_ele.BarresHorList.value[nBarraSup].Posicio  - build_ele.dadesTDHortInter.value[nBarraSup].Altura/2) < build_ele.dadesTDVert.value[nVert].BarraAlcada and build_ele.dadesTDVert.value[nVert].MostrarTDVertical and not build_ele.dadesTDVert.value[nVert].EncaixSup #and coincideix
                BarraFront = (build_ele.BarresHorList.value[nBarraSup].Posicio  - build_ele.dadesTDHortInter.value[nBarraSup].Altura/2) <= build_ele.dadesTDVert.value[nVert].BarraAlcada and build_ele.dadesTDVert.value[nVert].MostrarTDVertical and not build_ele.dadesTDVert.value[nVert].EncaixSup #and coincideix
            else:
                prof = -1
                BarraFront = False
...
def mostrar_valors_Ref(build_ele, nBarraVert):
    ...
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(BarraRef = build_ele.BarresRefList.value[nBarraVert].BarraRef)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(Orientacio = build_ele.BarresRefList.value[nBarraVert].Orientacio)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(Posicio = build_ele.BarresRefList.value[nBarraVert].Posicio)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(BarraIni = build_ele.BarresRefList.value[nBarraVert].BarraIni)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(BarraFin = build_ele.BarresRefList.value[nBarraVert].BarraFin)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(AutoLongitud = build_ele.BarresRefList.value[nBarraVert].AutoLongitud)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(Longitud = build_ele.BarresRefList.value[nBarraVert].Longitud)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(Edit = build_ele.BarresRefList.value[nBarraVert].Edit)
    build_ele.BarresRefListToShow.value[0] = build_ele.BarresRefListToShow.value[0]._replace(acabatEditar = build_ele.BarresRefList.value[nBarraVert].acabatEditar)


    build_ele.dadesENReftInter.value[nBarraVert ] = build_ele.dadesENReftInter.value[nBarraVert ]._replace(estaEditant = build_ele.BarresRefListToShow.value[0].Edit)
    ...
    build_ele.ForatsVertListToShow.value[0] = build_ele.ForatsRefInt.value[nBarraVert *3 + 0]
    build_ele.ForatsVertListToShow.value[1] = build_ele.ForatsRefInt.value[nBarraVert *3 + 1]
    build_ele.ForatsVertListToShow.value[2] = build_ele.ForatsRefInt.value[nBarraVert *3 + 2]
    build_ele.ForatsVertListToShow.value[3] = build_ele.ForatsRefInt.value[nBarraVert *3 + 3]
    build_ele.ForatsVertListToShow.value[4] = build_ele.ForatsRefInt.value[nBarraVert *3 + 4]
    build_ele.ForatsVertListToShow.value[5] = build_ele.ForatsRefInt.value[nBarraVert *3 + 5]
    build_ele.ForatsVertListToShow.value[6] = build_ele.ForatsRefInt.value[nBarraVert *3 + 6]
    build_ele.ForatsVertListToShow.value[7] = build_ele.ForatsRefInt.value[nBarraVert *3 + 7]
...
def guardar_valors_Ref(build_ele, nBarraVert):
    ...
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(BarraRef = build_ele.BarresRefListToShow.value[0].BarraRef)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(Orientacio = build_ele.BarresRefListToShow.value[0].Orientacio)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(Posicio = build_ele.BarresRefListToShow.value[0].Posicio)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(BarraIni = build_ele.BarresRefListToShow.value[0].BarraIni)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(BarraFin = build_ele.BarresRefListToShow.value[0].BarraFin)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(AutoLongitud = build_ele.BarresRefListToShow.value[0].AutoLongitud)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(Longitud = build_ele.BarresRefListToShow.value[0].Longitud)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(Edit = build_ele.BarresRefListToShow.value[0].Edit)
    build_ele.BarresRefList.value[nBarraVert] = build_ele.BarresRefList.value[nBarraVert]._replace(acabatEditar = build_ele.BarresRefListToShow.value[0].acabatEditar)
    ...
    build_ele.ForatsRefInt.value[nBarraVert *3 + 0] = build_ele.ForatsVertListToShow.value[0]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 1] = build_ele.ForatsVertListToShow.value[1]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 2] = build_ele.ForatsVertListToShow.value[2]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 3] = build_ele.ForatsVertListToShow.value[3]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 4] = build_ele.ForatsVertListToShow.value[4]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 5] = build_ele.ForatsVertListToShow.value[5]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 6] = build_ele.ForatsVertListToShow.value[6]
    build_ele.ForatsRefInt.value[nBarraVert *3 + 7] = build_ele.ForatsVertListToShow.value[7]
...
def definirAtributPers(build_ele,n,tipusTub):
        ...
        if not build_ele.AtrPersAutomaticInf.value:
            ...
            if build_ele.TubInferior.value == "L":
                if build_ele.SepararTDHoritzontalInf.value:
                    ...
                    for i in range(0, len(build_ele.dadesTDVert.value) ):
                        if i == 0:
                            if build_ele.listDesplHoritzontalsInf.value[n].BarraLlargadaInf == build_ele.dadesTDVert.value[i].BarraAmple and 0 == build_ele.listDesplHoritzontalsInf.value[n].desplX:
                                inferiorSota1Verical = True
                        elif i == build_ele.IntegerTDSelector.value -1:
                            if build_ele.listDesplHoritzontalsInf.value[n].BarraLlargadaInf == build_ele.dadesTDVert.value[i].BarraAmple and (build_ele.listDesplVerticals.value[i].desplXAbs ) == build_ele.listDesplHoritzontalsInf.value[n].desplX - build_ele.dadesTDVert.value[0].BarraAmple:
                                inferiorSota1Verical = True
                        else:
                            if build_ele.listDesplHoritzontalsInf.value[n].BarraLlargadaInf == build_ele.dadesTDVert.value[i].BarraAmple and (build_ele.listDesplVerticals.value[i].desplXAbs ) == build_ele.listDesplHoritzontalsInf.value[n].desplX - build_ele.dadesTDVert.value[0].BarraAmple:
                                inferiorSota1Verical = True
    ...
    if tipusTub == "TubHoritzontal":
        if not build_ele.dadesTDHortInter.value[n].AtrPersAutomatic:
            nomAtr = build_ele.dadesTDHortInter.value[n].AtributPersonTub
        else:
            if build_ele.dadesTDHortInter.value[n].Gruix >= build_ele.dadesTDHortInter.value[n].Ample/2 or build_ele.dadesTDHortInter.value[n].Gruix >= build_ele.dadesTDHortInter.value[n].Altura/2:#Xapa
                nomAtr = "XAPA FRONTAL"#11
            else :#build_ele.dadesTDHortInter.value[n].Gruix >= build_ele.dadesTDHortInter.value[n].Ample/2 or build_ele.dadesTDHortInter.value[n].Gruix >= build_ele.dadesTDHortInter.value[n].Altura/2:#Tub
                if build_ele.dadesTDHortInter.value[n].desplX != 0:
                    #comprovar llargada automatica? o desplX?
                    nomAtr = "TUB FRONTAL"#13
                else:
                    if not build_ele.BarresHorList.value[n].AutoLongitud and build_ele.dadesTDHortInter.value[n].desplX == 0 :
                        mateixFinal = False
                        for i in range(0, len(build_ele.dadesTDVert.value) ):
                            NBarraInici = 0
                            if len(build_ele.BarresHorList.value[n].BarraInici) == 5:
                                NBarraInici = int(build_ele.BarresHorList.value[n].BarraInici[-1])
                            elif len(build_ele.BarresHorList.value[n].BarraInici) == 6:
                                NBarraInici = int(build_ele.BarresHorList.value[n].BarraInici[-2])
                                NBarraInici = int(build_ele.BarresHorList.value[n].BarraInici[-1] + NBarraInici)
                            else:
                                NBarraInici = int(build_ele.BarresHorList.value[n].BarraInici[-3])
                                NBarraInici = int(build_ele.BarresHorList.value[n].BarraInici[-2] + NBarraInici)
                                NBarraInici = int(build_ele.BarresHorList.value[n].BarraInici[-1] + NBarraInici)

                            posfinalHor = build_ele.BarresHorList.value[n].Longitud + (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple)
                            if NBarraInici == 0:
                                posfinalHor = build_ele.BarresHorList.value[n].Longitud + (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple)
                            else:
                                posfinalHor = build_ele.dadesTDVert.value[0].BarraAmple + build_ele.BarresHorList.value[n].Longitud + (build_ele.listDesplVerticals.value[NBarraInici].desplXAbs + build_ele.dadesTDVert.value[NBarraInici].BarraAmple)

                            if NBarraInici == 0:
                                if build_ele.dadesTDVert.value[0].BarraAmple + build_ele.listDesplVerticals.value[i].desplXAbs + build_ele.dadesTDVert.value[i].BarraAmple == posfinalHor:
                                    mateixFinal = True
                            else:
                                if build_ele.dadesTDVert.value[0].BarraAmple + build_ele.listDesplVerticals.value[i].desplXAbs + build_ele.dadesTDVert.value[i].BarraAmple == posfinalHor:
                                    mateixFinal = True
                    ...
                    #for i in range(0, len(build_ele.dadesTDVert.value) ):
                    #if not build_ele.BarresHorList.value[n].AutoLongitud and build_ele.BarresHorList.value[n].Longitud
                        ...
                        for i in range(0, len(build_ele.dadesTDVert.value) ):
                            if build_ele.dadesTDVert.value[i].BarraInferior == "Tub " + str(n):
                                InferiorSota1Verical = True
                            if build_ele.dadesTDVert.value[i].BarraSuperior == "Tub " + str(n):
...
def getFemelles(build_ele, AmpleSup):
    femelles = []
    for nBarraVertical in range(0, build_ele.IntegerTDSelector.value):
        if build_ele.dadesTDVert.value[nBarraVertical].BarraSuperior == "EN Superior" :#Numeracio Malament #+ str(j*(posBarraHor - inici)):

            if build_ele.TubSuperior.value == "TUB":
                if nBarraVertical == 0:
                    pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs
                else:
                    pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs + build_ele.dadesTDVert.value[0].BarraAmple  #+ build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 #- build_ele.dadesTDVert.value[0].BarraAmple/2 ) #+ build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2
            elif build_ele.TubSuperior.value == "L" or build_ele.TubSuperior.value == "TUB Interior + L":
                if nBarraVertical == 0:
                    pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs - build_ele.desplXS.value - build_ele.dadesTDVert.value[0].Gruix - 1 - 2.25
                    #pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs +  build_ele.dadesTDVert.value[0].BarraAmple - build_ele.desplXS.value - build_ele.dadesTDVert.value[0].Gruix - 1 - 2.25 #+ (build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2)
                else:
                    pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs +  build_ele.dadesTDVert.value[0].BarraAmple - build_ele.desplXS.value - build_ele.dadesTDVert.value[0].Gruix - 1 - 2.25 #+ (build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2)

            else:#"TUB Interior"
                if nBarraVertical == 0:
                    pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs -build_ele.dadesTDVert.value[0].BarraAmple
                else:
                    pos = build_ele.listDesplVerticals.value[nBarraVertical].desplXAbs #+ (build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2 - build_ele.dadesTDVert.value[0].BarraAmple/2 ) #+ build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2
            ...
            Separacio_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAmple - 3.0
            if build_ele.TubSuperior.value == "L" or build_ele.TubSuperior.value == "TUB Interior + L":
                Separacio_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAmple -0.50
            Ample_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2
            if Ample_forat_femella >= 15:
                Ample_forat_femella = 15
            ...
            if build_ele.dadesTDVert.value[nBarraVertical].EncaixSup:
                ...
                if build_ele.dadesTDVert.value[nBarraVertical].PestanyesInv:
                    if desplY - desplYSup == 0:
                        ...
                        posFemellaY = desplY - desplYSup
                    Separacio_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAltura - 3
                    Ample_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAmple/2
                    if Ample_forat_femella >= 15:
                        Ample_forat_femella = 15
            else:
                posFemellaY = 0.0
                if not build_ele.dadesTDVert.value[nBarraVertical].invertirEncaixSup :
                    orientSupA = "Sup"
                else:
                    orientSupA = "Inf"
            ...
            if build_ele.TubSuperior.value == "L" or build_ele.TubSuperior.value == "TUB Interior + L":
                BarrallargadaVert = build_ele.dadesTDVert.value[nBarraVertical].BarraAlcada
                #if build_ele.TubSuperior.value == "L":
                if not build_ele.dadesTDVert.value[nBarraVertical].LlargadaAut:
                    BarrallargadaVert = build_ele.dadesTDVert.value[nBarraVertical].BarraAlcada #- posHorInf + alturaInf
                else:
                ...
                if build_ele.dadesTDVert.value[j].BarraInferior != "EN Inferior":
                    ...
                    #posHorInf = build_ele.dadesTDHortInter.value[nHorInf].Altura
                        if not build_ele.dadesTDVert.value[j].EncaixInf:
                            posHorInf = build_ele.BarresHorList.value[nHorInf].Posicio - build_ele.BarraAlturaInf.value   #- (build_ele.BarraAlturaInf.value/2 + build_ele.dadesTDHortInter.value[nHorInf].Altura/2)
                            posHorInf = build_ele.BarresHorList.value[nHorInf].Posicio - build_ele.BarraAlturaInf.value
                        else:
                            posHorInf = build_ele.BarresHorList.value[nHorInf].Posicio - build_ele.BarraAlturaInf.value + build_ele.dadesTDHortInter.value[nHorInf].Altura
                ...
                #if (nBarraVertical == 0 or nBarraVertical == build_ele.IntegerTDSelector.value-1) and not build_ele.dadesTDVert.value[nBarraVertical].LlargadaAut:
                #    profunditatFemella += 10
                #elif (nBarraVertical == 0 or nBarraVertical == build_ele.IntegerTDSelector.value-1) and  build_ele.dadesTDVert.value[nBarraVertical].LlargadaAut:
                #    profunditatFemella += 0#no detecta be la llargada!!
                #elif build_ele.dadesTDVert.value[nBarraVertical].LlargadaAut:
                if not build_ele.dadesTDVert.value[nBarraVertical].EncaixSup:
                    profunditatFemella = 3.1
                #else:
                #    profunditatFemella = 3.1
                ...
                #Ample_forat_femella = 15#build_ele.dadesTDVert.value[nBarraVertical].BarraAltura/2

                if profunditatFemella != 3.1:
                    Ample_forat_femella = build_ele.dadesTDVert.value[nBarraVertical].BarraAltura + build_ele.dadesTDVert.value[nBarraVertical].Gruix * 2
            ...
            bob = TDHorCollection(Femella = build_ele.dadesTDVert.value[nBarraVertical].MostrarTDVertical and build_ele.dadesTDVert.value[nBarraVertical].FemellaSup,
                                ...
                                PosFemellaX = pos,
                                PosFemellaY = posFemellaY,
                                Separacio_forat_femella = Separacio_forat_femella,#build_ele.dadesTDVert.value[nBarraVertical].BarraAmple - 3.0,
...
class EN_Conjunt_8():
    ...
    def create(self):
        ...
        result = create_element_class(self.build_ele_TD , self.doc_TD)
        model_elem_list = result["elements"]
        handle_list = result["handles"]
        model_elem_list_preview = result["preview_elements"]
