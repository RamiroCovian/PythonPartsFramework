...
print('Load EN_Horitzontal_inf_PP(|_|).py')
...
def create_element(build_ele, doc):
    ...
    TDHoritzontal = PP_EN_Horitzontal_Inf(build_ele.zUnique.value, build_ele.BarraAmple.value, build_ele.BarraAltura.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value, build_ele.invertirPeca.value,False,
    ...
    pythonpart = PythonPart ("PP_EN_Horitzontal_Inf",
...
class PP_EN_Horitzontal_Inf():
    ...
    def __init__(self, zUnique, BarraAmple, BarraAltura, BarraLlargada, BarraGruix, invertirPeca, invertirSup,
                    ...
                    femelles = [],
                    Separacio_forat_femella = 40, Altura_forat_femella = 5, Ample_forat_femella = 7.5,
                    retallInici = 0,
                    Encaixos = []):
        """ initialize
        ...
        self.EncaixosPar = Encaixos


        self.retallInici = retallInici
        self.llargBarraAmbRetallFinal = BarraLlargada
    ...
    def __repr__(self):
        ...
        return 'TD_Horitzontal(zUnique=%s, BarraAmple=%s, BarraAltura=%s, BarraLlargada=%s, BarraGruix=%s, invertirPeca=%s, invertirSup=%s,' \
            ...
            'BarraLayer=%s,' \
            'Separacio_forat_femella=%s, Altura_forat_femella=%s, Ample_forat_femella=%s,' \
            'EncaixosPar=%s)\n' \
            % (self.zUnique, self.BarraAmple, self.BarraAltura, self.BarraLlargada, self.BarraGruix, self.invertirPeca,  self.invertirSup,
               ...
               self.BarraLayer,
               self.Separacio_forat_femella, self.Altura_forat_femella, self.Ample_forat_femella,
               self.EncaixosPar)
    ...
    def create_PP_positions_femelles(self, ampleTDV, alturaTDV, matrixPosX, matrixPosY, matrixMostrar, offsetX, orientacio, Ample_forat_femellaVert, barraLlargadaReal, offsetHor,FemellesAux = [], desplXI = 0):
        ...
        distanciaentreTDVerticals = 1170#self.DistanciaEntreTD
        ...
        if TDV_count > 0:
            ...
            if len(self.Femelles)<1:
                ...
                bob = FemellesCollection( Femella = False,
                                        ...
                                        PosFemellaX = 0 + matrixPosX[0] - espaiCentrar,
                                        #PosFemellaX =  posX,
                                        PosFemellaY = offsetY + matrixPosY[0] - desplXI,
    ...
    def create(self):
        ...
        self.cara_a = "#" + str(self.invertirPeca) #Esq
        self.cara_a_inv = "" #Esq Invertida
        self.cara_b = "#"
        self.cara_b_inv = "" #Esq Invertida
        self.cara_c = "#"
        self.cara_c_inv = "" #Esq Invertida
        self.cara_d = "#"
        self.cara_d_inv = "" #Esq Invertida
        ...
        for Iencaix in dades_encaix:
            EncaixActiu = Iencaix.Encaix#Iencaix[0]
            if EncaixActiu:
                ele2, esValid = self.create_encaix(i)
                if esValid:
    ...
    def create_femelles(self, nFemella):
        ...
        #if PosFemellaX + 15 >= self.retallInici and (PosFemellaX - 15 < self.retallInici + self.llargBarraAmbRetallFinal or self.llargBarraAmbRetallFinal == 0):
        if PosFemellaX + 15 >= self.retallInici -1 and (PosFemellaX - 15 < self.retallInici + self.BarraLlargada):
            ...
            if Dfemella[3] != 0 and not ampleModificat:
    ...
    def create_encaix(self, nEncaix):
        ...
        geometry, esValid = self.crear_encaix_geo(geometry, nEncaix)
        return geometry, esValid
    ...
    def crear_encaix_geo(self, geometry, nEncaix):
        ...
        posEncaix = DEncaix.Posicio - self.retallInici - longEncaix/2
        ...
        try:
            ...
            print("deslplaçament y en encaix no trobat")

        Trans_ref_encaix = 0.0

        if posEncaix >= 0 and (posEncaix < self.BarraLlargada):


            #codi_encaix
            self.codi_encaix +=  "["+str(nEncaix+1)+"]C:@"+str(orientacio)+"@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posEncaix)+"@|Z:@"+str(Trans_ref_encaix)+"@#"
            ...
            if orientacio == "Dre":
                self.cara_b += "ENC@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posEncaix)+"@|Z:@"+str(Trans_ref_encaix)+"@#"
                ...
                vector = AllplanGeo.Vector3D(posEncaix , desplY, 0)
                ...
                if TePestanya:
                    ...
                    return cuboid_encaix_amb_pestanya[3], True
                else:
                    return cuboid_encaix_inf, True
            elif orientacio == "Sup":
                self.cara_c += "ENC@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posEncaix)+"@|Z:@"+str(Trans_ref_encaix)+"@#"
                ...
                vector = AllplanGeo.Vector3D(posEncaix , 0, self.BarraAltura/2 - AmplEncaix/2 + desplY)
                cuboid_encaix_inf = AllplanGeo.Move(cuboid_encaix_inf,vector)
                ...
                self.cara_a += "ENC@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posEncaix)+"@|Z:@"+str(Trans_ref_encaix)+"@#"
                self.cara_a_inv =  "ENC@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posInvEnca)+"@|Z:@"+str(Trans_ref_encaix)+"@#" + self.cara_a_inv

                cuboid_encaix_inf = AllplanGeo.Polyhedron3D.CreateCuboid( longEncaix, AmplEncaix ,profEncaix )
                vector = AllplanGeo.Vector3D(posEncaix, self.BarraAmple/2 - AmplEncaix/2 + desplY, self.BarraAltura - profEncaix)
                cuboid_encaix_inf = AllplanGeo.Move(cuboid_encaix_inf,vector)
                ...
                self.cara_d += "ENC@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posEncaix)+"@|Z:@"+str(Trans_ref_encaix)+"@#"
                self.cara_d_inv =  "ENC@|T:@"+str(TePestanya)+"@|D:@"+str(profEncaix)+"@|P:@"+str(posInvEnca)+"@|Z:@"+str(Trans_ref_encaix)+"@#" + self.cara_d_inv

                cuboid_encaix_inf = AllplanGeo.Polyhedron3D.CreateCuboid(longEncaix, profEncaix, AmplEncaix)
                vector = AllplanGeo.Vector3D(posEncaix, self.BarraAmple - profEncaix, self.BarraAltura/2 - AmplEncaix/2 + desplY)
                cuboid_encaix_inf = AllplanGeo.Move(cuboid_encaix_inf,vector)
