...
print('Load EN_Horitzontal_Sup_PP.py')
...
def create_element(build_ele, doc):
    ...
    TDHoritzontal = PP_EN_Horitzontal_Sup(build_ele.zUnique.value, build_ele.BarraAmple.value, build_ele.BarraAltura.value, build_ele.BarraLlargada.value, build_ele.BarraGruix.value, build_ele.invertirPeca.value,
                                          build_ele.AmpleEsq.value, build_ele.AmpleDre.value,
    ...
    pythonpart = PythonPart ("PP_EN_Horitzontal_Sup",
...
class PP_EN_Horitzontal_Sup():
    ...
    def __init__(self, zUnique, BarraAmple, BarraAltura, BarraLlargada, BarraGruix, invertirPeca,
        ...
        self.AmpleEsq = AmpleEsq
        self.AmpleDre = AmpleDre
    ...
    def set_false_femelles(self):
        for i in range(0,len(self.FemellesAux)):
            self.FemellesAux[i] = self.FemellesAux[i]._replace(Femella = False)
    ...
    def create(self):
        ...
        self.cara_a = "#" + str(self.invertirPeca)
        self.cara_a_inv = ""
        elements = []
    ...
    def create_barra(self):
        ...
        geometry2 = AllplanGeo.Polyhedron3D.CreateCuboid(self.BarraLlargada - self.BarraGruix*2,
                                                            ...
                                                            self.BarraAltura)
        #crear cuboid central
        geometry3 = AllplanGeo.Polyhedron3D.CreateCuboid(self.BarraLlargada - self.AmpleEsq - self.AmpleDre,
                                                            self.BarraAmple - self.BarraGruix,
                                                            self.BarraAltura)

        #es crea el vector per posicionarlo al centre i es resten els volums
        if self.invertirPeca:
            vector = AllplanGeo.Vector3D(self.AmpleEsq, -self.BarraGruix*2, 0)
        else:
            vector = AllplanGeo.Vector3D(self.AmpleEsq, self.BarraGruix*2, 0)
