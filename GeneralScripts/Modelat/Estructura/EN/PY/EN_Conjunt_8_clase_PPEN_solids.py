...
def create_element_class(build_ele, doc, placement_mat = AllplanGeo.Matrix3D(), element_listTD = [], Pared = [], ENLlargada = -1.0, ENAlcada = -1.0 , numIDParent = 1): # type: ignore
    """
    ...
    if ENLlargada != -1:
        build_ele.BarraLlargadaEN.value = ENLlargada
    if ENAlcada != -1:
        build_ele.BarraLlargadaVert.value = ENAlcada
    ...
    if build_ele.SelectorPPEN.value == 1 and build_ele.SelectorENH.value == "EN Inferior":
        ...
        if build_ele.UpdateBalconeraFinestraEN.value:
            ...
            if build_ele.SelectorTDHorInfBalcEN.value != "EN Inferior":
            ...
            if build_ele.TipusPortaEN.value == 'Corredissa':
                if getNumFromText(build_ele, build_ele.SelectorENHorSupBalcCorredisa.value, 3) != "":
                    nHorSup2 = getNumFromText(build_ele, build_ele.SelectorENHorSupBalcCorredisa.value, 3)
                if build_ele.SelectorENHorSupBalcCorredisa.value != "EN Inferior" and build_ele.SelectorENHorSupBalcCorredisa.value != "EN Superior":
    ...
    common_propsObject.Layer = KN_XPS_CAVITAT#56811#build_ele.BarraLayer.value#64178
    common_propsObject.Color = 16 #GRIS

    common_propsObjectRecess = AllplanBaseElements.CommonProperties()
    common_propsObjectRecess.Layer = KN_XPS_RECESS#56811#build_ele.BarraLayer.value#64178
            ...
            for nVertMat in range(0,build_ele.IntegerENSelector.value):
                ...
                if  j < len(build_ele.nListBarresHor.value):
                    ...
                    for nbarraHorAnterior in range(0,len(build_ele.BarresHorListEN.value)-1):
                        ...
                        if len(build_ele.BarresHorListEN.value[nbarraHorAnterior].BarraFinal) < 1:
                            break
...
def set_layers(build_ele, i):
    ...
    build_ele.NomTDEN.value = "EN"+str(i+1).zfill(3)
    ...
    if i % 2 == 0:
        ...
        EN_FIXACIO_X                     = 40099
        EN_FIXACIO_Y                     = 40100
        #build_ele.SelectorLayerSup.value = "EN_ESTRUCTURA"
        #build_ele.SelectorLayerInf.value = "EN_ESTRUCTURA"
        #build_ele.SelectorLayerHor.value = "EN_ESTRUCTURA"
        #build_ele.SelectorLayerRef.value = "EN_ESTRUCTURA"
        build_ele.BarraLayerVert.value   = 40001 #40076
        build_ele.BarraLayer.value       = 40001 #40076

        build_ele.InvertirENHoritzontalSupL.value = True
        build_ele.InvertirENHoritzontalInf.value = True
    else:
        KN_XPS_RECESS                    = 40089
        ...
        #build_ele.SelectorLayerHor.value = "EN_ESTRUCTURA"
        #build_ele.SelectorLayerRef.value = "EN_ESTRUCTURA"
        build_ele.BarraLayerVert.value   = 40076
        build_ele.BarraLayer.value       = 40076

        build_ele.InvertirENHoritzontalSupL.value = False
        build_ele.InvertirENHoritzontalInf.value = False
...
def guardar_valors_actuals(build_ele, i):
    ...
    if len(build_ele.listDesplVerticalsInteriorsEN.value) <= i:
        build_ele.listDesplVerticalsInteriorsEN.value.append(bobDespl)
    else:
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplX = build_ele.desplXVert.value)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplY = build_ele.desplYVert.value)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(mostrarLiniaVertA = build_ele.mostrarLiniaVertA.value)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplLinA = build_ele.desplVertLinA.value)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(mostrarLiniaVertB = build_ele.mostrarLiniaVertB.value)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplLinB = build_ele.desplVertLinB.value)
    '''
    #DADES TDV UNIQUES
...
def mostrar_valors_actuals(build_ele, i):
    ...
    while len(build_ele.listDesplVerticalsInteriorsEN.value) <= i:
        build_ele.listDesplVerticalsInteriorsEN.value.append(bobDespl)
    else:
        build_ele.desplXVert.value = build_ele.listDesplVerticalsInteriorsEN.value[i].desplX
        build_ele.desplYVert.value = build_ele.listDesplVerticalsInteriorsEN.value[i].desplY
        build_ele.mostrarLiniaVertA.value = build_ele.listDesplVerticalsInteriorsEN.value[i].mostrarLiniaVertA
        build_ele.desplVertLinA.value = build_ele.listDesplVerticalsInteriorsEN.value[i].desplLinA
        build_ele.mostrarLiniaVertB.value = build_ele.listDesplVerticalsInteriorsEN.value[i].mostrarLiniaVertB
        build_ele.desplVertLinB.value = build_ele.listDesplVerticalsInteriorsEN.value[i].desplLinB
    '''
...
def set_values(build_ele, i):
    ...
    while len(build_ele.listDesplVerticalsInteriorsEN.value) <= i:
        ...
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplX = 0)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplY = 0)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(mostrarLiniaVertA = True)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplLinA = 0)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(mostrarLiniaVertB = False)
        build_ele.listDesplVerticalsInteriorsEN.value[i] = build_ele.listDesplVerticalsInteriorsEN.value[i]._replace(desplLinB = 0)
...
def remove_values(build_ele, i):
    ...
    for x in range(i*2, len(build_ele.listDesplVerticalsInteriorsEN.value)):
        build_ele.listDesplVerticalsInteriorsEN.value.pop()
...
def mostrar_valors_Vert(build_ele, nBarraVert, nVert):
    ...
    #build_ele.desplXVert.value = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert].desplX
    #build_ele.desplYVert.value  = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert].desplY
    build_ele.mostrarLiniaVertA.value = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert].mostrarLiniaVertA
    build_ele.desplVertLinA.value = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert].desplLinA
    build_ele.mostrarLiniaVertB.value = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert].mostrarLiniaVertB
    build_ele.desplVertLinB.value = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert].desplLinB
...
def guardar_valors_Vert(build_ele, nBarraVert, nVert):
    ...
    build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert] = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert]._replace(desplX = build_ele.desplXVert.value )
    build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert] = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert]._replace(desplY = build_ele.desplYVert.value )
    build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert] = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert]._replace(mostrarLiniaVertA = build_ele.mostrarLiniaVertA.value)
    build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert] = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert]._replace(desplLinA = build_ele.desplVertLinA.value)
    build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert] = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert]._replace(mostrarLiniaVertB = build_ele.mostrarLiniaVertB.value)
    build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert] = build_ele.listDesplVerticalsInteriorsEN.value[nBarraVert * nBarresVertInt + nVert]._replace(desplLinB = build_ele.desplVertLinB.value)
...
def crear_barres_inferiors_multiples(build_ele, nBarraInf, mostrarActual, doc,placement_mat):
    ...
    if build_ele.SepararTDHoritzontalInf.value:
        ...
        #--------
        common_propsObject = AllplanBaseElements.CommonProperties()
        common_propsObject.Layer = KN_XPS_CAVITAT#KN_XPS_CAVITAT
        common_propsObject.Color = 16 #blanc
        common_propsObjectRecess = AllplanBaseElements.CommonProperties()
        common_propsObjectRecess.Layer = KN_XPS_RECESS#KN_XPS_RECESS
...
def set_dadesENVertEN_to_true(build_ele):
    for elem in range(0, len(build_ele.dadesTDVertEN.value)):
        build_ele.dadesTDVertEN.value[elem] =  build_ele.dadesTDVertEN.value[elem]._replace(MostrarTDVertical = True)

    print()



def values_from_solid3D(build_ele, doc, placement_mat, element_listEN, Pared, desplAbs = 0, atrib = []):
    solidsInteriors = []
    tubsPerSolidAfegit = []
    if len(element_listEN) == 0:
        return
    else:
        #inicialitzar_valors_inici(build_ele, len(element_listEN)-1)
        ...
        pos = 0
        for posVert in range( 0, len(build_ele.dadesTDVertEN.value)):
            ...
            lenActual = len(build_ele.listDesplVerticalsEN.value)
            nSolid = 0
            for solid in solidsInteriors:
                if posVert < len(build_ele.listDesplVerticalsEN.value) and \
                    solid["Esq"] > puntInferiorEsqTD.X + desplAbs and \
                    solid["Esq"] < puntInferiorEsqTD.X + desplAbs + build_ele.BarraLlargadaEN.value and \
                    solid["Dre"] > puntInferiorEsqTD.X + desplAbs  and \
                    solid["Esq"] < puntInferiorEsqTD.X + desplAbs + build_ele.BarraLlargadaEN.value :

                    if build_ele.listDesplVerticalsEN.value[posVert].desplXAbs + desplAbs > solid["Esq"] - puntInferiorEsqTD.X  and \
                            build_ele.listDesplVerticalsEN.value[posVert].desplXAbs + desplAbs < solid["Dre"] - puntInferiorEsqTD.X and \
                                posVert not in tubsAfegitsNous:

                        #build_ele.BarresVertList.value[posVert] = build_ele.BarresVertList.value[posVert]._replace(BarraVert = False)
                        build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(MostrarTDVertical = False)
                        build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(esProvisional = True)
                        build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(EncaixSup = True)
                        build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(EncaixInf = True)

                        listVertNous = []

                        if not tubsPerSolidAfegit[nSolid]:
                            solid["tubInf"], solid["tubSup"], listVertNous, altern = crear_tubs_per_solid(build_ele, solid, puntInferiorEsqTD, posVert, desplAbs)
                            ...
                            if len(listVertNous)>= 1:
                                build_ele.SelectorTDHorEsqBalcEN.value = "Tub " + str(esq)
                            if len(listVertNous)>= 2:
                                build_ele.SelectorTDHorDreBalcEN.value = "Tub " + str(dre)

                        #if solid["tubInf"] != "En Inferior":
                        #    build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(BarraSuperior = solid["tubInf"])
                        #    listVertNousProv = crear_tubs_provisionals_per_solid(build_ele, posVert, solid["tubInf"], solid["tubSup"])
                        #    tubsAfegitsNous += listVertNousProv
                        #else:
                        #    build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(BarraSuperior = solid["tubSup"])

                        tubsAfegitsNous.append(posVert)

                        #UpdateBalconeraFinestraEN
                        build_ele.SelectorPPEN.value = 3
                        build_ele.UpdateBalconeraFinestraEN.value = True
                        build_ele.MostrarBalconeraEN.value = True


                        build_ele.nomPremarcEN.value = "Premarc" + str(pos)
                        build_ele.TipusPortaEN.value = "Porta 1"
                        if len(atrib) > nSolid:
                            if atrib[nSolid] == "PortaCorredissa":
                                build_ele.TipusPortaEN.value = "Corredissa"
                                build_ele.VerticalAutoBalcCorred.value = True
                            elif atrib[nSolid] == "PortaEN":
                                build_ele.TipusPortaEN.value = "Porta 1"

                        build_ele.AmpleBalconeraEN.value = solid["Dre"] - solid["Esq"]
                        build_ele.LlargadaBalconeraEN.value = solid["Sup"] - solid["Inf"]

                        build_ele.PosicioXBalconeraEN.value = solid["Esq"] - puntInferiorEsqTD.X - desplAbs
                        build_ele.PosicioYBalconeraEN.value = 0
                        build_ele.PosicioZBalconeraEN.value = solid["Inf"] - puntInferiorEsqTD.Z

                        build_ele.SelectorENHorSupBalcCorredisa.value = "Tub 2"
                            #build_ele.SelectorTDHorCorred1.value = "tub 3"
                            #build_ele.SelectorTDHorCorred2.value = "tub 4"
                            #build_ele.SelectorTDHorCorred3.value = "tub 5"

                        build_ele.SelectorTDHorSupBalcEN.value = solid["tubSup"]
                        build_ele.SelectorTDHorInfBalcEN.value = solid["tubInf"]
                        build_ele.SelectorREFInfBalcEN.value = "tub 0"

                        build_ele.VerticalAutoBalcEN.value = False
                        build_ele.SelectorBalcFinAnt.value = 0
                        '''
                        build_ele.SelectorTDHorEsqBalcEN.value = "Tub 0"
                        if len(listVertNous)>= 1:
                            build_ele.SelectorTDHorEsqBalcEN.value = "Tub " + str(listVertNous[0])
                        build_ele.SelectorTDHorDreBalcEN.value = "Tub 9"
                        if len(listVertNous)>= 2:
                            build_ele.SelectorTDHorDreBalcEN.value = "Tub " + str(listVertNous[1])
                        '''
                        create_element_class(build_ele = build_ele, doc = doc, placement_mat = placement_mat, element_listTD = element_listEN, Pared = Pared)
                        pos+= 1
                nSolid += 1

            print("")
        build_ele.IntegerENSelector.value += 1
        set_values(build_ele, build_ele.IntegerENSelector.value)
        for posVert in range( 0, len(build_ele.dadesTDVertEN.value)-1):
            #if #build_ele.listDesplVerticalsEN.value[posVert].desplXAbs == build_ele.BarraLlargadaTD.value or \
            if build_ele.listDesplVerticalsEN.value[posVert].desplXAbs >= build_ele.BarraLlargadaEN.value - build_ele.dadesTDVertEN.value[0].BarraAmple and \
                posVert != 9:
                build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(MostrarTDVertical = False)
            if build_ele.listDesplVerticalsEN.value[posVert].desplXAbs <= 0 and posVert != 0:
                build_ele.dadesTDVertEN.value[posVert] = build_ele.dadesTDVertEN.value[posVert]._replace(MostrarTDVertical = False)
    build_ele.SelectorPPEN.value = 1



def crear_tubs_per_solid(build_ele, solid, puntInferiorEsqEN, posVert, desplAbs):
    #for solid in solids:

    build_ele.IntegerENSelector.value += 1
    posVertEsq = build_ele.IntegerENSelector.value
    set_values(build_ele, posVertEsq)
    build_ele.IntegerENSelector.value += 1
    posVertDre = build_ele.IntegerENSelector.value
    #posVertDre = len(build_ele.IntegerENSelector.value)
    set_values(build_ele, posVertEsq)
    trobatEsq = False
    trobatDre = False

    for pos in range(posVert, len(build_ele.dadesTDVertEN.value)):
        #if build_ele.dadesTDVertEN.value[pos].MostrarTDVertical == False and trobatEsq and not trobatDre:
        if build_ele.listDesplVerticalsEN.value[pos].desplXAbs > build_ele.BarraLlargadaEN.value and trobatEsq and not trobatDre:
            posVertDre = pos
            trobatDre = True
        if build_ele.listDesplVerticalsEN.value[pos].desplXAbs > build_ele.BarraLlargadaEN.value and not trobatEsq and not trobatDre:
            posVertEsq = pos
            trobatEsq = True

    #crear esquerra
    #set_values(build_ele, posVertEsq)
    build_ele.dadesTDVertEN.value[posVertEsq] = build_ele.dadesTDVertEN.value[posVertEsq]._replace(MostrarTDVertical = True)
    build_ele.dadesTDVertEN.value[posVertEsq] = build_ele.dadesTDVertEN.value[posVertEsq]._replace(BarraAmple = 50)
    posEsq = solid["Esq"] - puntInferiorEsqEN.X - build_ele.dadesTDVertEN.value[posVertEsq].BarraAmple - 5
    print("posEsq: " + str(posEsq))
    build_ele.listDesplVerticalsEN.value[posVertEsq] = build_ele.listDesplVerticalsEN.value[posVertEsq]._replace(desplXAbs =  solid["Esq"] - puntInferiorEsqEN.X - desplAbs - build_ele.dadesTDVertEN.value[posVertEsq].BarraAmple - 50)

    #crear dreta
    #set_values(build_ele, posVertDre)
    build_ele.dadesTDVertEN.value[posVertDre] = build_ele.dadesTDVertEN.value[posVertDre]._replace(MostrarTDVertical = True)
    build_ele.listDesplVerticalsEN.value[posVertDre] = build_ele.listDesplVerticalsEN.value[posVertDre]._replace(desplXAbs = solid["Dre"] - puntInferiorEsqEN.X - desplAbs+  50)
    posDre = solid["Dre"] - puntInferiorEsqEN.X + 5
    print("posDre: " + str(posDre))


    posBarraHorSup = len(build_ele.BarresHorListEN.value)-2
    posBarraHorInf = len(build_ele.BarresHorListEN.value)-1
    trobatEsq = False
    trobatDre = False

    for pos in range(posVert, len(build_ele.BarresHorListEN.value)):
        if build_ele.BarresHorListEN.value[pos].BarraHor == False and trobatEsq and not trobatDre:
            posBarraHorInf = pos
            trobatDre = True
        if build_ele.BarresHorListEN.value[pos].BarraHor == False and not trobatEsq and not trobatDre:
            posBarraHorSup = pos
            trobatEsq = True


    altern = False
    if build_ele.listDesplVerticalsEN.value[posVertEsq].desplXAbs > build_ele.listDesplVerticalsEN.value[posVertDre].desplXAbs:
        aux = posVertEsq
    ...
    #crear Superior
    #posBarraHor = len(build_ele.BarresHorListEN.value-1)
    set_values_barresHor(build_ele, posBarraHorSup)
    set_valors_hor_ini(build_ele, 0, posBarraHorSup)
    build_ele.BarresHorListEN.value[posBarraHorSup] = build_ele.BarresHorListEN.value[posBarraHorSup]._replace(BarraHor = True)
    build_ele.BarresHorListEN.value[posBarraHorSup] = build_ele.BarresHorListEN.value[posBarraHorSup]._replace(BarraInici = "Tub " + str(posVertEsq) )
    build_ele.BarresHorListEN.value[posBarraHorSup] = build_ele.BarresHorListEN.value[posBarraHorSup]._replace(BarraFinal = "Tub " + str(posVertDre) )
    build_ele.BarresHorListEN.value[posBarraHorSup] = build_ele.BarresHorListEN.value[posBarraHorSup]._replace(AutoLongitud = True )
    build_ele.BarresHorListEN.value[posBarraHorSup] = build_ele.BarresHorListEN.value[posBarraHorSup]._replace(Posicio = solid["Sup"] - puntInferiorEsqEN.Z )
    #build_ele.dadesTDHortInter.value[posBarraHor] = build_ele.dadesTDHortInter.value[posBarraHor]._replace(desplY = solid["Sup"] - puntInferiorEsqEN.Z)
    #build_ele.dadesTDHortInter.value[posBarraHor] = build_ele.dadesTDHortInter.value[posBarraHor]._replace(Posicio = solid["Sup"] - puntInferiorEsqEN.Z)
    posSup = solid["Sup"] - puntInferiorEsqEN.Z
    ...
    #crear inferior
    #posBarraHor = len(build_ele.BarresHorListEN.value)
    if solid["Inf"] - puntInferiorEsqEN.Z > 50:
        set_values_barresHor(build_ele, posBarraHorInf)
        set_valors_hor_ini(build_ele, 0, posBarraHorInf)
        build_ele.BarresHorListEN.value[posBarraHorInf] = build_ele.BarresHorListEN.value[posBarraHorInf]._replace(BarraHor = True)
        build_ele.BarresHorListEN.value[posBarraHorInf] = build_ele.BarresHorListEN.value[posBarraHorInf]._replace(BarraInici = "Tub " + str(posVertEsq) )
        build_ele.BarresHorListEN.value[posBarraHorInf] = build_ele.BarresHorListEN.value[posBarraHorInf]._replace(BarraFinal = "Tub " + str(posVertDre) )
        build_ele.BarresHorListEN.value[posBarraHorInf] = build_ele.BarresHorListEN.value[posBarraHorInf]._replace(AutoLongitud = True )
        build_ele.BarresHorListEN.value[posBarraHorInf] = build_ele.BarresHorListEN.value[posBarraHorInf]._replace(Posicio = solid["Inf"] - puntInferiorEsqEN.Z )
        #build_ele.dadesTDHortInter.value[posBarraHor] = build_ele.dadesTDHortInter.value[posBarraHor]._replace(desplY = solid["Inf"] - puntInferiorEsqEN.Z)
        #build_ele.dadesTDHortInter.value[posBarraHor] = build_ele.dadesTDHortInter.value[posBarraHor]._replace(Posicio = solid["Sup"] - puntInferiorEsqEN.Z)
        posInf = solid["Inf"] - puntInferiorEsqEN.Z
        print("posInf: " + str(posInf))

        posBarraHorInf = "Tub " + str(posBarraHorInf)
    else:
        posBarraHorInf = "EN Inferior"
    print()

    return posBarraHorInf, "Tub " + str(posBarraHorSup), [posVertEsq, posVertDre], altern



def crear_tubs_provisionals_per_solid(build_ele, posVert, tubInf, tubSup):
    listVertNous = []
    build_ele.IntegerENSelector.value += 2
    set_values(build_ele, build_ele.IntegerENSelector.value)
    Afegits = 0
    for pos in range(posVert, len(build_ele.dadesTDVertEN.value)):
        if (build_ele.listDesplVerticalsEN.value[pos].desplXAbs > build_ele.BarraLlargadaEN.value or pos >= len(build_ele.dadesTDVertEN.value)-3 ) \
            and Afegits < 2 : #and build_ele.dadesTDVertEN.value[posVert].esProvisional == False:
            build_ele.listDesplVerticalsEN.value[pos] = build_ele.listDesplVerticalsEN.value[pos]._replace(desplXAbs = build_ele.listDesplVerticalsEN.value[posVert].desplXAbs )
            build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(esProvisional = True)
            #build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(esProvisionalAuto = True)
            build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(EncaixSup = True)
            build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(EncaixInf = True)
            listVertNous.append(pos)

            if Afegits == 0:
                build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(BarraSuperior = tubSup)
                build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(BarraInferior = tubInf)
            elif Afegits == 1:
                build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(BarraSuperior = "TD Superior")
                build_ele.dadesTDVertEN.value[pos] = build_ele.dadesTDVertEN.value[pos]._replace(BarraInferior = tubSup)
...
class EN_Conjunt_8():
    ...
    def __init__(self, build_ele : BuildingElement, doc, placement_mat = AllplanGeo.Matrix3D(), element_listEN = [], Pared = [], BarraLlargada = -1.0, ENAlcada = -1.0 , numID= 1):

        self.build_ele_EN = BuildingElement()
        self.build_ele_EN = build_ele#.deep_copy()
        self.doc_EN = doc

        self.placement_mat = placement_mat

        self.element_listEN = element_listEN
        self.ENLlargada = BarraLlargada
        self.ENAlcada = ENAlcada
        self.Pared = Pared
    ...
    def filename(self):
        return "EN_Conjunt_8_clase_PPEN_solids.py"
    ...
    def create_estructura(self, build_ele):
        ...
        self.result = create_element_class(build_ele , doc, self.placement_mat, self.element_listEN, self.Pared, self.ENLlargada, self.ENAlcada, self.numID)
        #model_elem_list = result["elements"]
        #handle_list = result["handles"]
        #model_elem_list_preview = result["preview_elements"]
        #group_elems = result["group_elems"]
    ...
    def create(self):
        ...
        self.result = create_element_class(self.build_ele_EN , doc, self.placement_mat, self.element_listEN, self.Pared, self.ENLlargada, self.ENAlcada, self.numID)
    ...
    def modify_element_property(self,
        ...
        Args:
            ...
            value: new value
        """
        prop = self.build_ele_EN.get_property(name)

        print("prop: " + str(prop))

        if prop is not None:
            BuildingElementValueUtil.update_value(name, value, self.build_ele_EN, self.build_ele_EN)# ,self.build_ele_ctrl_props_list)


    def set_values_tubs_true(self):
        set_dadesENVertEN_to_true(self.build_ele_EN)
        return True
    ...
    def values_from_solid3D(self, desplAbs, build_ele, atr_list):
        values_from_solid3D(build_ele, self.doc_EN, self.placement_mat, self.element_listEN, self.Pared, desplAbs, atr_list)
