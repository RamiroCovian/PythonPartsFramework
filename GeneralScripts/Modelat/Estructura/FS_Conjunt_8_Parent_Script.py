...
print('Load FS_Conjunt_8.py Estructura FS cav auto')
...
class Conjunt_8_Parent(BaseScriptObject):
    ...
    def __init__(self,
        ...
        # FS cavidades manuales
        self.create_fs_cavidades_manuales = False
    ...
    def start_input(self):
        ...
        self.script_object_interactor = MultiElementSelectInteractor(
            ...
            prompt_msg = "Selecciona los elements"
        )

        self.INPUT_MODE = 0

        print("start_input END")
    ...
    def start_next_input(self):
        ...
        print(self.point_input_result.input_point)
        ...
        if not bool(self.element_list):
            ...
            self.element_list = [element for element in self.selection_result.sel_elements]
        ...
        if self.point_input_result != PointInteractorResult():
            self.script_object_interactor = None
    ...
    def execute(self) -> CreateElementResult:
        ...
        if self.conjuntSelected != 1:
            ...
            if self.conjuntSelected == 2:
                ...
                TD_conjunt  =  EN_Conjunt_8(self.build_ele, DocumentManager.get_instance().document)
            elif self.conjuntSelected == 4:
                #TD_conjunt  =  IS_Conjunt_8(self.build_ele_list[3], DocumentManager.get_instance().document)
                TD_conjunt  =  IS_Conjunt_8(self.build_ele, DocumentManager.get_instance().document)
            elif self.conjuntSelected == 5:
                fs_class = FS(self.build_ele, DocumentManager.get_instance().document)
                if self.create_fs_cavidades_manuales:
                    # Update cavidades manuales
                    fs_class.add_cavidades_manuales(self.build_ele)
                else:
                    # Update cavidades
                    fs_class.get_and_make_cavidades_from_list(self.selection_result.sel_elements)
                TD_conjunt  =  fs_class

                # FS text
                # Add preview text
                preview_symbols = fs_class.set_and_get_preview_symbols()
            ...
            python_part = python_part_util.create_pythonpart(self.build_ele,
                                         placement_matrix = self.placement_mat)

            #return CreateElementResult(python_part,
            #                   multi_placement = True)

            return CreateElementResult(model_elem_list_preview, handle_list,
                                   multi_placement = True,
    ...
    def gestio_palette_fills(self, pyp_path):
        ...
        if self.conjuntSelected == 1:
            ...
            self.build_ele_service.read_data_from_pyp(pyp_path + "\\Conjunt_8_Parent_Script.pal", self.str_table_service.str_table, False,
            ...
            self.palette_service.show_palette("\\Conjunt_8_Parent_Script.pal")
...
#
    ...
    def modify_element_property(self, _name: str, _value: Any) -> bool:
        ...
        print("modify_element_property " + " - " + str(_name) + " - " + str(_value))
        print("---------------------------------")


        if _name == "SelectorPPPare":
            self.conjuntSelected = _value
            try:
                if self.palette_service.build_ele_list[0].SelectorPPPare.value == _value:
                    self.is_update = True
                else:
                    self.is_update = False
                    ...
                    #self.palette_service.build_ele_list[0].SelectorPPPare.value = _value
                print("PRE - ")
                #self.gestio_palette_fills(self.pyp_path)
            except Exception as e:
    ...
    def on_cancel_function(self):
        ...
        if self.script_object_interactor:
            return self.on_cancel_function()
    ...
    def create_element(self):
        ...
        if self.conjuntSelected != 1:
            ...
            if self.conjuntSelected == 2:
                ...
                TD_conjunt  =  IS_Conjunt_8(build_ele, DocumentManager.get_instance().document, self.placement_mat)
            elif self.conjuntSelected == 5:
                fs_class = FS(self.build_ele, DocumentManager.get_instance().document, self.placement_mat)
                # if create cavidades manuales
                if self.create_fs_cavidades_manuales:
                    fs_class.add_cavidades_manuales(self.build_ele)
                else:
                    # Update cavidades
                ...
                TD_conjunt  =  fs_class
                #TD_conjunt  =  FS(build_ele, DocumentManager.get_instance().document, self.placement_mat)
    ...
    def on_control_event(self, event_id: int):
        ...
        if self.conjunt != None:
            ...
            if event_id == envent_id_create_figure:
                self.create_fs_cavidades_manuales = True
                self.create_element()
