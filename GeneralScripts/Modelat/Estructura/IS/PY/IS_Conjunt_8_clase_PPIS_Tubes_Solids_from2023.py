...
def on_control_event(build_ele, event_id: int):
    nVer = getNumFromText(build_ele, build_ele.SelectorTubHor.value, 3)


    if event_id == 1000:#recalcul IS
        build_ele.SelectorTubHor.value = "Tub 0"
        ...
        mostrar_valors_hor(build_ele, nVer)
...
def get_elem_pythonpart(build_ele: BuildingElement, doc, placement_mat = []) -> PythonPart:
    ...
    Returns:
        PythonPart object
    """
    result = create_element_class(build_ele, doc, placement_mat= placement_mat)
...
def create_pythonpart(build_ele: BuildingElement, doc, placement_mat = []) -> PythonPart:
    """Create a PythonPart containing cube geometry and some attributes
...
def create_element_class(build_ele, doc, placement_mat = AllplanGeo.Matrix3D(), element_listIS = [], Pared = [], ISLlargada = -1.0, ISAlcada = -1.0 ): # type: ignore


    #values_from_solid3D(build_ele, placement_mat, element_listIS)
    ...
    if ISLlargada != -1:
        build_ele.ISllargada.value = ISLlargada
    if ISAlcada != -1:
        build_ele.ISAmplada.value = ISAlcada
    ...
    if build_ele.SelectorPPIS.value != 1:
        ...
        if build_ele.RotarTub.value:
            handle_list = create_handles_pos_Tubs(build_ele)
        else:
    ...
    if build_ele.actualitzaTubs.value:
        ...
        for posBarraHor in range(0, build_ele.IntegerISSelectorHor.value):#len(build_ele.dadesTubHor.value)):
            ...
            if build_ele.dadesTubHor.value[posBarraHor].LlargadaAut :#and build_ele.dadesTubHor.value[posBarraHor].RotarTub:
                llargada = set_llargadaAut(build_ele, posBarraHor, nHorSeleccionat, build_ele.dadesTubHor.value[posBarraHor].RotarTub)
                ...
                if build_ele.dadesTubHor.value[posBarraHor].tipusTubAuto:
                    ...
                    mostratError = detectar_es_posible(build_ele, doc, posBarraHor, nHorSeleccionat, mostratError)
                if llargada <= 50:
                    build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(Mostrar = False)
            ...
            if nHorSeleccionat == posBarraHor:
                build_ele.BarraLlargadaIS.value = llargada
            ...
            posUltimForat = 0
            for forat in forats:
                posUltimForat = llargada - forat.Posicio - (forat.Llargada / 2 )
            ...
            if build_ele.dadesTubHor.value[posBarraHor].TipusTub == "TUB A":
                ...
                mostrarPestanyaSup = posUltimForat == 302.5 #and build_ele.dadesTubHor.value[posBarraHor].retallFinal <= 0

                TubHoritzontal = PP_IS_Horitzontal(random.random() * 3600,0, 50, 20,llargada, 1.5,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                            False,  14, build_ele.BarraLayer.value,# IsUseGlobalPropVert, FounColorVert, BarraLayerVert,
                                            colis, potes, forats, #ColisPar, PotaPar, #matrius
                                            15, 3.0, 30,#build_ele.dadesTubHor.value[posBarraHor].Ample_forat_femella, build_ele.dadesTubHor.value[posBarraHor].Altura_forat_femella, build_ele.dadesTubHor.value[posBarraHor].Separacio_forat_femella,
                                            femelles, #Femelles, #matriu
                                            llargada/2, #posicio_centre_masses,
                                            False,100, #IsFirstCancam, Dis1cancam,
                                            False, 500, #IsSecondCancam, Dis2cancam,
                                            (llargada == 650 and build_ele.dadesTubHor.value[posBarraHor].retallFinal <= 0) or mostrarPestanyaSup, build_ele.dadesTubHor.value[posBarraHor].retallInicial == 0,#PestanyaSuperior, PestanyaInferior,
                ...
                mostrarPestanyaSup = posUltimForat == 652.5 #and build_ele.dadesTubHor.value[posBarraHor].retallFinal <= 0
                ...
                TubHoritzontal = PP_IS_Horitzontal(random.random() * 3600,0, 50, 20,llargada, 1.5,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                            ...
                                            False,100, #IsFirstCancam, Dis1cancam,
                                            False, 500, #IsSecondCancam, Dis2cancam,
                                            (llargada == 5550 and build_ele.dadesTubHor.value[posBarraHor].retallFinal <= 0) or mostrarPestanyaSup, build_ele.dadesTubHor.value[posBarraHor].retallInicial == 0,#True, True,#PestanyaSuperior, PestanyaInferior,
                                            encaixos, True, #EncaixosPar))
                ...
                TubHoritzontal = PP_IS_Horitzontal(random.random() * 3600,0, 50, 20,llargada, 1.5,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                            ...
                                            False,100, #IsFirstCancam, Dis1cancam,
                                            False, 500, #IsSecondCancam, Dis2cancam,
                                            (llargada == 1350 and build_ele.dadesTubHor.value[posBarraHor].retallFinal <= 0) or mostrarPestanyaSup, build_ele.dadesTubHor.value[posBarraHor].retallInicial <= 0,#PestanyaSuperior, PestanyaInferior,
                                            encaixos, True, #EncaixosPar))
                                            build_ele.dadesTubHor.value[posBarraHor].retallInicial, llargada - build_ele.dadesTubHor.value[posBarraHor].retallFinal, 1350) #)
            ...
            volume = AllplanGeo.CalcMass(table_brepAux)# volume[0] = error, volume[1] = volume, volume[2] = surface, volume[3] = gravity_point
            ...
            table_attr_listAux = [AllplanBaseElements.AttributeString(684, "IfcBuildingElementProxy"),
                                    ...
                                    AllplanBaseElements.AttributeString(220,  str(round0_5( TubHoritzontal.get_llargada()) ) ),
                                    AllplanBaseElements.AttributeString(2455, str(round0_5( TubHoritzontal.get_llargada()) ) ),
                                    ...
                                    AllplanBaseElements.AttributeString(2547, ""),   #LONG_TUB

                                    AllplanBaseElements.AttributeString(2552, "7874"),   #Densitat
                                    #AllplanBaseElements.AttributeString(223, str(round(volume[1]/1000000000, 6 ) )+ " m³" ),   #Volum
                                    AllplanBaseElements.AttributeDouble(223, round(volume[1]/1000000000, 6 ) )   #Volum
            ...
            #if matrix_PosHor[12]< build_ele.BarraLlargadaIS.value + build_ele.ExtenderInf.value and build_ele.BarresHorList.value[posBarraHor].BarraHor:
            ...
            TranslationFather = matrix_PosHor
            TranslationFather.SetValue(12, placement_mat[12] + matrix_PosHor[12])
            TranslationFather.SetValue(13, placement_mat[13] + matrix_PosHor[13])
            TranslationFather.SetValue(14, placement_mat[14] + matrix_PosHor[14])
            ...
            if build_ele.dadesTubHor.value[posBarraHor].Mostrar and \
                ...
                group_elems.append(PythonPart ("PP_EN_Horitzontal", parameter_list = TubHoritzontal.get_params_list(),
                                        ...
                                        views = table_viewsAux, matrix = TranslationFather, common_props = common_propsHor, attribute_list = table_attr_listAux))
        ...
        for linia in liniesAutomatitzacio:
            ...
            group_elems_preview.append(linia)
            #preview_symbols.add_polyline(linia)


        preview_symbols.add_text(text=             "Actualitzant PythonPart IS",
                                reference_point=   AllplanGeo.Point3D(0,0,0),
        ...
        #---------------------mostrar linies----------------------------
        '''
        for posBarraHor in range(0, build_ele.IntegerISSelectorHor.value):
            llargada = 50
            if build_ele.dadesTubHor.value[posBarraHor].LlargadaAut :#and build_ele.dadesTubHor.value[posBarraHor].RotarTub:
                llargada = set_llargadaAut(build_ele, posBarraHor, build_ele.dadesTubHor.value[posBarraHor].RotarTub)
            ...
            if posBarraHor == nHorSeleccionat:
                matrix_PosHor.SetValue(12, build_ele.desplX.value)
                matrix_PosHor.SetValue(13, build_ele.desplY.value)
                matrix_PosHor.SetValue(14, 0 )
        ...
        '''
        tubSuperiorDelSeleccionat = getNumFromText(build_ele, build_ele.dadesTubHor.value[nHorSeleccionat].tubSuperior, 3)
        tubInferiorDelSeleccionat = getNumFromText(build_ele, build_ele.dadesTubHor.value[nHorSeleccionat].tubInferior, 3)


        #--------------------mostrar tubs---------------------------
        for posBarraHor in range(0, build_ele.IntegerISSelectorHor.value):
            if posBarraHor == nHorSeleccionat:
                guardar_valors_hor(build_ele, posBarraHor)
            ...
            forats = []
            colis = []
            if posBarraHor == tubSuperiorDelSeleccionat or posBarraHor == tubInferiorDelSeleccionat:
                encaixos, femelles, potes, forats, colis = get_valors_actuals(build_ele, posBarraHor)
            ...
            if build_ele.dadesTubHor.value[posBarraHor].TipusTub == "TUB A":
                ...
                TubHoritzontal = PP_IS_Horitzontal(random.random() * 3600,0, 50, 20,llargada, 1.5,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                            ...
                                            (build_ele.dadesTubHor.value[posBarraHor].retallFinal <= 0) or mostrarPestanyaSup, build_ele.dadesTubHor.value[posBarraHor].retallInicial <= 0,#PestanyaSuperior, PestanyaInferior,
                                            encaixos, True, #EncaixosPar))
                                            build_ele.dadesTubHor.value[posBarraHor].retallInicial, llargada - build_ele.dadesTubHor.value[posBarraHor].retallFinal, 1350) #)
            ...
            if posBarraHor == nHorSeleccionat:
                matrix_PosHor.SetValue(12, build_ele.desplX.value + 50)
                matrix_PosHor.SetValue(13, build_ele.desplY.value)
                matrix_PosHor.SetValue(14, 0 )
    ...
    #build_ele.SelectorPPAnt.value = build_ele.SelectorPPIS.value
    #build_ele.SelectorTDVAnt.value = nVer
    #build_ele.DistanciaEntreTDAnt.value = build_ele.DistanciaEntreTD.value
    #build_ele.BarraLlargadaAnt.value = build_ele.BarraLlargadaIS.value

    build_ele.TubAnt.value = nHorSeleccionat
    build_ele.RotarTubAnt.value = build_ele.RotarTub.value
    build_ele.actualitzaTubs.value = True
    build_ele.BarraLlargadaReal.value = build_ele.BarraLlargadaIS.value - build_ele.retallInicial.value - build_ele.retallFinal.value
    crearLlistaFullHoritzontals(build_ele)
...
def create_handles(build_ele ):
        ...
        #------------------ Create the handle
        #build_ele.BarraAmple.value, build_ele.BarraAltura.value, build_ele.BarraLlargadaIS.value, build_ele.BarraGruix.value,
...
def create_handles_linies(build_ele ):
        ...
        handle_list = [HandleProperties("BarraAmple",
                                        ...
                                        info_text= "Tamany"),
                        HandleProperties("BarraPosicio",
                                        AllplanGeo.Point3D(build_ele.PosicioXlinia.value  , build_ele.PosicioYlinia.value,  0 ),
                                        AllplanGeo.Point3D(0, 0, 0),
...
def create_handles_pos_Tubs(build_ele):
    """
    Create handles
    ...
    posY = build_ele.desplY.value
    if build_ele.posAutomatica.value:
        nHorSeleccionat = getNumFromText(build_ele, build_ele.SelectorTubHor.value, 3)
        posY = get_posicioY(build_ele, nHorSeleccionat)

    handle_list = [HandleProperties("ISllargada",
                                        #AllplanGeo.Point3D(build_ele.ISllargada.value -50 , build_ele.ISAmplada.value, 0 ),
                                        AllplanGeo.Point3D(build_ele.desplX.value , posY , 0),
                                        AllplanGeo.Point3D(0, 0, 0),
                                        [("desplX", HandleDirection.x_dir),
                                         ("desplY", HandleDirection.y_dir)], # type: ignore
                                        HandleDirection.xy_dir,
                                        info_text= "Posicio manual tub"),
                        HandleProperties("BarraPosicio",
                                        #AllplanGeo.Point3D(build_ele.PosicioXlinia.value, 0, build_ele.ISAmplada.value ),
                                        AllplanGeo.Point3D(0, 50, 0 ),
                                        AllplanGeo.Point3D(0, 0, 0),
                                        [("Punt0X", HandleDirection.x_dir),
                                         ("Punt0Y", HandleDirection.y_dir)],# type: ignore
                                        HandleDirection.xy_dir,
                                        info_text= "Posicio referencia"),
                        HandleProperties("BarraLlargada",
                                        #AllplanGeo.Point3D(build_ele.PosicioXlinia.value, 0, build_ele.ISAmplada.value ),
                                        AllplanGeo.Point3D(build_ele.desplX.value, build_ele.BarraLlargadaIS.value + posY, 0 ),
                                        AllplanGeo.Point3D(build_ele.desplX.value , posY, 0),
                                        [("BarraLlargada", HandleDirection.y_dir)],# type: ignore
                                        HandleDirection.xy_dir,
                                        info_text= "llargada manual tub")
                      ]
    ...
    '''
    handle_list = [HandleProperties("PosicioTub",
                                    AllplanGeo.Point3D(50 , 50, 0 ),
                                    AllplanGeo.Point3D(build_ele.desplX.value, build_ele.desplY.value , 0),
                                    [("desplX", HandleDirection.x_dir),
                                    ("desplY", HandleDirection.y_dir)],# type: ignore
                                    HandleDirection.xy_dir,
                                    False,
...
def crearLiniesAutomatitzacio(build_ele, doc, posBarraHor):
    group_elems_preview = []
    pointUbi = AllplanGeo.Point3D(build_ele.PosicioXlinia.value , build_ele.PosicioYlinia.value , 0 )
    lineX = create_polyline_interior(build_ele, 0, 0, build_ele.Amplelinia.value - 50, True, pointUbi)
    if lineX != []:
    ...
    pointUbi = AllplanGeo.Point3D(build_ele.PosicioXlinia.value, build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value , 0 )
    lineX = create_polyline_interior(build_ele, 0, 0, build_ele.Amplelinia.value - 50, True, pointUbi)
    if lineX != []:
        #group_elems.append(lineX[0])
        group_elems_preview.append(lineX[0])
...
def guardar_valors_hor(build_ele, j):
    ...
    while len(build_ele.dadesTubHor.value) <= j:
        ...
        if build_ele.TipusTUB.value == "TUB A":
            if build_ele.BarraLlargadaIS.value > 2150:
                ctypes.windll.user32.MessageBoxW(0, "la llargada del tub tipus TUB A no pot ser mes gran a 2150", 0)
                build_ele.BarraLlargadaIS.value = 2150
        elif build_ele.TipusTUB.value == "TUB E":
            if build_ele.BarraLlargadaIS.value > 5650:
                ctypes.windll.user32.MessageBoxW(0, "la llargada del tub tipus TUB E no pot ser mes gran a 5650", 0)
                build_ele.BarraLlargadaIS.value = 5650



        build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(Llargada = build_ele.BarraLlargadaIS.value)
        ...
        build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(retallFinal = build_ele.BarraLlargadaIS.value - build_ele.retallFinal.value)
...
def mostrar_valors_hor(build_ele, j):
    ...
    #build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(Ample = 40)
    #build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(Altura = 50)
    build_ele.BarraLlargadaIS.value = build_ele.dadesTubHor.value[j].Llargada
...
def set_values_tub_hor(build_ele, j, actual):
    ...
    if build_ele.direccioISInv.value:
        posinteriorVert = 700
        exteriorEsq = 2
        exteriorDre = 5
        build_ele.ExteriorEsquerra.value = 2
        build_ele.ExteriorDreta.value = 5

        tubEsq = "Tub 2"
        tubDre = "Tub 3"

    extremEsqGuardat = False
    extremDreGuardat = False
    ...
    while len(build_ele.dadesTubHor.value) <= j:
        if build_ele.direccioISInv.value:
            if len(build_ele.dadesTubHor.value) != 0 and len(build_ele.dadesTubHor.value) != 1:
                if (len(build_ele.dadesTubHor.value)-2) * 700 < build_ele.ISllargada.value - 50:
                    bobDadesGlobal = bobDadesGlobal._replace(desplX = build_ele.dadesTubHor.value[len(build_ele.dadesTubHor.value)-1].desplX + 700 )#+ build_ele.dadesTubHor.value[0].Ample)

                    bobDadesGlobal = bobDadesGlobal._replace(desplY = 0 )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(Llargada = build_ele.ISAmplada.value - 100 )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = True)
                    bobDadesGlobal = bobDadesGlobal._replace(RotarTub = True)
                    bobDadesGlobal = bobDadesGlobal._replace(tubInferior = str(tubinf))
                    bobDadesGlobal = bobDadesGlobal._replace(tubSuperior = str(tubsup))
                    bobDadesGlobal = bobDadesGlobal._replace(posAutomatica = True)
                    bobDadesGlobal = bobDadesGlobal._replace(TipusTub = "TUB E" )
                    if not extremEsqGuardat:
                        bobDadesGlobal = bobDadesGlobal._replace(desplX = ((len(build_ele.dadesTubHor.value)-2) * 650) )#+ build_ele.dadesTubHor.value[0].Ample)
                        build_ele.ExteriorEsquerra.value = len(build_ele.dadesTubHor.value)
                        extremEsqGuardat = True
                        bobDadesGlobal = bobDadesGlobal._replace(desplX = 0 )#+ build_ele.dadesTubHor.value[0].Ample)
                ...
                elif (len(build_ele.dadesTubHor.value)-2) * 700 >= build_ele.ISllargada.value - 50 and ((len(build_ele.dadesTubHor.value)-2)-1) * 700 < build_ele.ISllargada.value-50:
                    bobDadesGlobal = bobDadesGlobal._replace(desplX = build_ele.ISllargada.value - 100  )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(desplY = 0 )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(Llargada = build_ele.ISAmplada.value - 100 )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = True)
                    bobDadesGlobal = bobDadesGlobal._replace(RotarTub = True)
                    ...
                    if not extremDreGuardat:
                        build_ele.ExteriorDreta.value = len(build_ele.dadesTubHor.value)-1
                        extremDreGuardat = True
                    bobDadesGlobal = bobDadesGlobal._replace(Mostrar = True)
                    bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = True)
                    ...
                    if extremsVert <= 1:
                        ...
                        bobDadesGlobal = bobDadesGlobal._replace(Llargada = build_ele.ISllargada.value)
                        bobDadesGlobal = bobDadesGlobal._replace(posAutomatica = False)
                        #if actual == len(build_ele.dadesTubHor.value)-1:
                        #    build_ele.BarraLlargadaIS.value = build_ele.ISAmplada.value
                        bobDadesGlobal = bobDadesGlobal._replace(desplX = 0)
                        bobDadesGlobal = bobDadesGlobal._replace(desplY = 0)
                        if extremsVert == 1:
                            bobDadesGlobal = bobDadesGlobal._replace(desplX = build_ele.ISAmplada.value-50)
                            bobDadesGlobal = bobDadesGlobal._replace(posAutomatica = False)
                            #tubinf = "Tub "  + str(len(build_ele.dadesTubHor.value)-1)
                            #tubsup = "Tub "  + str(len(build_ele.dadesTubHor.value)  )
                    ...
                    '''
                    bobDadesGlobal = bobDadesGlobal._replace(tipusTubAuto = False )
                    bobDadesGlobal = bobDadesGlobal._replace(TipusTub = "TUB B" )
                    bobDadesGlobal = bobDadesGlobal._replace(RotarTub = False)
                    bobDadesGlobal = bobDadesGlobal._replace(tubInferior = tubEsq)
                    bobDadesGlobal = bobDadesGlobal._replace(tubSuperior = tubDre)
                    bobDadesGlobal = bobDadesGlobal._replace(desplY = posinteriorVert)
                    bobDadesGlobal = bobDadesGlobal._replace(desplX = 0)
                    bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = True)
                    bobDadesGlobal = bobDadesGlobal._replace(posAutomatica = True)

                    if posinteriorVert + 700 < build_ele.ISAmplada.value-100:
                        posinteriorVert += 700
                    else:
                        posinteriorVert = 700

                        tubEsq = "Tub " + str(int(getNumFromText(build_ele, tubEsq, 3))+1 )
                        tubDre = "Tub " + str(int(getNumFromText(build_ele, tubDre, 3))+1 )

            else:
                bobDadesGlobal = bobDadesGlobal._replace(TipusTub = "TUB A" )
                bobDadesGlobal = bobDadesGlobal._replace(desplX = 0  )#+ placement_mat[13])#+ build_ele.dadesTubHor.value[0].Ample)
                bobDadesGlobal = bobDadesGlobal._replace(desplY = 0 )#+ placement_mat[13])#+ build_ele.dadesTubHor.value[0].Ample)
                if len(build_ele.dadesTubHor.value) != 0:
                    bobDadesGlobal = bobDadesGlobal._replace(desplY = build_ele.ISAmplada.value - 50 )#+ placement_mat[13])#+ build_ele.dadesTubHor.value[0].Ample)
                bobDadesGlobal = bobDadesGlobal._replace(Llargada = build_ele.ISllargada.value  - 50)#+ build_ele.dadesTubHor.value[0].Ample)
                bobDadesGlobal = bobDadesGlobal._replace(posAutomatica = False)
                bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = False)
                ...
                if actual == 0:
                    ...
                    build_ele.BarraLlargadaIS.value = build_ele.ISllargada.value - 50
                ...
                if len(build_ele.dadesTubHor.value) * 700 < build_ele.ISAmplada.value:
                    ...
                    if extremsVert <= 1:
                        ...
                        if posinteriorVert + 700 < build_ele.ISllargada.value-100:
                            ...
                            tubinf = "Tub " + str(int(getNumFromText(build_ele, tubinf, 3))+1 )
                            tubsup = "Tub " + str(int(getNumFromText(build_ele, tubsup, 3))+1 )
            else:
                bobDadesGlobal = bobDadesGlobal._replace(desplY = len(build_ele.dadesTubHor.value) * 700 )#+ placement_mat[13])#+ build_ele.dadesTubHor.value[0].Ample)
                bobDadesGlobal = bobDadesGlobal._replace(Llargada = build_ele.ISllargada.value )#+ build_ele.dadesTubHor.value[0].Ample)
                bobDadesGlobal = bobDadesGlobal._replace(posAutomatica = True)
                bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = True)
                bobDadesGlobal = bobDadesGlobal._replace(tubInferior = "Tub "+str(exteriorEsq))
    ...
    if build_ele.direccioISInv.value:
        build_ele.dadesTubHor.value[0] = build_ele.dadesTubHor.value[0]._replace(tubInferior = "Tub "+str(build_ele.ExteriorEsquerra.value))
        build_ele.dadesTubHor.value[0] = build_ele.dadesTubHor.value[0]._replace(tubSuperior = "Tub "+str(build_ele.ExteriorDreta.value))
        build_ele.dadesTubHor.value[1] = build_ele.dadesTubHor.value[1]._replace(tubInferior = "Tub "+str(build_ele.ExteriorEsquerra.value))
        build_ele.dadesTubHor.value[1] = build_ele.dadesTubHor.value[1]._replace(tubSuperior = "Tub "+str(build_ele.ExteriorDreta.value))

        build_ele.dadesTubHor.value[0] = build_ele.dadesTubHor.value[0]._replace(posAutomatica = False)
        build_ele.dadesTubHor.value[0] = build_ele.dadesTubHor.value[0]._replace(LlargadaAut = False)
        build_ele.dadesTubHor.value[1] = build_ele.dadesTubHor.value[1]._replace(posAutomatica = False)
        build_ele.dadesTubHor.value[1] = build_ele.dadesTubHor.value[1]._replace(LlargadaAut = False)
        if actual == 0 or actual == 1:
            build_ele.desplX.value = 0
            build_ele.posAutomatica.value = False
            build_ele.RotarTub.value = False
            build_ele.llargadaAutomatica.value = False
            build_ele.SelectorTubInferiorHor.value = "Tub "+str(build_ele.ExteriorEsquerra.value)
            build_ele.SelectorTubSuperiorHor.value = "Tub "+str(build_ele.ExteriorDreta.value)#SelectorTubSuperiorHor
            build_ele.retallInicial.value = 0
            build_ele.retallFinal.value = 0#SelectorTubSuperiorHor

    #fer nomes quan es fa recalcul
    if build_ele.refactor.value and not build_ele.direccioISInv.value:
...
def set_llargadaAut(build_ele, posBarraHor, nHorSeleccionat, esVertical):
    if esVertical:
        nTubInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[posBarraHor].tubInferior, 3)
    ...
    if posSup-posInf > 0:
        #if posBarraHor == nHorSeleccionat:
        #    build_ele.BarraLlargadaIS.value = posSup - posInf
        #    build_ele.retallFinal.value = 0
        #else:
        build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(Llargada = posSup - posInf)
        build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(retallFinal = 0)
    else:
...
def get_llargadaAut(build_ele, posBarraHor, nHorSeleccionat, esVertical):
    if esVertical:
        nTubInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[posBarraHor].tubInferior, 3)
        if not build_ele.dadesTubHor.value[nTubInf].Mostrar or not HorEsTubDePos(build_ele, nTubInf, build_ele.dadesTubHor.value[posBarraHor].desplX):
            posInf = get_llargadaAutoInfMesProxima(build_ele, posBarraHor, nHorSeleccionat, len(build_ele.dadesTubHor.value)-1)
            if not build_ele.dadesTubHor.value[posBarraHor].posAutomatica:
                if posBarraHor == nHorSeleccionat:
                    build_ele.desplY.value = posInf
                else:
                    build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(desplY = posInf)
        else:
            if not build_ele.dadesTubHor.value[posBarraHor].posAutomatica:
                posInf = build_ele.dadesTubHor.value[posBarraHor].desplY
            else:
                posInf = build_ele.dadesTubHor.value[posBarraHor].desplY + build_ele.dadesTubHor.value[nTubInf].desplY
        if not build_ele.dadesTubHor.value[posBarraHor].posAutomatica:
            #posInf += build_ele.dadesTubHor.value[nTubInf].desplY + build_ele.dadesTubHor.value[nTubInf].Altura
            posInf += build_ele.dadesTubHor.value[nTubInf].desplY #+ build_ele.dadesTubHor.value[nTubInf].Altura
        nTubSup = getNumFromText(build_ele, build_ele.dadesTubHor.value[posBarraHor].tubSuperior, 3)
        if not build_ele.dadesTubHor.value[nTubSup].Mostrar or not HorEsTubDePos(build_ele, nTubSup, build_ele.dadesTubHor.value[posBarraHor].desplX):

            posSup = get_llargadaAutoSupMesProxima(build_ele, posBarraHor, nHorSeleccionat)
        else:
            posSup = build_ele.dadesTubHor.value[nTubSup].desplY
    else:
        nTubInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[posBarraHor].tubInferior, 3)
    ...
    if build_ele.dadesTubHor.value[posBarraHor].posAutomatica and build_ele.dadesTubHor.value[posBarraHor].LlargadaAut:
        ...
        return build_ele.dadesTubHor.value[posBarraHor].Llargada - build_ele.dadesTubHor.value[posBarraHor].retallInicial - posInf


def get_llargadaAutoSupMesProxima(build_ele, posBarraHor, nHorSeleccionat):
    posYmesGran = 0
    ntubMesProximAlSup = getNumFromText(build_ele, build_ele.dadesTubHor.value[posBarraHor].tubSuperior, 3)
    xMesPetita = build_ele.ISllargada.value
    xMesGran = 0
    for ntub in range(len(build_ele.dadesTubHor.value)):
        if not build_ele.dadesTubHor.value[ntub].RotarTub:
            if HorEsTubDePos(build_ele, ntub, build_ele.dadesTubHor.value[posBarraHor].desplX):#build_ele.PosicioXlinia.value):
                if posYmesGran < build_ele.dadesTubHor.value[ntub].desplY:
                    posYmesGran = build_ele.dadesTubHor.value[ntub].desplY
                    #llargada = build_ele.dadesTubHor.value[ntub].desplY - build_ele.PosicioYlinia.value
                    ntubMesProximAlSup = ntub
        else:
            if build_ele.dadesTubHor.value[ntub].desplX < xMesPetita:
                xMesPetita = build_ele.dadesTubHor.value[ntub].desplX
            if build_ele.dadesTubHor.value[ntub].desplX > xMesGran:
                xMesGran = build_ele.dadesTubHor.value[ntub].desplX

    if posYmesGran == 0:
        if build_ele.dadesTubHor.value[posBarraHor].desplX == xMesPetita or build_ele.dadesTubHor.value[posBarraHor].desplX == xMesGran:
            if posBarraHor == nHorSeleccionat:
                build_ele.MostrarTub.value = False
            else:
                build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(Mostrar = False)
    return build_ele.dadesTubHor.value[ntubMesProximAlSup].desplY

def get_llargadaAutoInfMesProxima(build_ele, posBarraHor, nHorSeleccionat, lenDadesTubHor):
    posYmesGran = 7000
    ntubMesProximAlInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[posBarraHor].tubInferior, 3)
    xMesPetita = build_ele.ISllargada.value
    xMesGran = 0
    posYTub = 0
    if build_ele.dadesTubHor.value[posBarraHor].posAutomatica:
        posYTub = build_ele.dadesTubHor.value[ntubMesProximAlInf].desplY + build_ele.dadesTubHor.value[posBarraHor].desplY
    else:
        posYTub = build_ele.dadesTubHor.value[posBarraHor].desplY
        if posYTub < 0 : posYTub = 0
    for ntub in range(lenDadesTubHor):
        if not build_ele.dadesTubHor.value[ntub].RotarTub:
            if HorEsTubDePos(build_ele, ntub, build_ele.dadesTubHor.value[posBarraHor].desplX):#build_ele.PosicioXlinia.value):
                if posYmesGran > build_ele.dadesTubHor.value[ntub].desplY and posYTub < build_ele.dadesTubHor.value[ntub].desplY :
                    posYmesGran = build_ele.dadesTubHor.value[ntub].desplY
                    #llargada = build_ele.dadesTubHor.value[ntub].desplY - build_ele.PosicioYlinia.value
                    ntubMesProximAlInf = ntub
        else:
            if build_ele.dadesTubHor.value[ntub].desplX < xMesPetita:
                xMesPetita = build_ele.dadesTubHor.value[ntub].desplX
            if build_ele.dadesTubHor.value[ntub].desplX > xMesGran:
    ...
    if posYmesGran == 7000 :
        if build_ele.dadesTubHor.value[posBarraHor].desplX == xMesPetita or build_ele.dadesTubHor.value[posBarraHor].desplX == xMesGran:
            if posBarraHor == nHorSeleccionat:
                ...
                build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(Mostrar = False)
    return build_ele.dadesTubHor.value[ntubMesProximAlInf].desplY

def HorEsTubDePos(build_ele, ntub, posX):
    posEsqInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[ntub].tubInferior, 3)
    deplsXEsq = build_ele.dadesTubHor.value[posEsqInf].desplX
    if build_ele.dadesTubHor.value[ntub].Mostrar and \
            posX > deplsXEsq + build_ele.dadesTubHor.value[ntub].desplX  + build_ele.dadesTubHor.value[ntub].retallInicial and \
            posX <= deplsXEsq + build_ele.dadesTubHor.value[ntub].desplX + build_ele.dadesTubHor.value[ntub].retallInicial + build_ele.dadesTubHor.value[ntub].Llargada  - build_ele.dadesTubHor.value[ntub].retallFinal:
            return True
...
def crear_forat_auto(build_ele, doc, nHorSeleccionat, tipusForat):
    ...
    flagPosXModificada = False
    flagPosYModificada = False
    if  build_ele.PosicioXlinia.value < 0:
        build_ele.Amplelinia.value -= abs(build_ele.PosicioXlinia.value)
        build_ele.PosicioXlinia.value = 0
        flagPosXModificada = True
    if  build_ele.PosicioYlinia.value < 0:
        build_ele.Llargadalinia.value -= abs(build_ele.PosicioYlinia.value)
        build_ele.PosicioYlinia.value = 0
        flagPosYModificada = True
    detectar_colisio_tubs(build_ele, doc, nHorSeleccionat, nCreats, llistaTubs)

    posicionsNousTubs = detectar_Tubs_sobreposats(build_ele, nHorSeleccionat, llistaTubs)

    if flagPosXModificada:
        build_ele.Amplelinia.value += 50
        build_ele.PosicioXlinia.value = -50
    #if  flagPosYModificada:
    #    build_ele.Llargadalinia.value += 50
    #    build_ele.PosicioXlinia.value = -50



def detectar_Tubs_sobreposats(build_ele, nHorSeleccionat, posicionsNousTubs):
    ...
    for NouTub in posicionsNousTubs: #.values():
        if posicionsNousTubs[NouTub] != None:
            ...
            trobat, posicions = detectar_tub_sobre_tub(build_ele, posNouTub, nHorSeleccionat )
            if trobat :
                for posicio in posicions:
                    tubsTrobatsSobreposats.append(posicio)
                    ...
                    #posicionsNousTubs[NouTub] = posicio
                    #eliminar ja existent
                    if posicio == nHorSeleccionat:
                        build_ele.MostrarTub.value = False
                    else:
                        build_ele.dadesTubHor.value[posicio] = build_ele.dadesTubHor.value[posicio]._replace(Mostrar = False)
                    #posicionsNousTubs[NouTub] = posicio
...
def detectar_tub_sobre_tub(build_ele, posTubNou, nHorSeleccionat ):
    ...
    llargadaNou = tubNou.Llargada#get_llargadaAut(build_ele,posTubNou, tubNou.RotarTub)
    posXAbsolutaNou = tubNou.desplX
    posYAbsolutaNou = tubNou.desplY
    posXFinalAbsolutaNou = tubNou.desplX + llargadaNou - tubNou.retallFinal
    posYFinalAbsolutaNou = tubNou.desplY + llargadaNou - tubNou.retallFinal
    if tubNou.posAutomatica:
        if tubNou.LlargadaAut:
            llargadaNou = build_ele.dadesTubHor.value[getNumFromText(build_ele, tubNou.tubSuperior, 3)].desplY - build_ele.dadesTubHor.value[getNumFromText(build_ele, tubNou.tubInferior, 3)].desplY
        posXAbsolutaNou = tubNou.desplX + build_ele.dadesTubHor.value[getNumFromText(build_ele, tubNou.tubInferior, 3)].desplX
        posYAbsolutaNou = tubNou.desplY + build_ele.dadesTubHor.value[getNumFromText(build_ele, tubNou.tubInferior, 3)].desplY
        posXFinalAbsolutaNou = posXAbsolutaNou + llargadaNou - tubNou.retallFinal
        posYFinalAbsolutaNou = posYAbsolutaNou + llargadaNou - tubNou.retallFinal
    ...
    for nTub in range(0, build_ele.IntegerISSelectorHor.value):
        ...
        llargadaExistent = tubExistent.Llargada #get_llargadaAut(build_ele,nTub, tubExistent.RotarTub)
        posXAbsolutaExistent = tubExistent.desplX
        posYAbsolutaExistent = tubExistent.desplY
        posXAbsolutaExistent = tubExistent.desplX + llargadaExistent - tubExistent.retallFinal
        posYAbsolutaExistent = tubExistent.desplY + llargadaExistent - tubExistent.retallFinal
        posXFinalAbsolutaExistent = posXAbsolutaExistent + llargadaExistent- tubExistent.retallFinal
        posYFinalAbsolutaExistent = posYAbsolutaExistent + llargadaExistent- tubExistent.retallFinal

        if tubExistent.posAutomatica:
            posXAbsolutaExistent = tubExistent.desplX + build_ele.dadesTubHor.value[getNumFromText(build_ele, tubExistent.tubInferior, 3)].desplX
            posYAbsolutaExistent = tubExistent.desplY + build_ele.dadesTubHor.value[getNumFromText(build_ele, tubExistent.tubInferior, 3)].desplY
            posXFinalAbsolutaExistent = posXAbsolutaExistent + llargadaExistent- tubExistent.retallFinal
            posYFinalAbsolutaExistent = posYAbsolutaExistent + llargadaExistent- tubExistent.retallFinal
        ...
        if nTub != posTubNou:
            if tubNou.RotarTub: #Vertical
                if tubNou.RotarTub == tubExistent.RotarTub and \
                        ...
                        posYAbsolutaNou <= posYAbsolutaExistent and \
                        posYFinalAbsolutaNou >= posYFinalAbsolutaExistent and \
                        tubNou.Llargada - tubNou.retallInicial - tubNou.retallFinal >= \
                        tubExistent.Llargada - tubExistent.retallInicial - tubExistent.retallFinal :
                    listSobreposats.append(nTub)
                    #return True, nTub
            else:#Horitzontal
                if tubNou.RotarTub == tubExistent.RotarTub and \
                        tubNou.desplX <= tubExistent.desplX and \
                        posYAbsolutaNou == posYAbsolutaExistent and \
                        posXFinalAbsolutaNou >= posXFinalAbsolutaExistent and \
                        tubNou.Llargada - tubNou.retallInicial - tubNou.retallFinal >= \
                        tubExistent.Llargada - tubExistent.retallInicial - tubExistent.retallFinal :
                    listSobreposats.append(nTub)
                    #return True, nTub
...
def crear_barres_quadrat(build_ele, doc, nHorSeleccionat, costats):
    ...
    if len(costats) == 1:
        ...
        if "Esq" in costats and "Dre" in costats:
            ...
            print("2 Vert")
            posicionsNousTubs["Esq"] = crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats, tipus=1)
            posicionsNousTubs["Dre"] = crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats,posAutomatica=True, tipus=2)
    ...
    #posicionsNousTubs = detectar_Tubs_sobreposats(build_ele, nHorSeleccionat, posicionsNousTubs)
...
def crear_barres_quadrat3(build_ele, doc, nHorSeleccionat, costats):
    ...
    if len(costats) == 0:
        ...
        if "Esq" in costats:
            ...
            posicio_llarga_barra_vertical(build_ele, doc, nHorSeleccionat, posicionsNousTubs["Esq"])
            posicionsNousTubs["Sup"] = crear_barra_sup_Forat(build_ele, nHorSeleccionat, costats, tubInf = posicionsNousTubs["Esq"], posAutomatica = True)
            build_ele.dadesTubHor.value[posicionsNousTubs["Sup"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Sup"]]._replace(Llargada =  build_ele.dadesTubHor.value[posicionsNousTubs["Sup"]].Llargada - 50)
            posicionsNousTubs["Inf"] = crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats, tubInf = posicionsNousTubs["Esq"], posAutomatica = True)
            build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(Llargada =  build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]].Llargada - 50)
        elif "Sup" in costats:
            print("1 Hor 2 Vert")
            posicionsNousTubs["Dre"] = crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats, posAutomatica=True, llargadaAutomatica=True)
            posicio_llarga_barra_vertical(build_ele, doc, nHorSeleccionat, posicionsNousTubs["Dre"])
            llargadaAct = get_llargadaAut(build_ele, posicionsNousTubs["Dre"], nHorSeleccionat, True)
            ...
            posicionsNousTubs["Esq"] = crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats, posAutomatica=True, llargadaAutomatica=True)
            posicio_llarga_barra_vertical(build_ele, doc, nHorSeleccionat, posicionsNousTubs["Esq"])
            llargadaAct = get_llargadaAut(build_ele, posicionsNousTubs["Esq"], nHorSeleccionat, True)
        ...
        if "Esq" in costats and "Dre" in costats:
            ...
            llargadaAct = get_llargadaAut(build_ele, posicionsNousTubs["Dre"], nHorSeleccionat, True)
            build_ele.dadesTubHor.value[posicionsNousTubs["Dre"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Dre"]]._replace(LlargadaAut = False)
            build_ele.dadesTubHor.value[posicionsNousTubs["Dre"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Dre"]]._replace(Llargada = llargadaAct )
            posicionsNousTubs["Inf"] = crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats, tubSup = posicionsNousTubs["Dre"], posAutomatica=False, llargadaAutomatica=False)
            extremEsq = detectar_vertical_anteriorExtrem(build_ele, posicionsNousTubs["Inf"], nHorSeleccionat, build_ele.PosicioXlinia.value + build_ele.Amplelinia.value )
            if extremEsq != None:
                build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(posAutomatica = True)
                build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(tubInferior = "Tub " + str(extremEsq))
                build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(LlargadaAut = True)

                llargadaAct = get_llargadaAut(build_ele, posicionsNousTubs["Inf"], nHorSeleccionat, True)
            #posicio_llarga_barra_horitzontal(build_ele, doc, nHorSeleccionat, posicionsNousTubs["Inf"])
            ...
            posicio_llarga_barra_vertical(build_ele, doc, nHorSeleccionat, posicionsNousTubs["Esq"])
            llargadaAct = get_llargadaAut(build_ele, posicionsNousTubs["Esq"], nHorSeleccionat, True)
            #posicioAct = get_posCorrecta?()
            #build_ele.dadesTubHor.value[posicionsNousTubs["Esq"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Esq"]]._replace(Llargada = llargadaAct)
            posicionsNousTubs["Sup"] = crear_barra_sup_Forat(build_ele, nHorSeleccionat, costats, tubInf = posicionsNousTubs["Esq"], posAutomatica=True, llargadaAutomatica=False)
            build_ele.dadesTubHor.value[posicionsNousTubs["Sup"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Sup"]]._replace(Llargada = build_ele.dadesTubHor.value[posicionsNousTubs["Sup"]].Llargada - 50)
        elif "Dre" in costats and "Sup" in costats:
            print("1 Hor 1 Vert")
            ...
            build_ele.dadesTubHor.value[posicionsNousTubs["Esq"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Esq"]]._replace(LlargadaAut = False)
            build_ele.dadesTubHor.value[posicionsNousTubs["Esq"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Esq"]]._replace(Llargada = llargadaAct )
            posicionsNousTubs["Inf"] = crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats, tubInf = posicionsNousTubs["Esq"], posAutomatica= True)#, posAutomatica=True, llargadaAutomatica=True)
            build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(Llargada = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]].Llargada - 100)
            extremDre = detectar_vertical_seguentExtrem(build_ele, posicionsNousTubs["Inf"], nHorSeleccionat, build_ele.PosicioXlinia.value  )
            if extremDre != None:
                posFinal = build_ele.dadesTubHor.value[extremDre].desplX
                if extremDre != build_ele.ExteriorDreta.value:
                    build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(Llargada = posFinal - build_ele.PosicioXlinia.value )
                else:
                    build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(Llargada = posFinal - build_ele.PosicioXlinia.value - 100)
                #build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(posAutomatica = True)
                #if extremDre != build_ele.ExteriorDreta.value:
                #    build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(LlargadaAut = True)
                #    build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(tubSuperior = "Tub " + str(extremDre))
                #else:
                #    build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]] = build_ele.dadesTubHor.value[posicionsNousTubs["Inf"]]._replace(tubSuperior = "Tub " + str(extremDre))
                #llargadaAct = get_llargadaAut(build_ele, posicionsNousTubs["Inf"], nHorSeleccionat, True)
        nTubsquadrat = 2
...
def posicio_llarga_barra_horitzontal_Esq_Extrem(build_ele, doc, nHorSeleccionat, noutub):
    '''
    detectar Vertical anterior per Sup i Inf #es crida cada vegada que es crea un tub Hor Sup o Inf
    '''
    verticalMesProper = detectar_vertical_anteriorExtrem(build_ele, noutub, nHorSeleccionat)
    verticalMesProperDre = detectar_vertical_seguent(build_ele, noutub, nHorSeleccionat)
    if verticalMesProper != None and build_ele.dadesTubHor.value[noutub].posAutomatica:
        asignar_barra_inferior(build_ele, noutub, nHorSeleccionat, verticalMesProper)
    if verticalMesProperDre != None and build_ele.dadesTubHor.value[noutub].LlargadaAut:
        ...
        if x not in llistaTubs.values():
            ...
            if build_ele.dadesTubHor.value[x].RotarTub:#tub Vertical
                if posHoritzontal >= build_ele.PosicioXlinia.value and posHoritzontal < build_ele.PosicioXlinia.value + build_ele.Amplelinia.value :
                    if  (posVerticalSup <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value and posVerticalSup > build_ele.PosicioYlinia.value ) and posVertical < build_ele.PosicioYlinia.value :
                        ...
                        if nHorSeleccionat == x :
                            if build_ele.PosicioYlinia.value + 50 < build_ele.BarraLlargadaIS.value:
                                build_ele.llargadaAutomatica.value = False
                                build_ele.BarraLlargadaIS.value =  build_ele.PosicioYlinia.value - posVertical - 50
                                if x == build_ele.ExteriorEsquerra.value or x == build_ele.ExteriorDreta.value:
                                    ...
                                    build_ele.BarraLlargadaIS.value = build_ele.PosicioYlinia.value + 50


                        else:
                            if build_ele.PosicioYlinia.value + 50 < build_ele.dadesTubHor.value[x].Llargada:
                        ...
                        if nHorSeleccionat == x:
                            build_ele.llargadaAutomatica.value = False
                            build_ele.BarraLlargadaIS.value = posVerticalSup -  posVerticalInf - 50 #posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value)
                        ...
                        if nHorSeleccionat == x:
                            build_ele.llargadaAutomatica.value = False
                            build_ele.BarraLlargadaIS.value = posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value)
                            build_ele.posAutomatica.value = False
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallInicial = build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value - posVerticalInf)
                        ...
                        if build_ele.PosicioYlinia.value != 0:
                            ...
                            posVerticalInfAux = get_llargadaAutoInfMesProxima(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat, lenDadesTubHorAct)
                ...
                if ( posVertical >= build_ele.PosicioYlinia.value and posVertical + 50   <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value):
                    if posHoritzontal + build_ele.dadesTubHor.value[x].Llargada >= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value and \
                        posHoritzontalEsq + build_ele.dadesTubHor.value[x].retallInicial < build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                        ...
                        if nHorSeleccionat == x:
                            ...
                            if build_ele.posAutomatica.value:
                                ...
                                #build_ele.BarraLlargadaIS.value = posHoritzontalDre - (build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) - 50
                            else:
                                build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallInicial = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value  - posHoritzontalEsq - 50)
                                build_ele.retallInicial.value = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value  - posHoritzontalEsq - 50
                        ...
                        if nHorSeleccionat == x:
                            if not build_ele.llargadaAutomatica.value:
                                ...
                                #build_ele.BarraLlargadaIS.value = build_ele.PosicioXlinia.value - posHoritzontal
...
def detectar_vertical_anteriorExtrem(build_ele, x, nHorSeleccionat, posX = -50, posY = 0):
    '''
    x = tub Horitzontal
    ...
    for posTub in range(0, len(build_ele.dadesTubHor.value)):
        if build_ele.dadesTubHor.value[posTub].RotarTub:
            if build_ele.dadesTubHor.value[posTub].desplX < posX:
                ...
                #print( str(build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriY) + " <= " + str(posY) + " and " +  str(build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada + sumatoriY) + " > " +  str(posY))
                #print( str(posTub) + ": " +str((build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriY)  <= posY ) + " and " +  str(build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada + sumatoriY > posY))
                if build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriY <= posY  and \
                        build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada + sumatoriY > posY :
                    if anteriorMesProper == None:
                        anteriorMesProper = posTub
                    elif build_ele.dadesTubHor.value[posTub].desplX < build_ele.dadesTubHor.value[anteriorMesProper].desplX:
                        anteriorMesProper = posTub
...
def detectar_vertical_seguentExtrem(build_ele, x, nHorSeleccionat, posX = 0, posY = 0):
    '''
    x = tub Horitzontal
...
def detectar_horitzontal_anterior(build_ele, x, nHorSeleccionat, posX = 0 , posY = 0):
    ...
    anteriorMesProper = None
    for posTub in range(0, len(build_ele.dadesTubHor.value)-1):
        if not build_ele.dadesTubHor.value[posTub].RotarTub:
            if get_posicioY(build_ele, posTub) < posY:
                ...
                if build_ele.dadesTubHor.value[posTub].desplX + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriX <= posX  and \
                    build_ele.dadesTubHor.value[posTub].desplX + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada + sumatoriX > posX :
                    if anteriorMesProper == None:
                        anteriorMesProper = posTub
...
def detectar_tipus_tub(build_ele, doc, posTub ,nHorSeleccionat, encaixInf = True, encaixSup = True):
    ...
    if build_ele.dadesTubHor.value[posTub].tipusTubAuto:
        if ((not encaixAmbTubInf and build_ele.dadesTubHor.value[posTub].retallInicial == 0) and \
            ...
            build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB E")#amb pestanyes/mascles
                ...
                build_ele.dadesTubHor.value[posTub].LlargadaAut:
            build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB E")#amb pestanyes/mascles
        elif not build_ele.dadesTubHor.value[posTub].posAutomatica and \
                build_ele.dadesTubHor.value[posTub].retallInicial == 0 and \
                build_ele.dadesTubHor.value[posTub].desplY == 0:
            build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB D")#sense pestanyes/mascles

        elif not encaixAmbTubInf and not encaixAmbTubSup :
            build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB E")#amb pestanyes/mascles
            sensePestanyes = True
        else:
            build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB E")#amb pestanyes/mascles
    ...
    if not build_ele.dadesTubHor.value[posTub].posAutomatica and \
                build_ele.dadesTubHor.value[posTub].retallInicial <= 0 and \
                build_ele.dadesTubHor.value[posTub].desplY <= 0 and (llargada <= 1350 ):
        if nHorSeleccionat == posTub:
            build_ele.TipusTUB.value = "TUB D"
        build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB D")
...
def detectar_es_posible(build_ele, doc, posTub ,nHorSeleccionat, encaixInf = True, encaixSup = True, mostratError = False):
    ...
    if not mostratError and teError :
        ...
        print("Tipus de tub Impossible per mesures")
        mostratError = True
        ...
        if build_ele.dadesTubHor.value[posTub].RotarTub:
            if build_ele.dadesTubHor.value[posTub].tubInferior == "Tub " +str(posAnt):
                if build_ele.dadesTubHor.value[posNou].desplX + build_ele.dadesTubHor.value[posNou].retallInicial < build_ele.dadesTubHor.value[posTub].desplX and build_ele.dadesTubHor.value[posNou].desplX + build_ele.dadesTubHor.value[posNou].retallInicial + build_ele.dadesTubHor.value[posNou].Llargada > build_ele.dadesTubHor.value[posTub].desplX:#posicio esta entre inici i final de tub nou(posNou)
                    if nHorSeleccionat == posTub:
                        build_ele.SelectorTubInferior.value = "Tub " +str(posNou)
            ...
            if build_ele.dadesTubHor.value[posTub].tubSuperior == "Tub " +str(posAnt):
                if build_ele.dadesTubHor.value[posNou].desplX + build_ele.dadesTubHor.value[posNou].retallInicial < build_ele.dadesTubHor.value[posTub].desplX and build_ele.dadesTubHor.value[posNou].desplX + build_ele.dadesTubHor.value[posNou].retallInicial + build_ele.dadesTubHor.value[posNou].Llargada > build_ele.dadesTubHor.value[posTub].desplX:#posicio esta entre inici i final de tub nou(posNou)
                    if nHorSeleccionat == posTub:
                        build_ele.SelectorTubSuperior.value = "Tub " +str(posNou)
...
def crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats, tubInf = 0, tubSup = 1, posAutomatica = False, llargadaAutomatica = False, tipus = 1):
    ...
    if llargada > build_ele.ISAmplada.value:
        llargada = build_ele.ISAmplada.value - 50
    ...
    if posAutomatica:
        ...
        build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(desplX = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value)
        if tipus == 3:
...
def crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats, tubInf = 0, tubSup = 1, posAutomatica = False, llargadaAutomatica = False, tipus = 1):
    ...
    if build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value > build_ele.ISAmplada.value:
        #llargada = build_ele.Llargadalinia.value - (build_ele.ISAmplada.value - build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value)
        llargada =build_ele.ISAmplada.value - build_ele.PosicioYlinia.value
        #si tub superior esta tallat o si no arriba a la posicio X indicada
        #llargada = llargada fins el anteriorHOritzontal Visible
    if llargada > build_ele.ISAmplada.value:
        llargada = build_ele.ISAmplada.value
...
def copiar_valors_Vert(build_ele,nVer):
    if len(build_ele.DadesVertCopy.value) == 0:
        #DADES TDV UNIQUES
        TDCollectionDadesVert = collections.namedtuple('namedtuple', 'nBarraGuardada Mostrar Ample Altura LlargadaAut Llargada Gruix '+
                                                    'desplX desplY ' +
                                                    'mostrarLiniaA desplLinA mostrarLiniaB desplLinB ' +
                                                    'IsUseGlobalProp FounColor BarraLayer '
                                                    'Ample_forat_femella Altura_forat_femella Separacio_forat_femella '+
                                                    ...
                                                    'tubInferior tubSuperior posAutomatica ' +
                                                    'retallInicial retallFinal')
        bobDadesGlobal = TDCollectionDadesVert( nBarraGuardada           =   nVer,
                                                Mostrar                  =   build_ele.dadesTubHor.value[nVer].Mostrar                 ,
                                                Ample                    =   build_ele.dadesTubHor.value[nVer].Ample                   ,
                                                Altura                   =   build_ele.dadesTubHor.value[nVer].Altura                  ,
                                                LlargadaAut              =   build_ele.dadesTubHor.value[nVer].Llargada                ,
                                                Llargada                 =   build_ele.dadesTubHor.value[nVer].LlargadaAut             ,
                                                Gruix                    =   build_ele.dadesTubHor.value[nVer].Gruix                   ,
                                                desplX                   =   build_ele.dadesTubHor.value[nVer].desplX                  ,
                                                desplY                   =   build_ele.dadesTubHor.value[nVer].desplY                  ,
                                                mostrarLiniaA            =   build_ele.dadesTubHor.value[nVer].mostrarLiniaA           ,
                                                desplLinA                =   build_ele.dadesTubHor.value[nVer].desplLinA               ,
                                                mostrarLiniaB            =   build_ele.dadesTubHor.value[nVer].mostrarLiniaB           ,
                                                desplLinB                =   build_ele.dadesTubHor.value[nVer].desplLinB               ,
                                                IsUseGlobalProp          =   build_ele.dadesTubHor.value[nVer].IsUseGlobalProp         ,
                                                FounColor                =   build_ele.dadesTubHor.value[nVer].FounColor               ,
                                                BarraLayer               =   build_ele.dadesTubHor.value[nVer].BarraLayer              ,
                                                Ample_forat_femella      =   build_ele.dadesTubHor.value[nVer].Ample_forat_femella     ,
                                                Altura_forat_femella     =   build_ele.dadesTubHor.value[nVer].Altura_forat_femella    ,
                                                Separacio_forat_femella  =   build_ele.dadesTubHor.value[nVer].Separacio_forat_femella ,
                                                PestanyaSuperior         =   build_ele.dadesTubHor.value[nVer].PestanyaSuperior        ,
                                                PestanyaInferior         =   build_ele.dadesTubHor.value[nVer].PestanyaInferior        ,
                                                PestanyesInv             =   build_ele.dadesTubHor.value[nVer].PestanyesInv            ,
                                                esProvisional            =   build_ele.dadesTubHor.value[nVer].esProvisional           ,
                                                provSupEsq               =   build_ele.dadesTubHor.value[nVer].provSupEsq              ,
                                                provSupDre               =   build_ele.dadesTubHor.value[nVer].provSupDre              ,
                                                provInfEsq               =   build_ele.dadesTubHor.value[nVer].provInfEsq              ,
                                                provInfDre               =   build_ele.dadesTubHor.value[nVer].provInfDre              ,
                                                provInfCen               =   build_ele.dadesTubHor.value[nVer].provInfCen              ,
                                                linia                    =   build_ele.dadesTubHor.value[nVer].linia                   ,
                                                layer                    =   build_ele.dadesTubHor.value[nVer].layer                   ,
                                                TipusTub                 =   build_ele.dadesTubHor.value[nVer].TipusTub                ,
                                                RotarTub                 =   build_ele.dadesTubHor.value[nVer].tipusTubAuto            ,
                                                tipusTubAuto             =   build_ele.dadesTubHor.value[nVer].RotarTub                ,
                                                tubInferior              =   build_ele.dadesTubHor.value[nVer].tubInferior             ,
                                                tubSuperior              =   build_ele.dadesTubHor.value[nVer].tubSuperior             ,
                                                posAutomatica            =   build_ele.dadesTubHor.value[nVer].posAutomatica           ,
                                                retallInicial            =   build_ele.dadesTubHor.value[nVer].retallInicial           ,
                                                retallFinal              =   build_ele.dadesTubHor.value[nVer].retallFinal             ,
                                                )
        build_ele.DadesVertCopy.value.append(bobDadesGlobal)
    else:
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(nBarraGuardada =           nVer                 )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Mostrar =                  build_ele.dadesTubHor.value[nVer].Mostrar                 )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Ample =                    build_ele.dadesTubHor.value[nVer].Ample                   )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Altura =                   build_ele.dadesTubHor.value[nVer].Altura                  )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Llargada =                 build_ele.dadesTubHor.value[nVer].Llargada                )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(LlargadaAut =              build_ele.dadesTubHor.value[nVer].LlargadaAut             )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Gruix =                    build_ele.dadesTubHor.value[nVer].Gruix                   )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(desplX =                   build_ele.dadesTubHor.value[nVer].desplX                  )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(desplY =                   build_ele.dadesTubHor.value[nVer].desplY                  )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(mostrarLiniaA =            build_ele.dadesTubHor.value[nVer].mostrarLiniaA           )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(desplLinA =                build_ele.dadesTubHor.value[nVer].desplLinA               )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(mostrarLiniaB =            build_ele.dadesTubHor.value[nVer].mostrarLiniaB           )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(desplLinB =                build_ele.dadesTubHor.value[nVer].desplLinB               )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(IsUseGlobalProp =          build_ele.dadesTubHor.value[nVer].IsUseGlobalProp         )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(FounColor =                build_ele.dadesTubHor.value[nVer].FounColor               )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(BarraLayer =               build_ele.dadesTubHor.value[nVer].BarraLayer              )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Ample_forat_femella =      build_ele.dadesTubHor.value[nVer].Ample_forat_femella     )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Altura_forat_femella =     build_ele.dadesTubHor.value[nVer].Altura_forat_femella    )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(Separacio_forat_femella =  build_ele.dadesTubHor.value[nVer].Separacio_forat_femella )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(PestanyaSuperior =         build_ele.dadesTubHor.value[nVer].PestanyaSuperior        )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(PestanyaInferior =         build_ele.dadesTubHor.value[nVer].PestanyaInferior        )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(PestanyesInv =             build_ele.dadesTubHor.value[nVer].PestanyesInv            )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(esProvisional =            build_ele.dadesTubHor.value[nVer].esProvisional           )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(provSupEsq =               build_ele.dadesTubHor.value[nVer].provSupEsq              )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(provSupDre =               build_ele.dadesTubHor.value[nVer].provSupDre              )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(provInfEsq =               build_ele.dadesTubHor.value[nVer].provInfEsq              )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(provInfDre =               build_ele.dadesTubHor.value[nVer].provInfDre              )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(provInfCen =               build_ele.dadesTubHor.value[nVer].provInfCen              )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(linia =                    build_ele.dadesTubHor.value[nVer].linia                   )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(layer =                    build_ele.dadesTubHor.value[nVer].layer                   )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(TipusTub =                 build_ele.dadesTubHor.value[nVer].TipusTub                )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(tipusTubAuto =             build_ele.dadesTubHor.value[nVer].tipusTubAuto            )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(RotarTub =                 build_ele.dadesTubHor.value[nVer].RotarTub                )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(tubInferior =              build_ele.dadesTubHor.value[nVer].tubInferior             )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(tubSuperior =              build_ele.dadesTubHor.value[nVer].tubSuperior             )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(posAutomatica =            build_ele.dadesTubHor.value[nVer].posAutomatica           )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(retallInicial =            build_ele.dadesTubHor.value[nVer].retallInicial           )
        build_ele.DadesVertCopy.value[0] = build_ele.DadesVertCopy.value[0]._replace(retallFinal =              build_ele.dadesTubHor.value[nVer].retallFinal             )
...
def pegar_valors_Vert(build_ele,nVer):
    ...
    try:
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Mostrar =                  build_ele.DadesVertCopy.value[0].Mostrar                 )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Ample =                    build_ele.DadesVertCopy.value[0].Ample                   )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Altura =                   build_ele.DadesVertCopy.value[0].Altura                  )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Llargada =                 build_ele.DadesVertCopy.value[0].Llargada                )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(LlargadaAut =              build_ele.DadesVertCopy.value[0].LlargadaAut             )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Gruix =                    build_ele.DadesVertCopy.value[0].Gruix                   )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(desplX =                   build_ele.DadesVertCopy.value[0].desplX                  )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(desplY =                   build_ele.DadesVertCopy.value[0].desplY                  )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(mostrarLiniaA =            build_ele.DadesVertCopy.value[0].mostrarLiniaA           )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(desplLinA =                build_ele.DadesVertCopy.value[0].desplLinA               )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(mostrarLiniaB =            build_ele.DadesVertCopy.value[0].mostrarLiniaB           )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(desplLinB =                build_ele.DadesVertCopy.value[0].desplLinB               )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(IsUseGlobalProp =          build_ele.DadesVertCopy.value[0].IsUseGlobalProp         )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(FounColor =                build_ele.DadesVertCopy.value[0].FounColor               )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(BarraLayer =               build_ele.DadesVertCopy.value[0].BarraLayer              )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Ample_forat_femella =      build_ele.DadesVertCopy.value[0].Ample_forat_femella     )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Altura_forat_femella =     build_ele.DadesVertCopy.value[0].Altura_forat_femella    )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(Separacio_forat_femella =  build_ele.DadesVertCopy.value[0].Separacio_forat_femella )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(PestanyaSuperior =         build_ele.DadesVertCopy.value[0].PestanyaSuperior        )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(PestanyaInferior =         build_ele.DadesVertCopy.value[0].PestanyaInferior        )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(PestanyesInv =             build_ele.DadesVertCopy.value[0].PestanyesInv            )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(esProvisional =            build_ele.DadesVertCopy.value[0].esProvisional           )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(provSupEsq =               build_ele.DadesVertCopy.value[0].provSupEsq              )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(provSupDre =               build_ele.DadesVertCopy.value[0].provSupDre              )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(provInfEsq =               build_ele.DadesVertCopy.value[0].provInfEsq              )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(provInfDre =               build_ele.DadesVertCopy.value[0].provInfDre              )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(provInfCen =               build_ele.DadesVertCopy.value[0].provInfCen              )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(linia =                    build_ele.DadesVertCopy.value[0].linia                   )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(layer =                    build_ele.DadesVertCopy.value[0].layer                   )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(TipusTub =                 build_ele.DadesVertCopy.value[0].TipusTub                )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(tipusTubAuto =             build_ele.DadesVertCopy.value[0].tipusTubAuto            )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(RotarTub =                 build_ele.DadesVertCopy.value[0].RotarTub                )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(tubInferior =              build_ele.DadesVertCopy.value[0].tubInferior             )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(tubSuperior =              build_ele.DadesVertCopy.value[0].tubSuperior             )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(posAutomatica =            build_ele.DadesVertCopy.value[0].posAutomatica           )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(retallInicial =            build_ele.DadesVertCopy.value[0].retallInicial           )
        build_ele.dadesTubHor.value[nVer] = build_ele.dadesTubHor.value[nVer]._replace(retallFinal =              build_ele.DadesVertCopy.value[0].retallFinal             )
...
def round0_5( num ):
    numrounded = round(float(num))
    ultimDigit = numrounded % 10
    if ultimDigit < 2.5:
        return round(numrounded - ultimDigit)
    else:
        return round(numrounded + ultimDigit)


def set_dadesISVertIS_to_true(build_ele):
    for elem in range(0, len(build_ele.dadesTubHor.value)):
        build_ele.dadesTubHor.value[elem] =  build_ele.dadesTubHor.value[elem]._replace(Mostrar = True)

    print()

'''
def values_from_solid3D(build_ele, placement_mat, element_listIS):
    if len(element_listIS) == 0:
        return
    else:
        amplada  = 0 #max 2150
        llargada = 0 #max 5650
        for element in element_listIS:
            print(element.GetGeometry())
            print(element.GetModelGeometry())
        build_ele.ISAmplada.value  = amplada  #1190
        build_ele.ISllargada.value = llargada #2530
        build_ele.IntegerISSelectorHor.value = 11
        set_values_tub_hor(build_ele, len(element_listIS), 0)
'''

def values_from_solid3D(build_ele, placement_mat, element_listIS, Pared, desplAbs = 0):
    solidsInteriors = []
    if len(element_listIS) == 0:
        return
    else:
        #inicialitzar_valors_inici(build_ele, len(element_listIS)-1)

        llistaPosicions = []
        pos = 0
        puntInferiorEsqIS = AllplanGeo.Point3D()
        iniciTD = True
        for element in Pared:
            try:
            ...
            for vert in llistaVertex:
                if iniciTD:
                    puntInferiorEsqIS = vert
                    iniciTD = False
                if vert.X <= puntInferiorEsqIS.X:
                    puntInferiorEsqIS.X = vert.X
                if vert.Y <= puntInferiorEsqIS.Y:
                    puntInferiorEsqIS.Y = vert.Y

        #puntInferiorEsqIS.X += desplAbs
        ...
        for element in element_listIS:
            ...
            solidsInteriors.append({"Inf" : puntInf,
                                "Sup" : puntSup,
                                "Esq" : puntEsq,
                                "Dre" : puntDre
                                })

        #for posVert in range( 0, len(build_ele.dadesTubHor.value)):
            #solidsInteriors[posVert]["Inf"]
            #solidsInteriors[posVert]["Sup"]
            #solidsInteriors[posVert]["Esq"]
            #solidsInteriors[posVert]["Dre"]

            #if build_ele.BarresVertList.value[posVert]
        for solid in solidsInteriors:

            #print("---------- " + str(posVert) + " ---> " + str(build_ele.dadesTubHor.value[posVert].desplXAbs + desplAbs) )
            #print('solid["Esq"]: ')
            #print(solid["Esq"] - puntInferiorEsqIS.X)
            #print('solid["Dre"]: ')
            #print(solid["Dre"]- puntInferiorEsqIS.X)

            build_ele.SelectorPPIS.value = 2
            build_ele.PosicioXlinia.value = solid["Esq"] - placement_mat[12]
            build_ele.PosicioYlinia.value = solid["Inf"] - placement_mat[13]
            build_ele.Amplelinia.value = solid["Dre"] - solid["Esq"] #- puntInferiorEsqIS.X
            build_ele.Llargadalinia.value = solid["Sup"] - solid["Inf"] #- puntInferiorEsqIS.Y
            #si el forat esta dins del IS
            puntos = []
            puntoA = AllplanGeo.Point3D(build_ele.PosicioXlinia.value, build_ele.PosicioYlinia.value,0)
            puntoB = AllplanGeo.Point3D(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value, build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value,0)
            puntoC = AllplanGeo.Point3D(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value, build_ele.PosicioYlinia.value ,0)
            puntoD = AllplanGeo.Point3D(build_ele.PosicioXlinia.value , build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value,0)
            puntos.append(puntoA)
            puntos.append(puntoB)
            puntos.append(puntoC)
            puntos.append(puntoD)
            build_ele.PosicioXlinia.value = solid["Esq"] - puntInferiorEsqIS.X - placement_mat[12]
            build_ele.PosicioYlinia.value = solid["Inf"] - puntInferiorEsqIS.Y - placement_mat[13]

            puntInfEsqISdespl = AllplanGeo.Point3D()
            puntInfEsqISdespl.X = puntInferiorEsqIS.X + placement_mat[12]
            puntInfEsqISdespl.Y = puntInferiorEsqIS.Y + placement_mat[13]


            llargadaIS = build_ele.ISllargada.value
            AmpleIS = build_ele.ISAmplada.value
            puntosPared = []
            puntoAPared = AllplanGeo.Point3D(puntInferiorEsqIS.X , puntInferiorEsqIS.Y,0)
            puntoBPared = AllplanGeo.Point3D(puntInferiorEsqIS.X + llargadaIS, puntInferiorEsqIS.Y + AmpleIS,0)
            puntoCPared = AllplanGeo.Point3D(puntInferiorEsqIS.X + llargadaIS, puntInferiorEsqIS.Y ,0)
            puntoDPared = AllplanGeo.Point3D(puntInferiorEsqIS.X , puntInferiorEsqIS.Y + AmpleIS,0)
            puntosPared.append(puntoAPared)
            puntosPared.append(puntoBPared)
            puntosPared.append(puntoCPared)
            puntosPared.append(puntoDPared)



            if punt_inside_pared(build_ele, puntInferiorEsqIS, puntos) or poligonos_se_intersectan(puntos, puntosPared): #pared_inside_rectangulo(build_ele, puntos, puntosPared):
                on_control_event(build_ele, event_id = 1001)
            build_ele.SelectorPPIS.value = 1
...
def punto_en_poligono(punto, poligono):
    ...
    for i in range(n):
        xi, yi = poligono[i].X,poligono[i].Y
        xj, yj = poligono[(i + 1) % n].X, poligono[(i + 1) % n].Y
        if ((yi > y) != (yj > y)):
            x_inters = (xj - xi) * (y - yi) / (yj - yi + 1e-12) + xi
            if x < x_inters:
                dentro = not dentro
    return dentro
...
def segmentos_se_cruzan(p1, p2, q1, q2):
    ...
    return False  # Para simplificar, omitimos casos especiales (colineales)

def poligonos_se_intersectan(poly1, poly2):
    # Paso 1: algún vértice de poly1 dentro de poly2
    for p in poly1:
        if punto_en_poligono(p, poly2):
            return True

    # Paso 2: algún vértice de poly2 dentro de poly1
    for p in poly2:
        if punto_en_poligono(p, poly1):
            return True

    # Paso 3: algún par de segmentos se cruza
    for i in range(len(poly1)):
        ...
        for j in range(len(poly2)):
            ...
            if segmentos_se_cruzan(a1, a2, b1, b2):
                return True

    return False


def pared_inside_rectangulo(build_ele, puntosRectangulo, puntosPared):
    """
    Revisa si alguno de los puntos dados está dentro del rectángulo en XY.
    """

    for p in puntosPared:
        if punto_en_poligono2(p, puntosRectangulo):
            return True #, p  # Devuelve True y el punto encontrado
    return False #, None  # Ningún punto está dentro

def punto_en_poligono2(punto, poligono):
    x, y = punto.X, punto.Y
    dentro = False
    n = len(poligono)
    ...
    for i in range(n):
        ...
        if ((yi > y) != (yj > y)):
            x_interseccion = (xj - xi) * (y - yi) / (yj - yi + 1e-12) + xi
            if x < x_interseccion:
                dentro = not dentro
    return dentro


def punt_inside_pared(build_ele, puntInfEsqIS, puntos):
    """
    Revisa si alguno de los puntos dados está dentro del rectángulo en XY.
    """
    llargadaIS = build_ele.ISllargada.value
    ...
    for p in puntos:
        if punto_en_rectangulo(p, puntInfEsqIS, llargadaIS, AmpleIS):
            return True
    return False

def punto_en_rectangulo(p, A, largo, ancho):
    """
    Verifica si el punto p está dentro del rectángulo definido en XY
    por el punto A como esquina inferior izquierda, con ancho y largo.
    """
    xA, yA = A.X, A.Y
    x, y = p.X, p.Y
    return xA <= x <= xA + largo and yA <= y <= yA + ancho
...
class IS_Conjunt_8():
    ...
    def __init__(self, build_ele : BuildingElement, doc, placement_mat = AllplanGeo.Matrix3D(), element_listIS = [], Pared = [], BarraLlargada = -1.0, ISAlcada = -1.0 ):

        self.build_ele_IS = BuildingElement()
        self.build_ele_IS = build_ele#.deep_copy()
        self.doc_IS = doc

        self.placement_mat = placement_mat

        self.element_listIS = element_listIS
        self.ISLlargada = BarraLlargada
        self.ISAlcada = ISAlcada
        self.Pared = Pared

        self.result = {}

    def get_params_list(self):
        return self.build_ele_IS

    def filename(self):
        return "IS_Conjunt_8_clase_PPIS_Tubes_Solids.py"

    def create_estructura(self, build_ele):
        doc = AllplanElementAdapter.DocumentAdapter()
        #create_element_class(self.build_ele, doc=doc)

        self.result = create_element_class(build_ele , doc, self.placement_mat, self.element_listIS, self.Pared, self.ISLlargada, self.ISAlcada)
        #model_elem_list = result["elements"]
        #handle_list = result["handles"]
        #model_elem_list_preview = result["preview_elements"]
        #group_elems = result["group_elems"]
        #group_elems_preview = result["group_elems_preview"]
        #preview_symbols = result["preview_symbols"]
        #self.build_ele_TD = result["build_ele"]
        #build_ele_TD = result["build_ele"]


        result = {
            "elements"              :  self.result["elements"],#model_elem_list,
            "handles"               :  self.result["handles"],#handle_list,
            "preview_elements"      :  self.result["preview_elements"],# model_elem_list_preview,
            "group_elems"           :  self.result["group_elems"],#group_elems,
            "group_elems_preview"   :  self.result["group_elems_preview"],#group_elems_preview,
            "preview_symbols"       :  self.result["preview_symbols"],#preview_symbols,
            "build_ele"             :  self.result["build_ele"],#build_ele_TD
            }
    ...
    def create(self):
        ...
        self.result = create_element_class(self.build_ele_IS , doc, self.placement_mat, self.element_listIS, self.Pared, self.ISLlargada, self.ISAlcada)
    ...
    def modify_element_property(self,
        ...
        Args:
            ...
            value: new value
        """
        prop = self.build_ele_IS.get_property(name)

        print("prop: " + str(prop))

        if prop is not None:
            BuildingElementValueUtil.update_value(name, value, self.build_ele_IS, self.build_ele_IS)# ,self.build_ele_ctrl_props_list)


    def on_control_event(self, build_ele, event_id: int, ):
        self.on_control_event_aux(self.build_ele_IS, event_id, self.placement_mat, self.element_listIS, self.Pared, self.ISLlargada, self.ISAlcada)

    def on_control_event_aux(self, build_ele, event_id: int, placement_mat, element_listIS, Pared, ISLlargada, ISAlcada):
        nVer = getNumFromText(build_ele, build_ele.SelectorTubHor.value, 3)
        ...
        doc = DocumentManager.get_instance().document
        create_element_class(build_ele, doc, self.placement_mat, self.element_listIS, self.Pared, self.ISLlargada, self.ISAlcada)

    def set_values_tubs_true(self):
        set_dadesISVertIS_to_true(self.build_ele_IS)
        return True

    def values_from_solid3D(self, desplAbs, build_ele):
        self.on_control_event_aux(self.build_ele_IS, 1000, self.placement_mat, self.element_listIS, self.Pared, self.ISLlargada, self.ISAlcada)
        values_from_solid3D(build_ele, self.placement_mat, self.element_listIS, self.Pared, desplAbs)


    def set_dadesTDVertTD_to_true(self):
        set_dadesISVertIS_to_true(self.build_ele_IS)

    def getUUIDs(self):
        return self.getUUIDs()

    def crearLlistaFullHoritzontals(self):
        crearLlistaFullHoritzontals(self.build_ele_IS)
