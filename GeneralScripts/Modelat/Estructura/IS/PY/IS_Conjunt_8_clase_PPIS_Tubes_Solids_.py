...
def create_element_class(build_ele, doc, placement_mat = AllplanGeo.Matrix3D(), element_listIS = [], Pared = [], ISLlargada = -1.0, ISAlcada = -1.0 ): # type: ignore
    ...
    while build_ele.IntegerISSelectorHor.value +1 > len(build_ele.dadesTubHor.value):
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value+1, nHorSeleccionat, placement_mat)
...
def create_polyline_interior(build_ele, punt_central, llargadaX, llargadaZ, isHor, pointUbi, linia = 1, layer = "EN_FIXACIO_X", posYdiferent = 200):
    ...
    if (isHor):
        ...
        vectorPOS.SetValue(14, posZ + 150)
        ...
        vectorPOSInv.SetValue(14, posZ - 150 )
...
def set_values_tub_hor(build_ele, j, actual, placement_mat = AllplanGeo.Matrix3D()):
            ...
            if len(build_ele.dadesTubHor.value) != 0 and len(build_ele.dadesTubHor.value) != 1:
                if (len(build_ele.dadesTubHor.value)-2) * 700 < build_ele.ISllargada.value - 50:
                    ...
                    if not extremEsqGuardat:
                        ...
                        bobDadesGlobal = bobDadesGlobal._replace(desplX = 0 - 50)#+ build_ele.dadesTubHor.value[0].Ample)
                    ...
                    bobDadesGlobal = bobDadesGlobal._replace(desplX = build_ele.ISllargada.value -50  )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(desplY = 0 )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(Llargada = build_ele.ISAmplada.value - 100 )#+ build_ele.dadesTubHor.value[0].Ample)
                    bobDadesGlobal = bobDadesGlobal._replace(LlargadaAut = True)
                    bobDadesGlobal = bobDadesGlobal._replace(RotarTub = True)
                ...
                bobDadesGlobal = bobDadesGlobal._replace(desplX = 0 - 50 )#+ placement_mat[13])#+ build_ele.dadesTubHor.value[0].Ample)
                        ...
                        if nHorSeleccionat == x:
                            if not build_ele.llargadaAutomatica.value:
                                ...
                                if not build_ele.posAutomatica.value:
                                    ...
                                    #build_ele.BarraLlargadaIS.value = build_ele.PosicioXlinia.value - posHoritzontal -50
                        else:
...
def detectar_tipus_tub(build_ele, doc, posTub ,nHorSeleccionat, encaixInf = True, encaixSup = True):
    ...
    #si es extrem semprw sera A i no es invertit
    if (build_ele.dadesTubHor.value[posTub].desplX == build_ele.dadesTubHor.value[build_ele.ExteriorEsquerra.value].desplX or \
        ...
        not build_ele.direccioISInv.value:
        if nHorSeleccionat == posTub:
            build_ele.TipusTUB.value = "TUB A"
        build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(TipusTub = "TUB A")
...
def values_from_solid3D(build_ele, placement_mat, element_listIS, Pared, desplAbs = 0):
    ...
    if len(element_listIS) == 0:
        ...
        for solid in solidsInteriors:
            ...
            if punt_inside_pared(build_ele, puntInferiorEsqIS, puntos):
...
class IS_Conjunt_8():
    ...
    def values_from_solid3D(self, desplAbs, build_ele):
        on_control_event(self.build_ele_IS, 1000)
