...
def move_handle(build_ele, handle_prop, input_pnt, doc):
    ...
    #test de no actualitzar mentres es mou
        ...
        for posBarraHor in range(0, build_ele.IntegerISSelectorHor.value):#len(build_ele.dadesTubHor.value)):
            ...
            if build_ele.dadesTubHor.value[posBarraHor].TipusTub == "TUB A":
                ...
                TubHoritzontal = PP_IS_Horitzontal(random.random() * 3600,0, 50, 20,llargada, 1.5,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                            ...
                                            False, False,#PestanyaSuperior, PestanyaInferior,
                                            encaixos, False, #EncaixosPar
                                            build_ele.dadesTubHor.value[posBarraHor].retallInicial, build_ele.dadesTubHor.value[posBarraHor].retallFinal) #)
            ...
            table_attr_listAux = [AllplanBaseElements.AttributeString(684, "IfcBuildingElementProxy"),
                                    ...
                                    AllplanBaseElements.AttributeString(1087, Rotar),
...
def guardar_valors_hor(build_ele, j):
    ...
    while len(build_ele.dadesTubHor.value) <= j:
        ...
        if build_ele.TipusTUB.value == "TUB A":
            ...
            if build_ele.BarraLlargada.value > 5550:
                ctypes.windll.user32.MessageBoxW(0, "la llargada del tub tipus TUB E no pot ser mes gran a 5550", 0)
                build_ele.BarraLlargada.value = 5550
...
def crear_creu_auto(build_ele, doc, nHorSeleccionat):
    ...
    nCreats, llistaTubs = crear_barres_cuadrat(build_ele, doc, nHorSeleccionat, costats)
...
def crear_barres_cuadrat(build_ele, doc, nHorSeleccionat, costats):
    ...
    if len(costats) == 1:
        if "Esq" in costats:
            ...
            posicionsNousTubs["Inf"] = crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats, tubSup = posicionsNousTubs["Dre"], llargadaAutomatica = True)
        elif "Inf" in costats:
            print("1 Hor 2 Vert")
            posicionsNousTubs["Dre"] = crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats)
            posicionsNousTubs["Esq"] = crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats)
        ...
        if "Esq" in costats and "Dre" in costats:
            ...
            print("1 Hor 1 Vert")
            posicionsNousTubs["Dre"] = crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats)
            posicionsNousTubs["Sup"] = crear_barra_sup_Forat(build_ele, nHorSeleccionat, costats, tubSup = posicionsNousTubs["Dre"])
        elif "Esq" in costats and "Sup" in costats:
            print("1 Hor 1 Vert")
            posicionsNousTubs["Dre"] = crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats)
            posicionsNousTubs["Inf"] = crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats, tubSup = posicionsNousTubs["Dre"])
        elif "Dre" in costats and "Inf" in costats:
            print("1 Hor 1 Vert")
            posicionsNousTubs["Esq"] = crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats)
            posicionsNousTubs["Sup"] = crear_barra_sup_Forat(build_ele, nHorSeleccionat, costats, tubInf = posicionsNousTubs["Esq"])
        elif "Dre" in costats and "Sup" in costats:
            print("1 Hor 1 Vert")
            posicionsNousTubs["Esq"] = crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats)
            posicionsNousTubs["Inf"] = crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats, tubInf = posicionsNousTubs["Esq"])
        print("crear 2")
        return 2 , posicionsNousTubs
    elif len(costats) == 3:
...
def detectar_colisio_tubs(build_ele, doc, nHorSeleccionat, ntubsAfegits, llistaTubs):
    ...
    for x in range(0, lenDadesTubHorAct):
        ...
        if build_ele.dadesTubHor.value[x].RotarTub:#tub Vertical
                ...
                if  (posVerticalSup <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value and posVerticalSup > build_ele.PosicioYlinia.value ) and posVertical < build_ele.PosicioYlinia.value :
                    ...
                    if nHorSeleccionat == x:
                        ...
                        if x == build_ele.ExteriorEsquerra.value or x == build_ele.ExteriorDreta.value:
                            ...
                            build_ele.BarraLlargada.value =  build_ele.dadesTubHor.value[x].retallFinal + 50
                    ...
                    if nHorSeleccionat == x:
                        ...
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(LlargadaAut = False)
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Llargada = posVerticalSup -  posVerticalInf - 50)#posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value) )
                    ...
                    if nHorSeleccionat == x:
                        ...
                        #build_ele.desplY.value = build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value
                        if x == build_ele.ExteriorEsquerra.value or x == build_ele.ExteriorDreta.value:
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Llargada = build_ele.dadesTubHor.value[x].retallFinal + 50)#posHoritzontalDre - build_ele.PosicioXlinia.value  -50)
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallFinal = build_ele.dadesTubHor.value[x].retallFinal + 50)#posHoritzontalDre - build_ele.PosicioXlinia.value  -50)
            ...
            if ( posVertical >= build_ele.PosicioYlinia.value and posVertical <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value):
                if posHoritzontal + build_ele.dadesTubHor.value[x].Llargada >= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    ...
                    if nHorSeleccionat == x:
                        build_ele.retallInicial.value = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value  - posHoritzontalEsq
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallInicial = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value  - posHoritzontalEsq)
                    ...
                    if build_ele.PosicioXlinia.value != 0:
                        ...
                        verticalMesProper = detectar_vertical_anterior(build_ele, x, nHorSeleccionat)
                        ...
                        if verticalMesProper != None:
                            asignar_barra_inferior(build_ele, len(build_ele.dadesTubHor.value)-2, nHorSeleccionat, verticalMesProper)
                        if llistaTubs["Esq"] != None:
                            asignar_barra_superior(build_ele, len(build_ele.dadesTubHor.value)-2, nHorSeleccionat, llistaTubs["Esq"])
                        detectar_tipus_tub(build_ele, doc, len(build_ele.dadesTubHor.value)-2, nHorSeleccionat)
                        reasignarTubs(build_ele, x, len(build_ele.dadesTubHor.value)-2, nHorSeleccionat)
                    if llistaTubs["Esq"] != None:
                        asignar_barra_superior(build_ele, len(build_ele.dadesTubHor.value)-2, nHorSeleccionat, llistaTubs["Esq"])
                    ...
                    if nHorSeleccionat == x:
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallFinal = build_ele.PosicioXlinia.value - posHoritzontal)#posHoritzontalDre - build_ele.PosicioXlinia.value  -50)
                        ...
                        if not build_ele.posAutomatica.value:
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallFinal = build_ele.PosicioXlinia.value - posHoritzontal -50)#posHoritzontalDre - build_ele.PosicioXlinia.value  -50)
        ...
        detectar_tipus_tub(build_ele, doc, x, nHorSeleccionat)

def detectar_vertical_anterior(build_ele, x, nHorSeleccionat):
    '''
    x = tub Horitzontal

    return el tuv Vertical que el travessa, mes propers
    '''
    ...
    for posTub in range(0, len(build_ele.dadesTubHor.value)):
        if build_ele.dadesTubHor.value[posTub].RotarTub:
            if build_ele.dadesTubHor.value[posTub].desplX < build_ele.PosicioXlinia.value:
                sumatoriY = 0
                if build_ele.dadesTubHor.value[posTub].posAutomatica:
                    sumatoriY = 50
                print(str(posTub) + " : " + str(build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriY) + " <= " + str(build_ele.dadesTubHor.value[x].desplY ) + " and " + str(build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada) + " >= " + str(build_ele.dadesTubHor.value[x].desplY))
                if build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriY <= build_ele.dadesTubHor.value[x].desplY  and build_ele.dadesTubHor.value[posTub].desplY + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada >= build_ele.dadesTubHor.value[x].desplY :
                    print(str(posTub) + " es un candidat per a ser anterior")
    ...
    print("anteriorMesProper: " + str(anteriorMesProper))
...
def detectar_tipus_tub(build_ele, doc, posTub ,nHorSeleccionat):
    ...
    if build_ele.dadesTubHor.value[posTub].TipusTub == "TUB E" and (llargada < 1350  ):#or build_ele.dadesTubHor.value[posTub].retallFinal < 1350):
    ...
    if build_ele.dadesTubHor.value[posTub].TipusTub == "TUB D" and (build_ele.dadesTubHor.value[posTub].Llargada - build_ele.dadesTubHor.value[posTub].retallInicial < 650 or build_ele.dadesTubHor.value[posTub].Llargada < 650 ):#or build_ele.dadesTubHor.value[posTub].retallFinal < 650):
        if nHorSeleccionat == posTub:
...
def crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats, tubInf = 0, tubSup = 1, posAutomatica = False, llargadaAutomatica = False):
    #afegir Tub Vertical
    build_ele.IntegerISSelectorHor.value = build_ele.IntegerISSelectorHor.value + 1
    if build_ele.IntegerISSelectorHor.value> len(build_ele.dadesTubHor.value)-1:
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat)
...
def crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats, tubInf = 0, tubSup = 1, posAutomatica = False, llargadaAutomatica = False):
    #afegir Tub Vertical
    build_ele.IntegerISSelectorHor.value = build_ele.IntegerISSelectorHor.value + 1
    if build_ele.IntegerISSelectorHor.value > len(build_ele.dadesTubHor.value)-1:
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat)
