...
def create_element(build_ele, doc): # type: ignore
    ...
    if build_ele.SelectorPP.value != 1:
        ...
        if build_ele.crearForat.value :
            ...
            crear_forat_auto(build_ele, doc, nHorSeleccionat)
    ...
    for posBarraHor in range(0, build_ele.IntegerISSelectorHor.value):#len(build_ele.dadesTubHor.value)):
        ...
        if build_ele.dadesTubHor.value[posBarraHor].Mostrar:
            ...
            if build_ele.MostrarTDNums.value:
                preview_symbols.add_text(text=              str(posBarraHor),
                                 reference_point=   vertx + AllplanGeo.Point3D(-80,20,0),
            ...
            #    print("Error al crear numero: " + str(e))
...
def create_handles_linies(build_ele ):
        ...
        handle_list = [HandleProperties("BarraAmple",
                                        AllplanGeo.Point3D(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value - 50, build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value, 0 ),
                                        AllplanGeo.Point3D(build_ele.PosicioXlinia.value - 50, build_ele.PosicioYlinia.value , 0),
                                        [("Amplelinia", HandleDirection.x_dir),
                                         ("Llargadalinia", HandleDirection.y_dir)],
                                        HandleDirection.xy_dir,
                                        False,
                                        info_text= "Tamany"),
                        HandleProperties("BarraPosicio",
                                        ...
                                        [("PosicioXlinia", HandleDirection.x_dir),
                                         ("PosicioYlinia", HandleDirection.y_dir)],
                                        HandleDirection.xy_dir,
                                        False,
                                        info_text= "Posicio")
                                        #HandleProperties("Llargadalinia",
...
def create_num_on_view_Hor(build_ele, j, vectorH1):
    llargada = build_ele.dadesTubHor.value[j].Llargada - 30
    if build_ele.dadesTubHor.value[j].Llargada - 30 < 0:
        llargada = build_ele.dadesTubHor.value[j].Llargada
    numVert = TextSpline(random.random() * 3600, build_ele.dadesTubHor.value[j].Ample + 2, build_ele.dadesTubHor.value[j].Altura, llargada, build_ele.dadesTubHor.value[j].Gruix,
...
def get_posicioX(build_ele, posBarraHor):
    ...
    return build_ele.dadesTubHor.value[nTubInf].desplX  + build_ele.dadesTubHor.value[posBarraHor].desplX
...
def crear_creu_auto(build_ele, doc, nHorSeleccionat):
    detectar_colisio_tubs(build_ele, doc, nHorSeleccionat)
    ...
    crear_barra_Vertical(build_ele, nHorSeleccionat, build_ele.PosicioYlinia.value + 50 , build_ele.PosicioYlinia.value  + build_ele.Llargadalinia.value/2 - 25, build_ele.PosicioXlinia.value + build_ele.Amplelinia.value/2 - 25, "C")
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(esProvisional = True )
    crear_barra_Vertical(build_ele, nHorSeleccionat, build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value/2 +25, build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value -50, build_ele.PosicioXlinia.value + build_ele.Amplelinia.value/2 -25, "C")
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(esProvisional = True )

    costats = ["Esq", "Dre", "Inf", "Sup" ]
    crear_barres_cuadrat(build_ele, doc, nHorSeleccionat, costats)

def crear_forat_auto(build_ele, doc, nHorSeleccionat):

    lenDadesTubHorAct = build_ele.IntegerISSelectorHor.value
    costats = []
    if build_ele.PosicioXlinia.value <= 0:
        print("forat esquerra")
        costats.append("Esq")
        '''
        for x in range(0, lenDadesTubHorAct):
            posHoritzontal = get_posicioX(build_ele, x)
            posHoritzontalDre = get_posicioX(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubSuperior, 3 ))
            posVertical = get_posicioY(build_ele, x)
            posVerticalSup = get_posicioY(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubSuperior, 3 ))
            if build_ele.dadesTubHor.value[x].RotarTub:
                if posHoritzontal >= build_ele.PosicioXlinia.value and posHoritzontal <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    if posVertical >= build_ele.PosicioYlinia.value and posVertical <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value and posVertical + build_ele.dadesTubHor.value[x].Llargada > build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                        ...
                        #print("tallar tub VERTICAL [" + str(x) + "] a " + str(build_ele.PosicioYlinia.value) + " i a " + str(build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value))
                        if nHorSeleccionat == x:
                            ...
                            if build_ele.posAutomatica.value:
                                ...
                                build_ele.desplX.value = build_ele.dadesTubHor.value[x].desplX + 50
                            build_ele.posAutomatica.value = False
                            ...
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Llargada = posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value) )#+ (build_ele.ISAmplada.value - posVertical) )
                            if build_ele.dadesTubHor.value[x].posAutomatica:
                                build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(posAutomatica = False)
                ...
                if (posHoritzontal >= build_ele.PosicioXlinia.value and posHoritzontal <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) and ( posVertical >= build_ele.PosicioYlinia.value and posVertical <=build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value):
                    if posHoritzontal + build_ele.dadesTubHor.value[x].Llargada >= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                        print("ESQ tallar i desplaçar tub HORITZONTAL [" + str(x) + "] a " + str(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) + " i retallar " + str(posHoritzontalDre - (build_ele.PosicioXlinia.value + build_ele.Amplelinia.value)))
                        if nHorSeleccionat == x:
                            build_ele.llargadaAutomatica.value = False
                            build_ele.BarraLlargada.value = posHoritzontalDre - (build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) - 50
                            build_ele.posAutomatica.value = False
                            build_ele.desplX.value = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value - 50
                        else:
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(LlargadaAut = False)
    ...
    if build_ele.PosicioYlinia.value <= 0:
        ...
        for x in range(0, lenDadesTubHorAct):
            ...
            posVerticalSup = get_posicioY(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubSuperior, 3 ))
            if not build_ele.dadesTubHor.value[x].RotarTub:
                if build_ele.dadesTubHor.value[x].desplY >= build_ele.PosicioYlinia.value and build_ele.dadesTubHor.value[x].desplY <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                    #if build_ele.dadesTubHor.value[x].desplX >= build_ele.PosicioXlinia.value and build_ele.dadesTubHor.value[x].desplX <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    print("tallar i desplaçar tub HORITZONTAL [" + str(x) + "] a " + str(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) + " i retallar " + str(posHoritzontalDre - (build_ele.PosicioXlinia.value + build_ele.Amplelinia.value)))
                    #if nHorSeleccionat == x:
                    #    build_ele.llargadaAutomatica.value = False
                    #    build_ele.BarraLlargada.value = posHoritzontalDre - (build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) - 50
                    #    build_ele.posAutomatica.value = False
                ...
                if (build_ele.dadesTubHor.value[x].desplX >= build_ele.PosicioXlinia.value and build_ele.dadesTubHor.value[x].desplX <=build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) and ( build_ele.dadesTubHor.value[x].desplY >= build_ele.PosicioYlinia.value ):

                    posAltura = get_posicioY(build_ele, x)
                    if posAltura >= build_ele.PosicioYlinia.value and posAltura < build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value and posAltura + build_ele.dadesTubHor.value[x].Llargada > build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                        print("desplaçar tub VERTICAL [" + str(x) + "] a " + str(build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value) + " i retallar " + str(build_ele.dadesTubHor.value[x].Llargada) +" - "+str(build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value))
                    if posAltura >= build_ele.PosicioYlinia.value and posAltura + build_ele.dadesTubHor.value[x].Llargada < build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                        print("ocultar Tub [" + str(x) + "]")
                        if nHorSeleccionat == x:
                            build_ele.MostrarTub.value = False
                        else:
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Mostrar = False)
        '''
    if build_ele.PosicioXlinia.value + build_ele.Amplelinia.value  >= build_ele.ISllargada.value + 50:
        print("forat dreta")
        costats.append("Dre")
        '''
        for x in range(0, lenDadesTubHorAct):
            ...
            if  build_ele.dadesTubHor.value[x].RotarTub:
                if build_ele.dadesTubHor.value[x].desplX >= build_ele.PosicioXlinia.value and build_ele.dadesTubHor.value[x].desplX <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    if posVertical > build_ele.PosicioYlinia.value and posVertical + build_ele.dadesTubHor.value[x].Llargada < build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                        if nHorSeleccionat == x:
                            build_ele.MostrarTub.value = False
                        else:
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Mostrar = False)
                        print("ocultar VERTICAL [" + str(x) + "] " )
                    else:
                        if posVertical < build_ele.PosicioYlinia.value and posVertical + build_ele.dadesTubHor.value[x].Llargada > build_ele.PosicioYlinia.value :
                            print("tallar tub VERTICAL [" + str(x) + "] a " + str(build_ele.PosicioYlinia.value - posVertical) + " i a " + str(build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value - posVertical))
            else:
                if posHoritzontal + build_ele.dadesTubHor.value[x].Llargada > build_ele.PosicioXlinia.value and posHoritzontal < build_ele.PosicioXlinia.value + build_ele.Amplelinia.value and posVertical > build_ele.PosicioYlinia.value and posVertical < build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:

                    print("tallar i desplaçar tub HORITZONTAL [" + str(x) + "] a " + str(build_ele.PosicioXlinia.value) + " i a " + str(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value))
                    #print("detecto horitzontal int [" + str(x) + "]")
                    #print("detecto horitzontal int [" + str(x) + "]")
        '''
    if build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value >= build_ele.ISAmplada.value:
        ...
        for x in range(0, lenDadesTubHorAct):
            ...
            if not build_ele.dadesTubHor.value[x].RotarTub:
                if build_ele.dadesTubHor.value[x].desplY >= build_ele.PosicioYlinia.value and build_ele.dadesTubHor.value[x].desplY <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                    #if build_ele.dadesTubHor.value[x].desplX >= build_ele.PosicioXlinia.value and build_ele.dadesTubHor.value[x].desplX <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    print("tallar tub HORITZONTAL [" + str(x) + "] a " + str(build_ele.PosicioXlinia.value) + " i a " + str(build_ele.PosicioXlinia.value + build_ele.Amplelinia.value))
            else:
                if build_ele.dadesTubHor.value[x].desplX >= build_ele.PosicioXlinia.value and build_ele.dadesTubHor.value[x].desplX <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    if posVertical > build_ele.PosicioYlinia.value and posVertical + build_ele.dadesTubHor.value[x].Llargada < build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                        if nHorSeleccionat == x:
...
def crear_barres_cuadrat(build_ele, doc, nHorSeleccionat, costats):
    if len(costats) == 0:
        print("crear 4")
        crear_barra_sup_Forat(build_ele, nHorSeleccionat, costats)
        crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats)
        crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats)
        crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats)
    if len(costats) == 1:
        print("crear 3")
        if "Esq" in costats:
            print("2 Hor 1 Vert")
...
def detectar_colisio_tubs(build_ele, doc, nHorSeleccionat):
    lenDadesTubHorAct = build_ele.IntegerISSelectorHor.value
    #print("forat esquerra")
    for x in range(0, lenDadesTubHorAct):
        ...
        if not build_ele.dadesTubHor.value[x].posAutomatica and not build_ele.dadesTubHor.value[x].LlargadaAut:
            ...
            if not build_ele.dadesTubHor.value[x].RotarTub:#Tub Horitzontal
                ...
                posHoritzontalEsq = posHoritzontal = build_ele.dadesTubHor.value[x].desplX
                posHoritzontalDre = build_ele.dadesTubHor.value[x].desplX
                posVerticalInf = posVertical = get_posicioY(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubInferior, 3 ))
        ...
        '''
        posHoritzontal = build_ele.dadesTubHor.value[x].desplX
        if build_ele.dadesTubHor.value[x].posAutomatica:
            posHoritzontal = get_posicioX(build_ele, x)

        posHoritzontalDre = build_ele.dadesTubHor.value[x].desplX + build_ele.dadesTubHor.value[x].Llargada
        if build_ele.dadesTubHor.value[x].LlargadaAut:
            posHoritzontalDre = get_posicioX(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubSuperior, 3 ))

        posHoritzontalEsq = build_ele.dadesTubHor.value[x].desplX
        if build_ele.dadesTubHor.value[x].posAutomatica:
            posHoritzontalEsq = get_posicioX(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubInferior, 3 ))

        posVerticalSup = build_ele.dadesTubHor.value[x].desplY + build_ele.dadesTubHor.value[x].Llargada
        if build_ele.dadesTubHor.value[x].LlargadaAut:
            posVerticalSup = get_posicioY(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubSuperior, 3 ))

        posVertical = build_ele.dadesTubHor.value[x].desplY
        if build_ele.dadesTubHor.value[x].posAutomatica:
            posVertical = get_posicioY(build_ele, x)

        posVerticalInf = build_ele.dadesTubHor.value[x].desplY
        if build_ele.dadesTubHor.value[x].posAutomatica:
            posVerticalInf = get_posicioY(build_ele, getNumFromText(build_ele, build_ele.dadesTubHor.value[x].tubInferior, 3 ))
        '''
        if build_ele.dadesTubHor.value[x].RotarTub:#tub Vertical
            if posHoritzontal >= build_ele.PosicioXlinia.value and posHoritzontal <= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                if  (posVerticalSup <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value and posVerticalSup > build_ele.PosicioYlinia.value ) and posVertical < build_ele.PosicioYlinia.value :
                    ...
                    if nHorSeleccionat == x:
                        build_ele.llargadaAutomatica.value = False
                        build_ele.BarraLlargada.value =  build_ele.PosicioYlinia.value - posVertical - 50#posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value)
                    else:
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(LlargadaAut = False)
                    ...
                    if nHorSeleccionat == x:
                        build_ele.llargadaAutomatica.value = False
                        build_ele.BarraLlargada.value = posVerticalSup -  (build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value)#posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value)
                        if build_ele.posAutomatica.value:
                            build_ele.posAutomatica.value = False
                            build_ele.desplX.value = build_ele.dadesTubHor.value[x].desplX + 50
                        ...
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(LlargadaAut = False)
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Llargada = posVerticalSup -  (build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value))#posVerticalSup - (build_ele.Llargadalinia.value  +  build_ele.PosicioYlinia.value) )
                        if build_ele.dadesTubHor.value[x].posAutomatica:
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(posAutomatica = False)
                            build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(desplX = build_ele.dadesTubHor.value[x].desplX + 50)
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallInicial = build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value - posVerticalInf)
                    ...
                    if nHorSeleccionat == x:
                        ...
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(desplY = build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value )
                    if build_ele.PosicioYlinia.value != 0:
                        crear_barra_Vertical(build_ele, nHorSeleccionat, posVerticalInf, build_ele.PosicioYlinia.value, build_ele.dadesTubHor.value[x].desplX, "A")
                elif posVertical >= build_ele.PosicioYlinia.value and posVertical + build_ele.dadesTubHor.value[x].Llargada <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value:
                    print("ESQ ocultar Tub  VERTICAL[" + str(x) + "]")
                    if nHorSeleccionat == x:
                        build_ele.MostrarTub.value = False
            ...
            if ( posVertical >= build_ele.PosicioYlinia.value and posVertical <= build_ele.PosicioYlinia.value + build_ele.Llargadalinia.value):
                if posHoritzontal + build_ele.dadesTubHor.value[x].Llargada >= build_ele.PosicioXlinia.value + build_ele.Amplelinia.value:
                    print("posVertical [" + str(x) + "] a " + str(posVertical) )
                    ...
                    if nHorSeleccionat == x:
                        ...
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(Llargada = posHoritzontalDre - (build_ele.PosicioXlinia.value + build_ele.Amplelinia.value) -50)
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(posAutomatica = False)
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(retallInicial = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value - 50 - posHoritzontalEsq)
                        build_ele.dadesTubHor.value[x] = build_ele.dadesTubHor.value[x]._replace(desplX = build_ele.PosicioXlinia.value + build_ele.Amplelinia.value - 50)
                    if build_ele.PosicioXlinia.value != 0:
                        crear_barra_Horitzontal(build_ele, nHorSeleccionat, posHoritzontalEsq - 50, build_ele.PosicioXlinia.value-50, build_ele.dadesTubHor.value[x].desplY, "A")
...
def detectar_tipus_tub(build_ele, doc, posTub ,nHorSeleccionat):
    '''
...
def crear_barra_sup_Forat(build_ele, nHorSeleccionat, costats):
    #afegir Tub Horitzontal
    build_ele.IntegerISSelectorHor.value = build_ele.IntegerISSelectorHor.value + 1
    if build_ele.IntegerISSelectorHor.value> len(build_ele.dadesTubHor.value)-1:
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat)
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(Mostrar = True )
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(TipusTub = "TUB D" )
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(RotarTub = False)
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(tubInferior = "Tub 0")
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(tubSuperior = "Tub 1")
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(LlargadaAut = False)
...
def crear_barra_inf_Forat(build_ele, nHorSeleccionat, costats):
    #afegir Tub Horitzontal
    build_ele.IntegerISSelectorHor.value = build_ele.IntegerISSelectorHor.value + 1
    if build_ele.IntegerISSelectorHor.value> len(build_ele.dadesTubHor.value)-1:
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat)
...
def crear_barra_dreta_Forat(build_ele, nHorSeleccionat, costats):
    #afegir Tub Vertical
    build_ele.IntegerISSelectorHor.value = build_ele.IntegerISSelectorHor.value + 1
    if build_ele.IntegerISSelectorHor.value> len(build_ele.dadesTubHor.value)-1:
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat)
    ...
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(tubSuperior = "Tub 1")
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(LlargadaAut = False)
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(Llargada =  build_ele.Llargadalinia.value)
    build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1] = build_ele.dadesTubHor.value[build_ele.IntegerISSelectorHor.value-1]._replace(posAutomatica = False)
...
def crear_barra_esquerra_Forat(build_ele, nHorSeleccionat, costats):
    #afegir Tub Vertical
    build_ele.IntegerISSelectorHor.value = build_ele.IntegerISSelectorHor.value + 1
    if build_ele.IntegerISSelectorHor.value > len(build_ele.dadesTubHor.value)-1:
        set_values_tub_hor(build_ele, build_ele.IntegerISSelectorHor.value, nHorSeleccionat)
