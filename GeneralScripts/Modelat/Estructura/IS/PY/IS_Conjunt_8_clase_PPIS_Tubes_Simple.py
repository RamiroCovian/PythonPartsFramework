...
from .IS_Tub_Solid3d import PP_IS_tub
...
def create_element(build_ele, doc): # type: ignore
    ...
    result = create_element_class(build_ele, doc, AllplanGeo.Matrix3D(), [])
...
def create_element_class_ind(build_ele, doc, placement_mat, element_listIS): # type: ignore
    ...
    TubHoritzontal = PP_IS_tub(random.random() * 3600,0, 50, 20, llargada, 1.5,#DistanciaEntreTD(TDV BarraLlargada), BarraAmple, BarraAltura,BarraLlargada(DistanciaEntreTD), BarraGruix,
                                        element_listIS[posBarraHor],
    ...
    TranslationFather =AllplanGeo.Matrix3D()
...
def create_element_class(build_ele, doc, placement_mat, element_listIS): # type: ignore


    nHorSeleccionat = getNumFromText(build_ele, build_ele.SelectorTubHor.value, 3)

    if len(element_listIS) >= 2:
        build_ele.IntegerISSelectorHor.value = len(element_listIS)
    ...
    if len(element_listIS) > 0 and len(build_ele.dadesTubHor.value) == 0:
        values_from_solid3D(build_ele, placement_mat, element_listIS)
        asignar_colindants(build_ele,nHorSeleccionat)
    ...
    for posBarraHor in range(0, len(build_ele.dadesTubHor.value)):#len(build_ele.dadesTubHor.value)):
...
def values_from_solid3D(build_ele, placement_mat, element_listIS):
    if len(element_listIS) == 0:
        ...
        set_values_tub_hor(build_ele, len(element_listIS)-1, 0)
        amplada  = 0 #max 2150
        llargada = 0 #max 5650
        ...
        for edge in geo.Vertex():

            # Get the curve data (e.g., control points for a NURBS curve)
            #curve_data = edge.getCurveData()

            # Calculate the edge length using the curve's length method
            #llargadaAct = edge.length()

            llargadaAct = geo.EndPoint- geo.StartPoint
            if llargadaAct > llargadaMesGranAct:
                llargadaMesGranAct = llargadaAct
        '''

        #print("-----------------------------")
        #print("|" +str(puntSuperiorEsqIS)+ "|"+str(puntSuperiorDreIS)+ "|")
        #print("-----------------------------")
        #print("|" +str(puntInferiorEsqIS)+ "|"+str(puntInferiorDreIS)+ "|")
        #print("-----------------------------")
        ...
        #if amplada > 2150 and llargada > 5650:
        #    amplada = 2150
        #    llargada = 5650
        #elif llargada <= 5650 and amplada <= 5650:

        '''
        if llargada <= 5650 and amplada <= 5650:
            if amplada > 2150 and llargada <= 2150:
                #aux = llargada
                #llargada = amplada
                #amplada = aux
                element_listIS_New = rotar_elements_solids(build_ele, puntInferiorEsqIS, placement_mat, element_listIS)
                if element_listIS_New != []:
                    element_listIS = element_listIS_New
                isRotat90 = True
        '''

        build_ele.ISAmplada.value  = amplada  #1190
        build_ele.ISllargada.value = llargada #2530
        #print("ISAmplada: " + str(build_ele.ISAmplada.value))
        #print("ISllargada: " + str(build_ele.ISllargada.value))
        ...
        for element in element_listIS:
            ...
            llargadaMesGranAct = AllplanGeo.CalcMinMax(geo)

            puntInferiorEsq = llistaPosicions[posBarraHor][0]
            puntInferiorDre = llistaPosicions[posBarraHor][1]
            puntSuperiorEsq = llistaPosicions[posBarraHor][2]
            puntSuperiorDre = llistaPosicions[posBarraHor][3]

            build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(posAutomatica = False)
            build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(desplX = round(puntInferiorEsq.X - puntInferiorEsqIS.X,2))
            build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(desplY = round(puntInferiorEsq.Y - puntInferiorEsqIS.Y,2))


            llargadaTub = llargadaMesGranAct[0].SizeX
            if llargadaTub < llargadaMesGranAct[0].SizeY:#Vertical
                llargadaTub = llargadaMesGranAct[0].SizeY
                build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(RotarTub = True)

                build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(desplX = round((puntInferiorEsq.X + 50) - puntInferiorEsqIS.X,2) )
                #build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(desplY = round(puntInferiorEsq.Y - puntInferiorEsqIS.Y,2) )
            else:
                build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(TipusTub = "TUB E")

            build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(LlargadaAut = False)
            build_ele.dadesTubHor.value[posBarraHor] = build_ele.dadesTubHor.value[posBarraHor]._replace(Llargada = llargadaTub)


            posBarraHor += 1


        for posBarraHor in range(0,len(build_ele.dadesTubHor.value)):
            if build_ele.dadesTubHor.value[posBarraHor].RotarTub:
                #mirar si es extrem
                if build_ele.dadesTubHor.value[posBarraHor].desplX <= build_ele.dadesTubHor.value[build_ele.ExteriorEsquerra.value].desplX:
                    build_ele.ExteriorEsquerra.value = posBarraHor

                if build_ele.dadesTubHor.value[posBarraHor].desplX >= build_ele.dadesTubHor.value[build_ele.ExteriorDreta.value].desplX:
                    build_ele.ExteriorDreta.value = posBarraHor


        #print("build_ele.ISAmplada.value: " + str(build_ele.ISAmplada.value))
        #print("build_ele.ISllargada.value: " + str(build_ele.ISllargada.value))
        build_ele.IntegerISSelectorHor.value = len(element_listIS)
        #set_values_tub_hor(build_ele, len(element_listIS)-1, 0)


        #placement_mat = AllplanGeo.Matrix3D()
        #placement_mat.SetTranslation(AllplanGeo.Vector3D(puntInferiorEsqIS))

def asignar_colindants(build_ele,nHorSeleccionat):
    '''
    for nPosTubCalc in range(0, len(build_ele.dadesTubHor.value)):
        #detectar si es hor o vert
        esVert = build_ele.dadesTubHor.value[nPosTubCalc].RotarTub
        #calcular posInf i PosSup del tub
        ...
        trobatSup = False
        nPosTubaBuscar = 0
        while not trobatInf and not trobatSup and nPosTubaBuscar < len(build_ele.dadesTubHor.value):

            #si es l'invers (hor->vert, vert->hor)
            if esVert != build_ele.dadesTubHor.value[nPosTubaBuscar].RotarTub:
                #si posInf coincideix amb costat
                if posInfCalc :
...
def rotar_elements_solids(build_ele, puntInferiorEsqIS, placement_mat, element_listIS):
    elements_listNew = []


    doc = AllplanElementAdapter.DocumentAdapter()
    view_world_projection= AllplanIFW.ViewWorldProjection()
    puntInfEsq2D = AllplanGeo.ConvertTo2D(puntInferiorEsqIS)
    AllplanBaseElements.RotateElements(doc, element_listIS, puntInfEsq2D, 90, view_world_projection)
...
def set_values_tub_hor(build_ele, j, actual):
    ...
    while len(build_ele.dadesTubHor.value) <= j:
        ...
        build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(LlargadaAut = False)
        build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(Gruix = 1.5)
        ...
        build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(mostrarLiniaA = False)
        ...
        build_ele.dadesTubHor.value[j] = build_ele.dadesTubHor.value[j]._replace(posAutomatica = False)
...
def detectar_vertical_anterior_solids(build_ele, x, nHorSeleccionat, posX , posY):
    '''
    x = tub Horitzontal
...
def detectar_vertical_seguent_solids(build_ele, x, nHorSeleccionat, posX , posY ):
    '''
    x = tub Horitzontal
...
def detectar_horitzontal_anterior_solids(build_ele, x, nHorSeleccionat, posX = 0 , posY = 0):
    '''
    x = tub Vertical
    ...
    for posTub in range(0, len(build_ele.dadesTubHor.value)):
        if not build_ele.dadesTubHor.value[posTub].RotarTub and build_ele.dadesTubHor.value[posTub].Mostrar:
            if build_ele.dadesTubHor.value[posTub].desplY < posY:#get_posicioY(build_ele, posTub) < posY:
                sumatoriX = 0
                #if build_ele.dadesTubHor.value[posTub].posAutomatica:
                #    nTubInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[posTub].tubInferior, 3)
                #    sumatoriX = 50 + get_posicioX(build_ele, nTubInf)
                if build_ele.dadesTubHor.value[posTub].desplX + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriX <= posX  and \
                    ...
                    if anteriorMesProper == None:
                        anteriorMesProper = posTub
                    elif build_ele.dadesTubHor.value[posTub].desplY > build_ele.dadesTubHor.value[anteriorMesProper].desplY:
                    #elif get_posicioY(build_ele, posTub) > get_posicioY(build_ele, anteriorMesProper):
                        anteriorMesProper = posTub
...
def detectar_horitzontal_seguent_solids(build_ele, x, nHorSeleccionat, posX = 0, posY = 0):
    '''
    x = tub Vertical
    ...
    for posTub in range(0, len(build_ele.dadesTubHor.value)):
        if not build_ele.dadesTubHor.value[posTub].RotarTub and build_ele.dadesTubHor.value[posTub].Mostrar:
            #print(str(posTub) + " : " + str(get_posicioY(build_ele, posTub)) + " > " + str(get_posicioY(build_ele, seguentExtrem)))
            #if seguentExtrem == None or build_ele.dadesTubHor.value[posTub].desplY > build_ele.dadesTubHor.value[seguentExtrem].desplY:
            #    seguentExtrem = posTub
            if build_ele.dadesTubHor.value[posTub].desplY >= posY:
                sumatoriX = 0


                #if build_ele.dadesTubHor.value[posTub].posAutomatica:
                #    nTubInf = getNumFromText(build_ele, build_ele.dadesTubHor.value[posTub].tubInferior, 3)
                #    sumatoriX = 50 + build_ele.dadesTubHor.value[nTubInf].desplX

                if build_ele.dadesTubHor.value[posTub].desplX + build_ele.dadesTubHor.value[posTub].retallInicial + sumatoriX <= posX  and \
                    build_ele.dadesTubHor.value[posTub].desplX + build_ele.dadesTubHor.value[posTub].retallInicial + build_ele.dadesTubHor.value[posTub].Llargada + sumatoriX >= posX :
                    if seguentMesProper == None:
                        seguentMesProper = posTub
                    elif build_ele.dadesTubHor.value[posTub].desplY < build_ele.dadesTubHor.value[seguentMesProper].desplY:
                    #elif get_posicioY(build_ele, posTub) < get_posicioY(build_ele, seguentMesProper):
                        seguentMesProper = posTub
...
def assignar_infSup_corresponents(build_ele, nHorSeleccionat):
    ...
    for posTub in range(0, lenDadesTubHorAct):
        desplX = build_ele.dadesTubHor.value[posTub].desplX
        desplY = build_ele.dadesTubHor.value[posTub].desplY
        llargada = build_ele.dadesTubHor.value[posTub].Llargada
        posAutomatica = build_ele.dadesTubHor.value[posTub].posAutomatica
        #print(str(posTub))

        if not build_ele.dadesTubHor.value[posTub].RotarTub:
            anteriorMesProper = detectar_vertical_anterior_solids(build_ele, posTub, nHorSeleccionat, desplX, desplY)
            posAntMesProper = build_ele.dadesTubHor.value[anteriorMesProper].desplX

            #distanciaMenora50 = False
            #if desplX - posAntMesProper < 50 and desplX - posAntMesProper > (-50):
            #    distanciaMenora50 = True

            #anteriorMesProper = detectar_vertical_anterior(build_ele, -1, nHorSeleccionat, desplX, desplY)
            if anteriorMesProper != None and anteriorMesProper != posTub :#and distanciaMenora50:
                asignar_barra_inferior(build_ele, posTub, nHorSeleccionat, anteriorMesProper)
                posAutomatica = build_ele.dadesTubHor.value[posTub].posAutomatica
                #build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(desplX = build_ele.dadesTubHor.value[posTub].desplX - 50)
            if posAutomatica:
                desplX += 50
            seguentMesProper = detectar_vertical_seguent_solids(build_ele, posTub, nHorSeleccionat, round(desplX + llargada,2),  desplY )
            #seguentMesProper = detectar_vertical_seguent(build_ele, -1, nHorSeleccionat, desplX, desplY)
            if seguentMesProper != None and seguentMesProper != posTub :
                asignar_barra_superior(build_ele, posTub, nHorSeleccionat, seguentMesProper)

        else:
            #if build_ele.dadesTubHor.value[posTub].RotarTub:

            #anteriorMesProper = detectar_horitzontal_anterior(build_ele, posTub, nHorSeleccionat )
            anteriorMesProper = detectar_horitzontal_anterior_solids(build_ele, posTub, nHorSeleccionat, desplX, desplY)
            #anteriorMesProper = detectar_horitzontal_anterior(build_ele, -1, nHorSeleccionat, desplX, desplY )
            if anteriorMesProper != None and anteriorMesProper != posTub:
                asignar_barra_inferior(build_ele, posTub, nHorSeleccionat, anteriorMesProper)
                build_ele.dadesTubHor.value[posTub] = build_ele.dadesTubHor.value[posTub]._replace(desplX = build_ele.dadesTubHor.value[posTub].desplX - 50)
            #seguentMesProper = detectar_horitzontal_seguent(build_ele, posTub, nHorSeleccionat)
            seguentMesProper = detectar_horitzontal_seguent_solids(build_ele, posTub, nHorSeleccionat, desplX, round(desplY + llargada,2))
            #seguentMesProper = detectar_horitzontal_seguent(build_ele, -1, nHorSeleccionat,desplX, desplY)
            if seguentMesProper != None and seguentMesProper != posTub:
                asignar_barra_superior(build_ele, posTub, nHorSeleccionat, seguentMesProper)
...
class IS_Conjunt_8():
    ...
    def __init__(self, build_ele, doc, element_listIS = [], placement_mat = AllplanGeo.Matrix3D()):
    ...
    def create(self):
        ...
        result = create_element_class(self.build_ele_TD , self.doc_TD, self.placement_mat, self.element_listIS)
        model_elem_list = result["elements"]
        handle_list = result["handles"]
        model_elem_list_preview = result["preview_elements"]
