# -*- coding: utf-8 -*-
"""
PointInput – Captura por roles + Editor de polilíneas en un único script.

CAPTURA (sin preview):
- Seteás el rol con botones 2100..2103 (0 Inicial, 1 Paso, 2 Bifurcación, 3 Final).
- Al marcar 'Final' se crea Polyline3D en el documento y se exporta/mergea JSON por PathId.
- ESC en captura: cierra corridas abiertas, crea y exporta JSON.

EDITOR (con preview rico):
- Botones: 1001=Crear (toggle), 1002=Guardar, 1003=Finalizar y crear, 1004=Borrar sección, 1006=Bifurcar.
- Checkbox 'CheckBoxValue' = modo insertar punto en segmentos/midpoints.
- Doble clic en extremo de poly guardada = extender.
- Selección por marco para borrar múltiples segmentos.

Requiere en paleta:
 PathId, SegmentId, PointRole, EnableAssistWndClick, EnableZCoordinate, EnableUndoStep, SetProjectionBase0,
 CommonProp, CreateSymbol, SymbolCommonProps, CheckBoxValue (insert mode) y los botones arriba.
"""
...
class UnifiedScriptObject(BaseScriptObject):
    ...
    def _ensure_capture(self):
        ...
        # Allplan nos provee coord_input en start_input del interactor
...
class CaptureInteractor:
    ...
    def _init_coord_prompt(self):
        ...
        try:
            ...
            msg_role = "Paso"
        prompt = AllplanIFW.InputStringConvert(f"Click ({msg_role}). Botones: Inicial/Paso/Bifurcación/Final")
    ...
    def on_preview_draw(self):
        if self.coord_input:
            self.coord_input.GetCurrentPoint()  # no preview
    ...
    def process_mouse_msg(self, mouse_msg: int,
        ...
        if self.input_pnt is not None:
            ...
            if getattr(self.build_ele, "CreateSymbol", False):
                self._create_point_symbol(self.input_pnt)
    ...
    def _create_point_symbol(self, p: AllplanGeo.Point3D):
        if not self.coord_input or p is None:
            return
        try:
            sym_props = AllplanBasisElements.Symbol3DProperties()
            sym_props.IsScaleDependent = False  # type: ignore
            sym_props.SymbolID         = 1      # type: ignore
            ...
            AllplanBaseElements.CreateElements(
                ...
                AllplanGeo.Matrix3D(),
                [AllplanBasisElements.Symbol3DElement(common_props, sym_props, p)], [], None
            )
        except Exception as e:
            print("[SYMBOL] EXC:", e); _flush()

# -------------------- EDITOR (con preview) --------------------
# === Config ===
HANDLE_SIZE            = 90.0
HIT_TOL_VERTEX         = 30.0
HIT_TOL_SEGMENT        = 80.0
HOVER_SCALE            = 1.25
DRAG_SCALE             = 1.5
...
class PolylineInteractor:
    ...
    # --- NEW: helpers para buscar lo más cercano en polilíneas guardadas ---
    def _nearest_saved_vertex(self, p: AllplanGeo.Point3D, tol_mult: float = 1.25):
        """
        Devuelve (path_idx, vertex_idx, punto) del vértice guardado más cercano a p
        dentro de una tolerancia (HIT_TOL_VERTEX * tol_mult). Si no hay, devuelve None.
        """
    ...
    def _nearest_saved_segment(self, p: AllplanGeo.Point3D, tol_mult: float = 1.5):
        """
        Devuelve (seg_tuple, q) donde seg_tuple=('saved', path_idx, seg_idx) es el
        segmento guardado más cercano a p dentro de (HIT_TOL_SEGMENT * tol_mult) y
        q es el punto proyectado sobre ese segmento. Si no hay, devuelve (None, None).
        """
        max_d2 = (HIT_TOL_SEGMENT * tol_mult) ** 2
        best = None      # ('saved', path_idx, seg_idx)
        best_q = None    # punto proyectado
        best_d2 = max_d2 + 1.0
    ...
    def delete_selected_or_hovered_segment(self) -> bool:
        # Prioridad: seleccionado -> hover -> más cercano al cursor -> fallback último tramo
        target = self.selected_seg if self.selected_seg is not None else self.hover_seg

        curr = self.coord_input.GetCurrentPoint(self.current_point).GetPoint()

        if target is None:
            # Buscar el segmento guardado más cercano al cursor (tolerancia ampliada)
            seg, _ = self._nearest_saved_segment(curr, tol_mult=1.75)
            if seg:
                target = seg

        if target is None:
            # Fallback: si hay una sola polilínea guardada, borrar su último tramo
