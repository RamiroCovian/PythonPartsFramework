...
"""
import random
from .PLD_inclination import PLD
from .XPS_inclination import XPS
...
print('Load FS inclination rotate.py')
...
def create_element_class(build_ele, doc, placement_mat = AllplanGeo.Matrix3D()):
    ...
    inclination_rotation_angle = build_ele.RotationInputAngle.value
    inclination_selector_axis = build_ele.SelectorAxis.value


    # Define model element and add figures
    model_elements = ModelEleList()

    pld_element = PLD(doc, z_unique, ancho_pld, alto_pld, z_rotation, type_pld, layer_value_name, borde_afilado_selector, inclination_rotation_angle, inclination_selector_axis)
    xps_element = XPS(doc, z_unique, ancho_pld, alto_pld, grosor_xps, z_rotation, layer_value_name, inclination_rotation_angle, inclination_selector_axis)
...
class FS():
    ...
    def rotate_element(self, geo_object):
        rotation_point = AllplanGeo.Point2D(0, 0)
        rotation_angle = AllplanGeo.Angle.FromDeg(15)

        translated_geometry = AllplanGeo.Rotate(geo_object,
                                                rotation_point,
                                                rotation_angle)
