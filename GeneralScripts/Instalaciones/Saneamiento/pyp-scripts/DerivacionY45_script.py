# -*- coding: utf-8 -*-
"""Bifurcación Y Ø40, derivación 45°."""
...
class DerivacionY45D40:
    ...
    def _add_user_attributes_40mm(self, attr_list):
        ...
        try:
            ...
            if id_nom and id_nom > 0:
                attr_list.append(
                    AllplanBaseElements.AttributeString(id_nom, "DER. Y 40Ø 45°")
                )
            if id_pes and id_pes > 0:
                attr_list.append(AllplanBaseElements.AttributeDouble(id_pes, 0.0654))
            if id_sec and id_sec > 0:
                try:
                    attr_list.append(AllplanBaseElements.AttributeInt(id_sec, 40))
                except Exception:
                    attr_list.append(AllplanBaseElements.AttributeString(id_sec, "40"))
        except Exception as e:
            print(f"[DerivacionY45_script] Atributos usuario 40mm: {e}")
    ...
    def create_result(self) -> CreateElementResult:
        ...
        p2 = AllplanGeo.Point3D(
            0, 0, half_largo - rama_offset_z_local + h["OFFSET_RAMA_40MM"]
        )
...
def create_element(build_ele, _doc) -> CreateElementResult:
    return DerivacionY45D40(build_ele, _doc).create_result()


class DerivacionY45Script(BaseScriptObject):
