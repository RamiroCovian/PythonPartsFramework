# -*- coding: utf-8 -*-
"""Codo rígido fecal Ø110 a 45° (portado de Saneamiento_old)."""
...
class CodoRigido110mF45:
    ...
    def _add_user_attributes_110mm_45(self, attr_list):
        """Atributos de usuario exactos de Saneamiento_old para 110mm 45°."""
        if not self.doc:
            return
        try:
            ...
            if attr_pmp_carticulo_id and attr_pmp_carticulo_id > 0:
                attr_list.append(
                    AllplanBaseElements.AttributeString(attr_pmp_carticulo_id, "KN07_004_003")
                )
            if attr_pmp_nom_id and attr_pmp_nom_id > 0:
                attr_list.append(
                    AllplanBaseElements.AttributeString(attr_pmp_nom_id, "COLZE M-F 110Ø 45°")
                )
            if attr_pmp_pes_unitari_id and attr_pmp_pes_unitari_id > 0:
                attr_list.append(
                    AllplanBaseElements.AttributeDouble(attr_pmp_pes_unitari_id, 0.2110)
                )
            if attr_pmp_seccio_id and attr_pmp_seccio_id > 0:
                attr_list.append(
                    AllplanBaseElements.AttributeString(attr_pmp_seccio_id, "Ø110/45°")
                )
        except Exception as e:
            print(
                f"[Colze_rigid_110m_f_45_script] Advertencia atributos 110mm 45°: {e}"
            )
...
def create_element(build_ele, _doc) -> CreateElementResult:
    return CodoRigido110mF45(build_ele, _doc).create_result()
...
class CodoRigido110mF45Script(BaseScriptObject):
    """PythonPart: codo rígido fecal Ø110 para 45°."""
