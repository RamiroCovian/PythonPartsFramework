...
def try_apply_old_d110_d110_bif_transform(
    ...
    """
    Igual que el port D110-D110: rota el BRep y traslada a vértice + offset base
    (ox,oy,oz) ya rotado.

    ``extra_dx/dy/dz`` (p. ej. geo_handler.BIF_Y110_40_PLACEMENT_OFFSET_*) van en el
    mismo marco local que BIF110_D110_OFFSET_* y se rotan con la misma secuencia
    Rx→Ry→Rz; no son traslación en ejes mundo (eso desplazaba mal la TE 110-110-40
    según el rumbo del troncal en XY).
    """
    ...
    ex, ey, ez = _rotate_offset_like_old(
        rot_x, rot_y, rot_z, float(extra_dx), float(extra_dy), float(extra_dz)
    )
