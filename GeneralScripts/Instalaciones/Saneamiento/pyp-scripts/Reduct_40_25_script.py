...
Compatible con el flujo nuevo de saneamiento:
- uso clásico vía ``create_element``
- uso dinámico vía clase con ``set_diameters`` + ``build`` (reductor 40-25 / 25-40)
"""
...
# ===== Matriz de traslación (mm) — desplaza todo el conjunto en X, Y, Z =====
# Se construye con Matrix3D.SetTranslation y se aplica tras ROT_* y el giro 90° en Y.
TRANSLATION_X_MM = -30.0
TRANSLATION_Y_MM = -12.50
TRANSLATION_Z_MM = 12.50

SUPPORTED_REDUCTION_PAIRS = {(40, 25), (25, 40)}


def _translation_matrix(tx_mm: float, ty_mm: float, tz_mm: float) -> AllplanGeo.Matrix3D:
    """Matriz de transformación con solo traslación (equivalente a T(tx,ty,tz) en mm)."""
    m = AllplanGeo.Matrix3D()
    m.SetTranslation(AllplanGeo.Vector3D(float(tx_mm), float(ty_mm), float(tz_mm)))
    return m


def _diameter_labels(d1: int, d2: int):
    d_max = max(int(d1), int(d2))
    d_min = min(int(d1), int(d2))
    label_red = f"RED.Ø{d_max}-{d_min}"
    label_diam = f"Ø{d_max}-{d_min}"
    label_seccio = f"{d_max}-{d_min}"
    return label_red, label_diam, label_seccio
...
def create_element(_be, _doc, _return_raw_list=False, _d1=40, _d2=25):
    ...
    # La separación permite manipularlo de forma independiente del reductor.
    ...
    # Traslación global: misma matriz para cuerpo principal y cuboides (mover en X,Y,Z).
    mat_traslacion = _translation_matrix(
        TRANSLATION_X_MM, TRANSLATION_Y_MM, TRANSLATION_Z_MM
    )
    brep = AllplanGeo.Transform(brep, mat_traslacion)
    cuboide_centro = AllplanGeo.Transform(cuboide_centro, mat_traslacion)
    ...
    attr_list = []
    label_red, label_diam, label_seccio = _diameter_labels(_d1, _d2)
    attr_list.append(AllplanBaseElements.AttributeString(1083, label_red))  # Custom attribute 01
    attr_list.append(AllplanBaseElements.AttributeString(1084, label_diam))     # Custom attribute 02
    ...
    attr_list.append(AllplanBaseElements.AttributeString(1896, label_seccio))      # Custom attribute 07
    ...
    if _doc:
        _add_user_attributes_reduction(attr_list, _doc, _d1, _d2)
    ...
    if _doc:
        ...
        try:
            ...
            print(f"[Reduct_40_25_script] Advertencia al agregar Material a copia: {e}")
...
def _add_user_attributes_reduction(attr_list, doc, d1=40, d2=25):
    """Agrega atributos de usuario para el reductor (original)."""
    try:
        label_red, label_diam, label_seccio = _diameter_labels(d1, d2)
        ...
        if attr_pmp_diametre_id and attr_pmp_diametre_id > 0:
            attr_list.append(AllplanBaseElements.AttributeString(attr_pmp_diametre_id, label_diam))

        # pmp_nom: "RED.Ø40-25"
        if attr_pmp_nom_id and attr_pmp_nom_id > 0:
            attr_list.append(AllplanBaseElements.AttributeString(attr_pmp_nom_id, label_red))

        # pmp_seccio: "40-25"
        if attr_pmp_seccio_id and attr_pmp_seccio_id > 0:
            attr_list.append(AllplanBaseElements.AttributeString(attr_pmp_seccio_id, label_seccio))

    except Exception as e:
        # No bloquear la creación si hay problemas con los atributos
        print(f"[Reduct_40_25_script] Advertencia al agregar atributos de usuario: {e}")

class Reduct4025Model:
    """Modelo de reductor compatible con el cargador dinámico de saneamiento."""
    ...
    def set_diameters(self, d1, d2):
        try:
            ...
            self.d1, self.d2 = 40, 25
        self.type_manguito = f"RED_{max(self.d1, self.d2)}_{min(self.d1, self.d2)}"

    def build(self):
        if (self.d1, self.d2) not in SUPPORTED_REDUCTION_PAIRS:
            return []
        return create_element(
            self.build_ele,
            self.doc,
            _return_raw_list=True,
            _d1=self.d1,
            _d2=self.d2,
        ) or []


# Alias para facilitar importación con distintos nombres esperados.
TapReductionModel = Reduct4025Model
ManguitoModel = Reduct4025Model
TapreductionModel = Reduct4025Model
