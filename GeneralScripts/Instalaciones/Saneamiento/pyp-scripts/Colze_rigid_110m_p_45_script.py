...
class CodoRigido110mP45:
    ...
    PMP_DIAMETRE_STR = "110"
    PMP_NOM = "CS45ºØ110"
    PMP_PES_UNITARI = 0.2110
    PMP_SECCIO = "Ø110/45º"
    ATTR_6_CC_IS = ""
    OBJECT_NAME = "COLZE MASCLE-FEMELLA 45º"
    FACTOR = 1.0
    CALCULATION_MODE = "m³"
    ...
    @staticmethod
    def _attr_id_first(doc, *names: str) -> int:
        """Resuelve el primer nombre de atributo que exista en el proyecto."""
        for name in names:
            if not name:
                continue
            try:
                aid = AllplanBaseElements.AttributeService.GetAttributeID(doc, name)
                if aid and aid > 0:
                    return int(aid)
            except Exception:
                continue
    ...
    def _add_user_attributes_110mm_45(self, attr_list):
        ...
        try:
            ...
            if attr_pmp_diametre_id and attr_pmp_diametre_id > 0:
                attr_list.append(
                    AllplanBaseElements.AttributeString(
                        attr_pmp_diametre_id, self.PMP_DIAMETRE_STR
                    )
                )
            ...
            # Nombre de objeto, factor y modo de cálculo (IDs por nombre; catálogo ES/EN).
            oid = self._attr_id_first(
                self.doc,
                "Nombre de objeto",
                "Object_name",
            )
            if oid:
                attr_list.append(
                    AllplanBaseElements.AttributeString(oid, self.OBJECT_NAME)
                )

            fid = self._attr_id_first(self.doc, "Factor")
            if fid:
                attr_list.append(
                    AllplanBaseElements.AttributeDouble(fid, float(self.FACTOR))
                )

            cid = self._attr_id_first(
                self.doc,
                "Modo de cálculo",
                "Calculation mode",
            )
            if cid:
                attr_list.append(
                    AllplanBaseElements.AttributeString(cid, self.CALCULATION_MODE)
                )
