...
def _apply_own_attributes_110_pluvial(model_elem, doc) -> None:
    """Asigna atributos propios del TS-110 pluvial según ficha solicitada."""
    try:
        attr_list = [
        ...
        if doc:
            ...
            if id_cart and id_cart > 0:
                attr_list.append(AllplanBaseElements.AttributeString(id_cart, "KN07_005_004"))
        ...
        print(f"[TubPvcTricapaP110] Advertencia al asignar atributos propios: {ex}")
...
def _create_pluvial_110_with_own_arrow(
    ...
    tube_geom = _rotate_z(tube.GetGeometryObject())
    tube = AllplanBasisElements.ModelElement3D(tube.GetCommonProperties(), tube_geom)
    _apply_own_attributes_110_pluvial(tube, doc)
