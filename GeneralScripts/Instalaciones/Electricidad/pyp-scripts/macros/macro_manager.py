# -*- coding: utf-8 -*-
"""
ElectricidadMacroManager — Electricidad-specific subclass of MarkerManager.

Overrides palette-reading methods, element geometry creation, cursor preview,
and room/local selection for the Electricidad installation.
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ELECTRICIDAD MACRO MANAGER
# ═══════════════════════════════════════════════════════════════════════════════

class ElectricidadMacroManager(MarkerManager):
    """Electricidad-specific overrides for macro/element marker management.

    Reads palette parameters specific to the Electricidad installation
    ...
    def get_macro_settings_from_palette(self) -> Optional[dict]:
        """Read macro configuration from Electricidad palette parameters."""
    ...
    def get_element_settings_from_palette(self) -> Tuple[str, float]:
        """Read element type key and radius from Electricidad palette."""
        element_label = str(
        ...
        try:
            ...
            print(f"[ELEC_MGR] Error resolving element type from modules: {ex}")
            element_type = "circle"

        if element_type == "circle":
            print(f"[ELEC_MGR] WARNING: Could not resolve element '{element_label}' from pythonparts_modules. "
    ...
    def get_element_z_abs(self) -> float:
        """Read absolute Z coordinate for defined elements from Electricidad palette."""
        try:
            return float(
    ...
    def get_element_rotation(self) -> Tuple[float, float, float]:
        """Read rotation angles from Electricidad palette."""
    ...
    def create_element_geometry(self, marker: dict) -> List[Any]:
        ...
        if pos is None:
            print(f"[ELEC_MGR] create_element_geometry: pos is None for marker")
            return []

        element_type = str(marker.get("element_type", "circle") or "circle")
        print(f"[ELEC_MGR] create_element_geometry: type={element_type}, pos=({pos.X:.1f}, {pos.Y:.1f}, {pos.Z:.1f})")
        ...
        print(f"[ELEC_MGR] create_element_geometry: got {len(result)} model elements")
        return result
    ...
    def append_element_pythonparts(self, pythonpart_group_list: list, build_ele: Any) -> None:
        ...
        if not self.element_markers:
            print("[ELEC_MGR] append_element_pythonparts: no element_markers, skipping")
            return

        print(f"[ELEC_MGR] append_element_pythonparts: {len(self.element_markers)} markers")

        marker_groups = self._create_defined_element_model_elements_grouped()
        if not marker_groups:
            print("[ELEC_MGR] append_element_pythonparts: marker_groups is empty!")
            return

        print(f"[ELEC_MGR] marker_groups count={len(marker_groups)}, "
        ...
        try:
            ...
            for i, group in enumerate(marker_groups):
                if not group:
                    print(f"[ELEC_MGR]   group[{i}] is empty, skipping")
                    continue
                print(f"[ELEC_MGR]   group[{i}] has {len(group)} model elements")
                try:
                    ...
                    print(f"[ELEC_MGR]   group[{i}] → {len(pps)} PythonParts created")
                except Exception as ex:
                    print(f"[ELEC_MGR]   group[{i}] create_individual_pythonpart error: {ex}")
                    import traceback
                    traceback.print_exc()
        except Exception as e:
            print(f"[ELEC_MGR] Error appending element PythonParts: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _load_element_module_by_file(self, element_type: str) -> Any:
        ...
        (e.g. 'pyp-scripts.Elementos.caixa_connexions_200_script').
        """
        pythonparts_modules = getattr(self.script_object, "pythonparts_modules", [])
        elem_def = None
        for item in pythonparts_modules or []:
        ...
        module_name = f"_elec_elem_{relative_parts[-1]}"
        ...
        try:
            ...
            print(f"[ELEC_MGR] _load_element_module_by_file error: {ex}")
    ...
    def _get_model_elems_from_module(self, element_type: str) -> List[Any]:
        ...
        try:
            ...
            print(f"[ELEC_MGR] _get_model_elems_from_module error for '{element_type}': {ex}")
        return []
