...
from .vehicle_electric_3D import VehicleElectric3D
...
def create_preview(build_ele, script_object_data):
    obj = VehicleElectric3D()
    model_ele_list = obj.create_geometry_3d()
    return model_ele_list, []


def create_script_object(build_ele, script_object_data):
    return VehicleElectricScript(build_ele, script_object_data)


class VehicleElectricScript(BaseScriptObject):
