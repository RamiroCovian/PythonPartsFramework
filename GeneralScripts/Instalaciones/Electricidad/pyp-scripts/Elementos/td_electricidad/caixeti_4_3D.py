...
class Caixeti4Object3D:
    ...
    INT_EXT_HEIGHT   = 68.0
    INT_EXT_WIDTH    = 276.07   # proporcional a caixeti_3 (207.05 × 4/3)
    INT_EXT_DEPTH    = 50.0
    ...
    def _create_cuerpo(self, color: int) -> list:
        half_w   = self.WIDTH / 2.0      # 140mm
        half_d   = self.DEPTH / 2.0      # 20mm
        top_half = self.TOP_WIDTH / 2.0  # 65mm
        y_plane  = -half_d
    ...
    def _create_interior(self, color: int) -> list:
        ...
        half_d        = self.DEPTH / 2.0         # 20mm
        y_plane       = -half_d
        half_straight = (self.WIDTH - 2.0 * r) / 2.0  # 105mm
        mid_z         = 0.0
        n             = self.ARC_SEGMENTS
    ...
    def _create_cilindros_base(self, color: int) -> ModelEleList:
        ...
        Ubicados en la base del estadio interior (z=-35), centros en x=±105 (centro de cada semicírculo).
        Flush con el cuerpo: y=-20 a y=+20.
        """
        half_d        = self.DEPTH / 2.0
        cyl_radius    = 10.0
        half_straight = (self.WIDTH - 2.0 * self.INT_RADIUS) / 2.0  # 105mm
        cyl_x         = half_straight
        cyl_z         = -self.INT_RADIUS      # -35mm
    ...
    def _create_caja_trasera(self, color: int) -> list:
        """
        Caja 210mm × 68mm × 16mm centrada en x=0, z=0.
        Parte de la cara trasera (y=+20) y sobresale 16mm (hasta y=+36).
        """
        half_d   = self.DEPTH / 2.0       # 20mm
        half_x   = 210.0 / 2.0            # 105mm
    ...
    def _create_cilindros_traseros(self, color: int) -> ModelEleList:
        ...
        Centros: x = ±(half_w - margin - radius) = ±(140 - 1 - 34) = ±105mm, z = 0.
        Extruyen hacia afuera: y=+20 a y=+36.
        """
        half_w  = self.WIDTH / 2.0                                        # 140mm
        half_d  = self.DEPTH / 2.0                                        # 20mm
        cyl_x   = half_w - self.CYL_BACK_MARGIN - self.CYL_BACK_RADIUS   # 105mm
        y_dir   = AllplanGeo.Vector3D(0, 1, 0)
