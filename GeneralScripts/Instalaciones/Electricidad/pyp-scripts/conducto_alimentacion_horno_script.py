...
def create_preview(build_ele, script_object_data):
    ...
    model_ele_list = conducto_3D.create_conducto_geometry_3D(color=28)
    return model_ele_list, []


def create_script_object(build_ele, script_object_data):
    return ConductoAlimentacionHornoScript(build_ele, script_object_data)


class ConductoAlimentacionHornoScript(BaseScriptObject):
    """PythonPart para conducto Alimentación horno (corrugados)."""
    ...
    def get_attributes_custom(self, value: float = 0, tipo: str | None = None,
        ...
        Atributos técnicos del conducto (Atributo personalizado):
          ...
          06 → Color del cable (nombre, p.ej. "Blanc")
    ...
    def execute(self, diameter=None) -> CreateElementResult:
        ...
        selec_color = 28
        length = 1000.0
        width = diameter if diameter else 25.0
        height = diameter if diameter else 25.0
