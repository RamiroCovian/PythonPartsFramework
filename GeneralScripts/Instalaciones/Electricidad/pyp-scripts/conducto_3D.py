...
class ConductoObject3D:
    ...
    def create_conducto_geometry_3D(self, length: float = 500, witdh: float = 75, height: float = 75, color: int = 1) -> ModelEleList:
        # Si es eléctrico, queremos que sea CUADRADO con el diámetro especificado
        # El interactor envía 'witdh' y 'height' como el diámetro del tubo.
        conexion_desc  = {
            "bases": [
                {"type": "cubo", "length": length, "width": witdh,"height": height, "base_point": (0, 0, 0), "color": color, "operations": []},
            ]}
