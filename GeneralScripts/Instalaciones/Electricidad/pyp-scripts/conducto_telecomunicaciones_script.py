...
def create_script_object(build_ele, script_object_data):
    return ConductoTelecomunicacionesScript(build_ele, script_object_data)


# --- Clase Principal del PythonPart ---
class ConductoTelecomunicacionesScript(BaseScriptObject):
    """PythonPart para conducto de Telecomunicaciones (corrugados)."""
    ...
    def get_attributes_user(self, diameter: float = 20.0, color: int = 0) -> list:
        ...
        try:
            ...
            tipo_cablejat = f"Corrugat buit de Ø{int(diameter)}"
            ...
            user_attributes.append(AllplanBaseElements.AttributeString(attr_ids["pmp_CARTICULO"], "KN02_004_007"))
            ...
            user_attributes.append(AllplanBaseElements.AttributeString(attr_ids["pmp_tipus_cablejat"], tipo_cablejat))
    ...
    def get_attributes_custom(self, value: float = 0, tipo: str | None = None,
        ...
        Atributos técnicos del conducto (Atributo personalizado):
          04 → Tipo derivado del layer: TELECOS SOBRE REJIBAND / D'ENTRADA / DE SORTIDA
          06 → Color del cable (nombre, p.ej. "Lila")
        ...
        try:
            ...
            print(f"[Telecos] Error en get_attributes_custom: {e}")
        return attr_list
    ...
    def execute(self, diameter=None) -> CreateElementResult:
        ...
        selec_color = 15
        length = 1000.0
        width = diameter if diameter else 20
        height = diameter if diameter else 20
