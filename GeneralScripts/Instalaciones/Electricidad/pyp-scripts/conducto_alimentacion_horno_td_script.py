...
_COLOR_INNER = 28   # mismo que IS alimentación horno
...
def create_preview(build_ele, script_object_data):
    ...
    desc = {
        "bases": [
            {"type": "cubo", "length": 1000.0, "width": 25.0, "height": 25.0,
             "base_point": (0, 0, 0), "color": _COLOR_INNER, "operations": []},
            {"type": "cubo", "length": 1000.0, "width": 25.0, "height": 25.0 + _OUTER_EXTRA,
...
def create_script_object(build_ele, script_object_data):
    return ConductoAlimentacionHornoTdScript(build_ele, script_object_data)


# --- Clase Principal del PythonPart ---
class ConductoAlimentacionHornoTdScript(BaseScriptObject):
    """PythonPart para conducto Alimentación Horno TD (encapsulado XPS)."""
    ...
    def get_attributes_user(self, diameter: float = 25.0, color: int = 0) -> list:
        user_attributes = []
        try:
            attr_ids = {
            ...
            user_attributes.append(AllplanBaseElements.AttributeString(attr_ids["pmp_CARTICULO"], "KN02_004_006"))
            ...
            user_attributes.append(AllplanBaseElements.AttributeString(attr_ids["pmp_tipus_cablejat"], "6,0N + 6,0B + 6,0TT"))
    ...
    def get_attributes(self, value=None, tipo=None, color=0, overlap_mm=0.0):
        diameter = float(value) if value else 25.0
        attr_list = []
