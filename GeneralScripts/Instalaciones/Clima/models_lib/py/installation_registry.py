...
def auto_load_installations(
    ...
    if program_data:
        default_candidates.append(
            os.path.join(program_data, "Nemetschek", "Allplan", "2025", "Etc", "PythonPartsFramework", "GeneralScripts")
        )
    # Always include the current working directory to ease local development
    default_candidates.append(os.getcwd())

    for root in search_paths or default_candidates:
        ...
        for sub in base_path.iterdir():
            if not sub.is_dir():
                continue
            for candidate in sub.glob("polyline_*.py"):
                module_rel = candidate.with_suffix("").relative_to(root_path).as_posix().replace("/", ".")
                try:
                    importlib.import_module(module_rel)
                except Exception:
                    continue

    _AUTOLOADED_KEYS.add(base_folder)
