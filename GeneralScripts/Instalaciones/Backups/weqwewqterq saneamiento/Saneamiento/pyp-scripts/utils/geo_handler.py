...
class PipelineProcessor:
    ...
    def _load_manguito_classes(self):
        ...
        try:
            ...
            # Fallback nuevo saneamiento: Tapreduction_010 como reductor 40-25 / 25-40.
            # Se expone con API de manguito (set_diameters + build).
            tap_reduction_path = os.path.join(base_dir, "Tapreduction_010.py")
            if os.path.exists(tap_reduction_path):
                spec_red = importlib.util.spec_from_file_location(
                    "saneamiento_tapreduction_010_runtime",
                    tap_reduction_path,
                )
    ...
    def _get_manguito_models_for_diameters(self, d1, d2, distribution_type="IS"):
        ...
        # Forzar TapReduction_010 para transición 40<->25.
        if pair_sorted == (25, 40) and classes and classes.get("RED") is not None:
            ModelClass = classes.get("RED")
            print(
                f"[AGUA][MANGUITO] usando Tapreduction_010 para par {pair_sorted} "
    ...
    def process(self, segments, default_attrs=None, segment_cuts=None) -> list:
        ...
        from .vertex_utils import is_90_deg_turn, is_straight_turn
        ...
        def _is_single_codo90_25_vertex(p_prev, p_curr, p_next, seg_idx_for_diam: int) -> bool:
            """True si el vértice es codo_90 real de 25mm (no convertido a 2x45)."""
            try:
                ...
                info = getattr(segments[seg_idx_for_diam], "info", None)
                diam = getattr(info, "diameter", None) if info is not None else None
                if isinstance(diam, (list, tuple)) and diam:
                    diam = diam[0]
                if diam is None:
                    return False
                return int(round(float(diam))) == 25
            except Exception:
                return False

        def _is_single_codo45_25_vertex(p_prev, p_curr, p_next, seg_idx_for_diam: int) -> bool:
            """True si el vértice es codo_45 individual de 25mm."""
            try:
                if _is_double45_vertex(p_prev, p_curr, p_next):
                    return False
        ...
        for i, seg_item in enumerate(segments):
            ...
            if i > 0:
                ...
                if _is_double45_vertex(p_prev, p_curr, p_next):
                    ...
                    cuts["start"] = float(cuts.get("start", 0.0)) + DOUBLE45_TRIM_OUT_MM
                # trim_out para codo_90 real Ø25.
                if _is_single_codo90_25_vertex(p_prev, p_curr, p_next, i):
                    cuts = segment_cuts.setdefault(i, {"start": 0.0, "end": 0.0})
                    cuts["start"] = float(cuts.get("start", 0.0)) + CODO90_25_TRIM_OUT_MM
                # trim_out para codo_45 individual Ø25.
                if _is_single_codo45_25_vertex(p_prev, p_curr, p_next, i):
                    cuts = segment_cuts.setdefault(i, {"start": 0.0, "end": 0.0})
            ...
            if i < num_seg - 1:
                ...
                if _is_double45_vertex(p_prev, p_curr, p_next):
                    cuts = segment_cuts.setdefault(i, {"start": 0.0, "end": 0.0})
                    cuts["end"] = float(cuts.get("end", 0.0)) + DOUBLE45_TRIM_IN_MM
                # trim_in para codo_90 real Ø25.
                if _is_single_codo90_25_vertex(p_prev, p_curr, p_next, i):
                    cuts = segment_cuts.setdefault(i, {"start": 0.0, "end": 0.0})
                    cuts["end"] = float(cuts.get("end", 0.0)) + CODO90_25_TRIM_IN_MM
                # trim_in para codo_45 individual Ø25.
                if _is_single_codo45_25_vertex(p_prev, p_curr, p_next, i):
                    cuts = segment_cuts.setdefault(i, {"start": 0.0, "end": 0.0})
            ...
            if i < num_seg - 1:
                ...
                if (
                            ...
                            if n1 > 1e-9 and n2 > 1e-9:
                                ...
                                if nm > 1e-9:
                                    ...
                                    base_codo_1 = AllplanGeo.Point3D(
                                        p_curr.X - (v1u.X * DOUBLE45_AUTO_SEP_MM),
                                        p_curr.Y - (v1u.Y * DOUBLE45_AUTO_SEP_MM),
                                        p_curr.Z - (v1u.Z * DOUBLE45_AUTO_SEP_MM),
                                    )
                                    base_codo_2 = AllplanGeo.Point3D(
                                        p_curr.X + (v2u.X * DOUBLE45_AUTO_SEP_MM),
                                        p_curr.Y + (v2u.Y * DOUBLE45_AUTO_SEP_MM),
                                        p_curr.Z + (v2u.Z * DOUBLE45_AUTO_SEP_MM),
                                    )
                                    ...
                                    p_prev_trim = AllplanGeo.Point3D(
                                        p_prev.X + (v1u.X * DOUBLE45_TRIM_IN_MM),
                                        p_prev.Y + (v1u.Y * DOUBLE45_TRIM_IN_MM),
                                        p_prev.Z + (v1u.Z * DOUBLE45_TRIM_IN_MM),
                                    )
                                    p_next_trim = AllplanGeo.Point3D(
                                        p_next.X - (v2u.X * DOUBLE45_TRIM_OUT_MM),
                                        p_next.Y - (v2u.Y * DOUBLE45_TRIM_OUT_MM),
                                        p_next.Z - (v2u.Z * DOUBLE45_TRIM_OUT_MM),
                                    )
                    ...
                    if turn_fitting_type in ("codo_90", "codo_45"):
                        try:
                            ...
                            if isinstance(diam, (list, tuple)) and diam:
                                diam = diam[0]
                            if diam is not None and int(round(float(diam))) == 25:
                                # Offsets de codos Ø25 aplicados en marco local del giro
                ...
                if True:
                    ...
                    if is_colinear or diam_change:
                        ...
                        # - 25->20: invertir
                        # - 25->40: invertir (Tapreduction_010 base 40->25)
                        need_mirror_x = False
                        try:
                        ...
                        # Offset configurable para Tapreduction_010 (par 40<->25), en ejes locales.
                        pos_manguito = pos_nodo
                        try:
                            is_tap_40_25 = (
                                d_curr is not None
                                and d_next is not None
                                and {int(round(float(d_curr))), int(round(float(d_next)))} == {25, 40}
                            )
                            if is_tap_40_25:
                                def _norm(v):
                                    n = math.sqrt(v.X * v.X + v.Y * v.Y + v.Z * v.Z)
                                ...
                                # Si el reductor se invierte (25->40), el eje longitudinal local se invierte.
                                # Compensamos para que "OFFSET_X" tenga el mismo sentido visual en ambos flujos.
                                off_x = -TAPRED_40_25_OFFSET_X_MM if need_mirror_x else TAPRED_40_25_OFFSET_X_MM
                                off_y = TAPRED_40_25_OFFSET_Y_MM
                                off_z = TAPRED_40_25_OFFSET_Z_MM

                                pos_manguito = AllplanGeo.Point3D(
                                    pos_nodo.X
                                    + (x_local.X * off_x)
                                    + (y_local.X * off_y)
                                    + (z_local.X * off_z),
                ...
                for at_start in (False, True):
                    ...
                    try:
                        ...
                        if is_tap_40_25:
                            ...
                            v_in = getattr(seg_for_conn, "vector_normalizado", AllplanGeo.Vector3D(0.0, 0.0, 0.0))
                            v_out = getattr(next_for_conn, "vector_normalizado", AllplanGeo.Vector3D(0.0, 0.0, 0.0))
                            x_local = _norm(AllplanGeo.Vector3D(v_in.X + v_out.X, v_in.Y + v_out.Y, v_in.Z + v_out.Z))
                            if abs(x_local.X) < 1e-9 and abs(x_local.Y) < 1e-9 and abs(x_local.Z) < 1e-9:
                                x_local = _norm(v_out)
                            z_local = _norm(_cross(v_in, v_out))
                            ...
                            # Compensación de offset longitudinal cuando el reductor va invertido (25->40).
                            off_x = -TAPRED_40_25_OFFSET_X_MM if need_mirror_x else TAPRED_40_25_OFFSET_X_MM
                            off_y = TAPRED_40_25_OFFSET_Y_MM
                            off_z = TAPRED_40_25_OFFSET_Z_MM
