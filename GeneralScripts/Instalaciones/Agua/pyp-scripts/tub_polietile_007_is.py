...
class TubPolietileModel:
    ...
    def __init__(self, build_ele, doc: AllplanElementAdapter.DocumentAdapter):
        ...
        # 1.a) Intentar leer TipoDeAguaIS (nuevo nombre desde polilínea)
        type_water = None
        type_water_raw = getattr(build_ele, "TipoDeAguaIS", None)
        ...
        if type_water not in PARAMS:
            print(
                f"[TubPolietileIS] Warning: Tipo de agua '{type_water}' no válido, usando 'Fred' por defecto"
            )
            type_water = "Fred"

        self.type_water = type_water
        print(f"[TubPolietileIS] Tipo de agua seleccionado: '{self.type_water}'")
        ...
        # Log de control para comprobar que el override se aplica correctamente
        print(f"[TubPolietileIS] DiametroAplicar={diametro_attr} -> idx={idx}")
    ...
    def build(self):
        ...
        mat_center.SetTranslation(AllplanGeometry.Vector3D(0.0, -half_y, -half_y))
