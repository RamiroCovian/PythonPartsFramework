...
class PolylineOptimizer:
    ...
    def run(
        ...
        if not nodo_list:
            msg = (
                ...
                f"nodo_list tiene {n_nodos} nodo(s).\n"
        ...
        print(f"############################ type_groups: {type_groups}")
        for inst_key, entries in type_groups.items():
            ...
            optimizer_input = build_optimizer_input(group_dict)
    ...
    def _show_dialog(
        ...
        """Muestra un MessageBox de Windows con el resultado de la optimización."""
        try:
            import ctypes
            MB_OK = 0x0
            MB_TOPMOST = 0x40000
            if error:
                ctypes.windll.user32.MessageBoxW(
                    ...
                    MB_OK | 0x10 | MB_TOPMOST,  # MB_ICONERROR
                )
            else:
                ctypes.windll.user32.MessageBoxW(
                    0,
                    ...
                    "Generar Camino Óptimo",
                    MB_OK | 0x40 | MB_TOPMOST,  # MB_ICONINFORMATION
                )
        except Exception:
            pass
