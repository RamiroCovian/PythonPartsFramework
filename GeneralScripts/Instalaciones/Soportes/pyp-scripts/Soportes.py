...
try:
    try:
        ...
        from SoportesFromJson import (
            SupportJson,
            load_supports_from_json,
            compute_segment_length,
            get_default_json_path,
        )

        print("[Soportes] SoportesFromJson importado como módulo suelto.")
    except ImportError:
        # Caso 2: este script se carga como Soportes.Soportes dentro de un paquete
        from .SoportesFromJson import (  # type: ignore[import-not-found]
            SupportJson,
            load_supports_from_json,
            compute_segment_length,
            get_default_json_path,
...
class SupportModel:
    ...
    def __init__(self, build_ele, doc: AllplanElementAdapter.DocumentAdapter):
        ...
        # 1.c) Seguridad: si no está en PARAMS, caer a Zeta (puede ser sobreescrito por JSON después)
        if type_support not in PARAMS:
            print(
                f"[SupportModel] Tipo de soporte desde UI: '{type_support}' "
                f"(no válido o vacío, usando 'Zeta' por defecto — se puede sobreescribir por JSON)"
            )
            type_support = "Zeta"

        self.type_support = type_support
        print(f"[SupportModel] Tipo de soporte inicial (UI): '{self.type_support}'")
        ...
        if combo_param_name:
            ...
            print(
                f"[SupportModel] Omega/ Zeta: support_key='{self.support_key}', idx={self.support_idx}"
            )
    ...
    def apply_json_definition(self, support_json):
        ...
        if self.type_support == "Omega":
            ...
            idx = 0
            print(
                "[SupportModel] Cinta: variante única idx=0 (layer y atributos desde PARAMS)"
            )
        ...
        if self.type_support == "Omega" and self.support_key == "Electr./Clima(SEP)":
            if inferred_installation_type in ("Clima", "Electricidad"):
                ...
                if pmp_nom:
                    self.param["pmp_nom"] = pmp_nom
                print(
                    "[SupportModel] JSON: layer='{}', pmp_nom='{}' "
                    "(subtipo={})".format(layer, pmp_nom, subtipo_raw)
                )

        # En Varifix, Clima/Agua/Saneamiento también se deduce desde subtipo semántico.
        if self.type_support == "Omega" and self.support_key == "Varifix":
            if inferred_installation_type in ("Clima", "Agua", "Saneamiento"):
                layer = MAP_LAYER_BY_INSTALLATION.get(inferred_installation_type)
                pmp_nom = MAP_PMP_NOM_BY_INSTALLATION.get(inferred_installation_type)
                if layer:
                    self.param["LAYER_SHORT"] = layer
        ...
        if self.type_support == "Zeta" and inferred_installation_type:
            ...
            print(
                ...
                "(subtipo={}, idx={}, 0mm={})".format(
                    layer,
                    pmp_nom,
                    self.param.get("ATTR01"),
                    self.param.get("ATTR05"),
                    subtipo_raw,
                    self.support_idx,
                    is_0mm,
                )
        ...
        print(
            f"[SupportModel] Configuración desde JSON: tipo='{self.type_support}', subtipo='{subtipo_raw}', idx={self.support_idx}"
        )
    ...
    def _create_capsule_brep(self, param: dict):
        ...
        - Omega / Cinta: al inicio y al final.
        ...
        if brep_centered is not None:
            ...
            if outer_layer_short:
                ...
                if outer_layer_id > 0:
                    ...
                    print(
                        f"[SupportModel] Aviso: layer '{outer_layer_short}' no encontrado en el documento "
                        f"(GetIDByShortName=0). Añade el layer al proyecto o verifica el nombre."
                    )
...
def create_element(build_ele, doc: AllplanElementAdapter.DocumentAdapter):
    ...
    - En Omega, `subtipo` decide también capa y `pmp_nom` para:
      - `Electr./Clima(SEP)`: `Electricidad`
      - `Varifix`: `Clima`/`Agua`/`Saneamiento`
    - Cinta: variante única (idx=0), subtipo por defecto "Agua".
      Layer (`IS_CINTA_PERFORADA`) y atributos ya definidos en PARAMS.
    ...
    # Forzar refresco de helpers JSON para evitar desalineación por caché
    ...
    if json_supports:
        ...
        for js in json_supports:
            ...
            try:
                cota_b = _json_get_value(
                    js, "cota_b", "height_b", "length_b", default=0.0
                )
            ...
            try:
                ...
                if p1 is not None and p2 is not None:
                    ...
                    # Matriz de rotación que lleva el eje X local (1,0,0)
                    # al vector 3D definido por posicion1->posicion2.
                    mat_rot = AllplanGeometry.Matrix3D()
                    start_dir = AllplanGeometry.Vector3D(1.0, 0.0, 0.0)
                    mat_rot.SetRotation(start_dir, direction_vec)

                    # Rotación adicional por ángulo de inclinación (paralelo al IS),
                    # proveniente de `angulo_inclinacion` en JSON.
                    ...
                    if (
                        ...
                        mat_rot = mat_roll * mat_rot

                    # Matriz de traslación al punto inicial
                    mat_tra = AllplanGeometry.Matrix3D()
                    mat_tra.SetTranslation(AllplanGeometry.Vector3D(x1, y1, z1))

                    # Primero rotar, luego trasladar
